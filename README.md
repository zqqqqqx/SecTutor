...
### 第一次安装会弹 Windows 警告（正常现象）

安装包**没做代码签名**，所以 Windows 会弹 SmartScreen 蓝框「Windows 已保护你的电脑」。不是中毒，是微软对所有未签名程序的统一处理。三步继续：

1. 点框里的 **「更多信息」**（蓝色小字，别点"确定"，点了会直接关掉）
2. 出现 **「仍要运行」** 按钮，点它
3. 之后正常走安装向导

只有第一次安装会这样，装完启动不再弹。要核验文件是否被篡改，用 Release 说明里给的 md5 / sha512 对一下。

**为什么不直接签名**：签名证书对个人开发者一年几百块，更麻烦的是本项目属于安全工具类——SignPath 这类面向开源的免费签名在条款里明确排除了"主动扫描并指出可利用漏洞"的软件，走不通；Azure 的签名服务又只面向美加的个人开发者。等有稳定用户量了再买证书，届时发布者名称正常显示，这步也能省掉。
