...
(function () {
  ...
  const ALIAS_RULES = [
    ...
    // ↓ v1.5.6 依据新基准（103 条）的真实失败案例扩充：口语长句与正确知识点词面几乎不重合，
    //   纯词法覆盖不到，只能靠这类"口语 → 术语"映射。实测把口语组 P@1 从 25% → 85%。
    [/改我?网页|网页上的内容|网页内容被改/, "xss 跨站脚本 篡改"],
    [/数据库.*拖走|拖库|数据库被.*拿走/, "sqli 脱库 数据库 注入"],
    [/当成跳板|跳板|被人利用来攻击/, "ssrf 代理 跳板"],
    [/密码加盐|加盐|口令怎么存/, "hash 加盐 口令存储"],
    [/安全传数据|安全传输|两台机器.*传/, "tls 加密传输 密钥协商"],
    [/数据拷走|拷走|带走了?数据|内部人员/, "dlp 数据防泄漏 内部威胁"],
    [/日志没被改|日志.*改过|日志完整/, "完整性 签名 审计日志"],
    [/文件是不是恶意|判断.*恶意|样本分析/, "malware 沙箱 静态分析"],
    [/钓鱼邮件|邮件.*识别|识别.*钓鱼|点开链接就中招/, "social 钓鱼 邮件安全"],
    [/桶配置错|存储桶|对象存储/, "cloud 存储 配置错误"],
  ...
  const I18N = {
    zh: {
      ...
      "update.hint": "SecTutor 会在启动后自动检查更新（不打断你的操作）。你也可以随时手动检查；下载完成后可立即重启安装，或等下次退出时自动安装。",
      "btn.checkUpdate": "🔄 检查更新", "btn.downloadUpdate": "⬇ 下载更新", "btn.installUpdate": "✅ 安装并重启"
    },
    en: {
      ...
      "update.hint": "SecTutor checks for updates after startup (without interrupting your work). You can also check manually; once downloaded, restart to install now, or let it install automatically the next time you quit.",
      "btn.checkUpdate": "🔄 Check for updates", "btn.downloadUpdate": "⬇ Download update", "btn.installUpdate": "✅ Install & restart"
    }
  };
  // 最近一次收到的更新状态快照。声明在此处（而非更新模块内）是为了避开 TDZ：
  // applyLang 会在 init() 一开始就调用，此时必须已初始化。
  ...
  async function chatCompletions(messages, tools, onChunk, signal) {
    ...
    const resp = await fetch(base + "/chat/completions", {
      ...
      signal: signal || undefined,      // 副驾驶的「停止生成」用得上
    });
...
--- 资料结束 ---`;
  ...
  /* ---------- v1.2.0：自动更新（仅 Electron 桌面安装版） ---------- */
  // 前端只做两件事：渲染主进程推来的状态、触发「检查 / 下载 / 安装」三个动作。
  // 所有判断逻辑都在主进程，前端不自行判断版本号，避免出现两套真相。
  // 网页版（window.sectutor 无 updateState）整块隐藏，不留痕迹、不报错。
  const UPDATER = (window.sectutor && window.sectutor.updateState && window.sectutor.onUpdateState)
    ? window.sectutor : null;

  // 错误分类 → 中英文可读文案（与主进程 updater-core.classifyError 的分类对应）。
  // 未知分类回退到通用文案 + 原始错误摘要，绝不让用户看到一段技术黑话就懵掉。
  const UPDATE_ERR_TEXT = {
    network: {
      zh: "无法连接更新服务器（GitHub 网络不可达）。可稍后重试，或去 GitHub Releases 手动下载新版安装包。",
      en: "Could not reach the update server (GitHub unreachable). Retry later or download the new installer from GitHub Releases.",
    },
    ratelimit: {
      zh: "更新检查过于频繁被 GitHub 限流，请过一会儿再试。",
      en: "Too many update checks (GitHub rate limit). Please try again later.",
    },
    forbidden: {
      zh: "无权访问更新源（仓库可能为私有或访问令牌已失效）。其余功能不受影响。",
      en: "No access to the update source (private repo or expired token). Everything else works fine.",
    },
    unreleased: {
      zh: "更新源异常：最新 GitHub Release 未发布，或缺少更新清单（latest.yml）。",
      en: "Update source issue: the latest GitHub Release is missing or has no update manifest (latest.yml).",
    },
    corrupt: {
      zh: "下载内容校验失败（文件可能损坏），请重新下载。",
      en: "The downloaded file failed its integrity check. Please download again.",
    },
  };
  ...
  const UPDATE_DISABLED_TEXT = {
    ...
    loader: {
      zh: "自动更新组件未能加载，本次运行不启用更新。",
      en: "The update component could not be loaded; updates are disabled this session.",
    },
    dev: {
      zh: "开发模式不启用自动更新。",
      en: "Auto-update is disabled in development mode.",
    },
  };
  ...
  function renderUpdateState(st) {
    ...
    if (!status) return;
    lastUpdateState = st;   // 供切换语言时按当前语言重绘
    const en = curLang() === "en";
    const pctTxt = st.progress == null ? 0 : Math.round(st.progress);
    ...
    // —— 按钮显隐：任何时刻只暴露当前该做的那一个动作 ——
    // 出错但「有新版本」仍保留下载钮 = 上次下载失败可直接重试；出错且无版本 = 保留检查钮重试。
    ...
    // 进度条仅在下载中显示
    if (bar) {
      if (st.downloading) { bar.style.display = ""; if (fill) fill.style.width = pctTxt + "%"; }
      else bar.style.display = "none";
    }

    let text, cls = "backend-status";
    if (st.downloaded) {
      text = en ? `Update ${st.version || ""} is ready. Install whenever you are done.` : `更新 ${st.version || ""} 已就绪，准备好后点击安装。`;
      cls += " ok";
    } else if (st.downloading) {
      text = en ? `Downloading ${st.version || ""}… ${pctTxt}%` : `正在下载 ${st.version || ""}… ${pctTxt}%`;
    } else if (st.checking) {
      text = en ? "Checking for updates…" : "正在检查更新…";
      ...
      if (known) text = known[en ? "en" : "zh"];
      else text = en ? ("Update failed: " + (st.error || "unknown error")) : ("更新失败：" + (st.error || "未知错误"));
      cls += " off";
    } else if (st.checkedAt) {
      text = en ? `Up to date (v${st.currentVersion || ""}).` : `已是最新版本（v${st.currentVersion || ""}）。`;
      cls += " ok";
    } else {
      // 尚未做过检查：不假装「已是最新」
      text = en ? `Auto-check happens after startup. (v${st.currentVersion || ""})` : `打开后会自动检查更新（当前 v${st.currentVersion || ""}）。`;
    }
  ...
  function setupUpdater() {
    const box = $("#updateCtl");
    if (!UPDATER || !box) return;
    box.style.display = "";   // 仅桌面版显示
    // 更新属于「锦上添花」，任何异常都静默吞掉，绝不影响学习主流程。
    const guard = (p) => Promise.resolve(p).catch(() => {});

    const btnCheck = $("#checkUpdate");
    if (btnCheck && !btnCheck.dataset.bound) {
      btnCheck.dataset.bound = "1";
      btnCheck.addEventListener("click", () => {
        btnCheck.disabled = true;
        guard(UPDATER.checkUpdate()).then(() => { btnCheck.disabled = false; });
      });
    }
    const btnDown = $("#downloadUpdate");
    if (btnDown && !btnDown.dataset.bound) {
      btnDown.dataset.bound = "1";
      btnDown.addEventListener("click", () => guard(UPDATER.downloadUpdate()));
    }
    const btnInst = $("#installUpdate");
    if (btnInst && !btnInst.dataset.bound) {
      btnInst.dataset.bound = "1";
      btnInst.addEventListener("click", () => {
        if (btnInst.disabled) return;   // 防连点：安装会重启应用，双击会导致重复 quitAndInstall
        btnInst.disabled = true;
        const original = btnInst.textContent;
        btnInst.textContent = curLang() === "en" ? "Restarting…" : "正在重启应用…";
        guard(UPDATER.installUpdate().then((r) => {
          // 被主进程守卫拒绝（如状态未就绪）时还原按钮，允许用户再点
    ...
    UPDATER.onUpdateState((st) => renderUpdateState(st || {}));
    // 首帧先拉一次快照，避免刚打开界面时状态区停留在占位文案。
    guard(UPDATER.updateState()).then((st) => { if (st) renderUpdateState(st); });
  }
  ...
  function init() {
    ...
    // 自动更新（v1.2.0）：放在最后，桌面版才接管；网页版内部直接 return。
    setupUpdater();
  }
  ...
  // 今日主线当天固定：一旦生成就落盘，避免「做完一步它就消失」导致看不到完成标记
  const TODAY_PLAN_KEY = "sectutor_today_plan";
  function todayStamp() {
    const d = new Date();
  ...
  function loadTodayPlan() {
    try {
      const p = JSON.parse(localStorage.getItem(TODAY_PLAN_KEY) || "null");
      if (!p || p.d !== todayStamp() || !Array.isArray(p.items) || !p.items.length) return null;
      if (!p.items.every((it) => allTopics().some((t) => t.id === it.id))) return null;   // 数据变了就重算
      return p;
    } catch (e) { return null; }
  }
  function saveTodayPlan(items) {
    try {
      localStorage.setItem(TODAY_PLAN_KEY, JSON.stringify({ d: todayStamp(), items: items }));
    } catch (e) {}
  }
  const MINUTES_BY_KIND = (t, kind) => (kind === "quiz" ? quizMinutes(5) : readingMinutes(t));

  // 今天发生过的事件（用于标记「今天已完成」）——数据来自 state.activity，无需额外存储
  function todayEvents() {
    const d = new Date(); d.setHours(0, 0, 0, 0);
    const ts = d.getTime();
    return (state.activity || []).filter((e) => e && e.t >= ts);
  }
  // 只有能真正验证的步骤才标记完成：学/复习看「已掌握」事件、复习看 review、自测看 quiz
  function stepDone(s) {
    const ev = todayEvents();
    if (s.kind === "quiz") return ev.some((e) => e.type === "quiz");
    if (s.kind === "review") return ev.some((e) => e.type === "review");
    return !!(s.topic && ev.some((e) => e.type === "master" && e.meta === s.topic.id));
  }
  ...
  const DOMAIN_SHORT = { web: "Web", binary: "二进制", crypto: "密码学", pentest: "渗透", network: "网络", cloud: "云原生", blue: "蓝队", mobile: "移动", datasec: "数据", supply: "供应链", aisec: "AI", soccti: "SOC" };
  ...
  function todayPlan() {
    ...
    // —— 当天已有计划：直接复用（这样完成的步骤仍留在列表里，能显示 ✓）——
    const saved = loadTodayPlan();
    if (saved) {
      saved.items.forEach((it) => {
        const t = allTopics().find((x) => x.id === it.id);
        if (t) steps.push({ kind: it.kind, topic: t, mins: MINUTES_BY_KIND(t, it.kind), why: it.why });
      });
      if (steps.length) {
    ...
    // 落盘：今天剩下的时间都用这份计划（换一个会更新它）
    saveTodayPlan(steps.filter((s) => s.topic).map((s) => ({ id: s.topic.id, kind: s.kind, why: s.why })));
  ...
  function renderTodaySteps() {
    ...
    host.innerHTML = plan.map((s, i) => {
      ...
      const label = done
        ? (s.kind === "quiz" ? "再测一组" : (s.kind === "review" ? "再复习" : "再看一遍"))
        : (s.kind === "review" ? "开始复习" : (s.kind === "learn" ? "开始学习" : "开始自测"));
      ...
      return '<li class="today-step' + (i === 0 && !done ? ' now' : '') + (done ? ' done' : '') + '">' +
        ...
        timeText + ' · ' + meta + (done ? ' · <span class="today-done">✓ 今天已完成</span>' : '') + '</div>' +
    ...
    $$("#todaySteps .today-alt").forEach((b) => {
      ...
      b.addEventListener("click", () => {
        ...
        if (rerollTodayStep(s.topic.id)) renderTodaySteps();
        else renderTodaySteps();     // 没有可换的（该领域只剩这一个）——保持原样也不报错
      });
    });
  }
  ...
  function renderTodayAbility() {
    ...
    const weekSub = $("#todayWeekSub");
    if (weekSub) {
      const d14 = activityByDay(14);
      const thisWeek = d14.slice(7).reduce((a, b) => a + b, 0);
      const lastWeek = d14.slice(0, 7).reduce((a, b) => a + b, 0);
      const diff = thisWeek - lastWeek;
      weekSub.textContent = "本周 " + thisWeek + " · 上周 " + lastWeek +
    ...
    if (dist && legend) {
      ...
      dist.innerHTML = segs.filter((s) => s.n > 0)
        .map((s, i) => {
          const lv = (s.lv === "已掌握") ? "" : s.lv;   // 已掌握没有对应筛选档，只跳转到知识库
          return '<i class="today-c' + i + ' today-seg" role="button" tabindex="0" data-lv="' + lv +
            '" style="--w:' + ((s.n / total) * 100).toFixed(1) + '%" title="' + s.lv + " " + s.n + '，点击查看"></i>';
        }).join("");
  ...
  // 把当日计划里的某个知识点换成同领域的另一个（「换一个」按钮）
  function rerollTodayStep(oldId) {
    const saved = loadTodayPlan();
    if (!saved) return false;
    const idx = saved.items.findIndex((it) => it.id === oldId);
    if (idx < 0) return false;
    const t = allTopics().find((x) => x.id === oldId);
    if (!t) return false;
    const taken = new Set(saved.items.map((it) => it.id));
    const pool = TOPICS.filter((x) => x.cat === t.cat && !state.mastery.has(x.id) &&
      !taken.has(x.id) && !todaySkipped.has(x.id));
    const next = pickTopic(pool);
    if (!next) return false;
    const catName = (catById(t.cat) || {}).name || "同领域";
    saved.items[idx] = {
      id: next.id,
      kind: saved.items[idx].kind,
      why: (saved.items[idx].kind === "review" ? "同领域复习：" : "同领域换一个：") + catName,
    };
    saveTodayPlan(saved.items);
    return true;
  }

  // 点「难度分布」的某一段 → 跳到知识库并套用对应难度筛选（让图表变成入口）
  function bindTodayDist() {
    const host = $("#todayDist"); if (!host || host.dataset.bound) return;
    host.dataset.bound = "1";
    const go = (seg) => {
      if (!seg) return;
      const lv = seg.dataset.lv;
      activateTab("knowledge");
      if (lv) {
        const chip = $('#levelChips .chip[data-level="' + lv + '"]');
        if (chip) chip.click();
      }
    };
    host.addEventListener("click", (e) => { if (e.target && e.target.classList.contains("today-seg")) go(e.target); });
    host.addEventListener("keydown", (e) => {
      if ((e.key === "Enter" || e.key === " ") && e.target && e.target.classList.contains("today-seg")) {
        e.preventDefault(); go(e.target);
      }
    });
  }
  ...
  function renderToday() {
    try {
      ...
      // 自测钩子：让回归能确定性地验证今日主线（沿用项目既有的 window.__SELFTEST__ 模式）
      if (window.__SELFTEST__) {
        window.__todayPlan = todayPlan;
        window.__renderToday = renderToday;
        window.__showTopic = showTopicDetail;
      }
      const diagBtn = $("#todayDiag");
      if (diagBtn && !diagBtn.dataset.bound) {
        diagBtn.dataset.bound = "1";
        diagBtn.addEventListener("click", () => {
          // #diagArea 在「学习计划」面板里：必须先切过去，否则诊断题渲染在看不见的地方
          activateTab("plan");
          startDiagnosis();
          const area = $("#diagArea");
          if (area && area.scrollIntoView) area.scrollIntoView({ block: "center" });
        });
      }
    } catch (e) {
  ...
  let copilotAbort = null;        // 当前请求的中止句柄（「停止生成」）
  ...
  // 每条回答下方的操作条（复制）
  function cpToolsHtml() {
    return '<div class="cp-tools"><button type="button" class="cp-act" data-copy="1">复制</button></div>';
  }
  function renderCopilotBusy(on) {
    const stop = $("#copilotStop"); if (stop) stop.hidden = !on;
    const send = $("#copilotSend"); if (send) send.disabled = !!on;
  }
  function clearCopilot() {
    copilotHistory = [];
    saveCopilot();
    const log = $("#copilotLog"); if (log) log.innerHTML = "";
    renderCopilotHints();
  }
  ...
  async function copilotAsk(q) {
    ...
    if (!state.llm || !state.llm.key) {
      ...
      const tipBubble = copilotAdd("bot", "还没配置大模型，所以我暂时没法在这里回答。<br>" +
        "打开右上角 <b>设置 → 模型</b> 填入 API Key（只需一次），之后我就能一直在顶栏待命。<br>" +
        '<span class="cp-dim">不配置也不影响其它功能：知识库、靶场、自测都能正常用。</span>');
      if (tipBubble) tipBubble.innerHTML += cpToolsHtml();
    ...
    try {
      ...
      if (bubble) bubble.innerHTML = mdLite(ans) + copilotSrcHtml(doc.docs) + cpToolsHtml();
      ...
      if (bubble) {
        if (aborted) {
          bubble.innerHTML = acc
            ? mdLite(acc) + '<div class="cp-src">（已停止，以上为已生成部分）</div>' + cpToolsHtml()
            : '<span class="cp-dim">已停止生成。</span>';
        } else {
          bubble.innerHTML = '<span class="cp-err">调用失败：' + escapeHtml(e && e.message ? e.message : String(e)) +
            ...
            cpToolsHtml();   // 报错内容也允许复制，便于反馈问题
        }
      }
    } finally {
      copilotBusy = false;
      copilotAbort = null;
      renderCopilotBusy(false);
    }
  }
  ...
  function bindCopilot() {
    ...
    if (clearBtn && !clearBtn.dataset.bound) { clearBtn.dataset.bound = "1"; clearBtn.addEventListener("click", clearCopilot); }
    const stopBtn = $("#copilotStop");
    if (stopBtn && !stopBtn.dataset.bound) {
      stopBtn.dataset.bound = "1";
      stopBtn.addEventListener("click", () => { if (copilotAbort) copilotAbort.abort(); });
    }
    const logEl = $("#copilotLog");
    if (logEl && !logEl.dataset.boundCopy) {
      logEl.dataset.boundCopy = "1";
      logEl.addEventListener("click", (e) => {
        const btn = e.target && e.target.closest ? e.target.closest("[data-copy]") : null;
        if (!btn) return;
        const bubble = btn.closest(".cp-bubble");
        const text = bubble ? bubble.textContent.replace(/复制|已复制/g, "").trim() : "";
        if (!text) return;
    ...
    // Mac 上显示 ⌘ 而不是 Ctrl
    const kbd = $("#copilotKbd");
    if (kbd) kbd.textContent = (/Mac|iPhone|iPad/.test(navigator.userAgent) ? "⌘ /" : "Ctrl /");
    renderCopilotCtx();
  }
