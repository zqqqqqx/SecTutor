...
# 或用专用扫描工具查看协议、套件、证书链与 HSTS`,
        ...
        {
          id: "crypto-random", name: "熵源与随机数实践：怎么拿到不可预测的数", level: "中级",
          summary: "知道该用 CSPRNG 之后，工程上还要回答：熵从哪来、什么时候会耗尽、虚拟机和容器里为什么容易出问题。",
          keywords: ["熵源", "getrandom", "urandom", "熵池", "crypto.randomBytes"],
          levels: {
            "入门": "密码学要求随机数「不可预测」，而计算机只能算出确定的结果。所以真正的随机来自外部（硬件噪声、中断时序），系统把它们收集进「熵池」，再由随机数接口提供给程序。",
            "初级": "实践上别自己写随机数，用平台提供的安全接口：Linux 的 getrandom()/urandom、Python 的 secrets 与 os.urandom、Node 的 crypto.randomBytes、浏览器与移动端的 CSPRNG。注意区分「统计上均匀」（rand）与「密码学安全」（CSPRNG）。",
            "中级": "常见坑：① 熵池不足时早期启动阶段生成的密钥可能偏弱（系统刚开机、容器刚起来）；② 虚拟机/容器克隆导致多实例拿到同一份「随机」（镜像快照把状态一起复制了）；③ 自己实现「时间戳 + PID」当种子，攻击者能猜。对策：用 CSPRNG、给密钥生成留出熵充足的时间、克隆后重新初始化。",
            "高级": "进阶：把「密钥生成」当成关键路径做验证——检查熵源可用性、对生成的密钥做统计与熵评估、在 CI 里用测试向量校验随机接口（如 NIST 的 RNG 测试向量）；对嵌入式设备优先选带硬件随机数发生器的方案，并验证其健康检测是否生效。",
          },
          codeLang: "python",
          code:
`# 正确与错误的随机数用法（Python / Linux）
# ✗ 不要用：可预测，攻击者能从种子推回来
import random, time
random.seed(int(time.time())); token = random.randbytes(16)

# ✓ 要用：平台 CSPRNG（Linux 上走 getrandom/urandom）
import secrets, os
print(secrets.token_urlsafe(32))
print(os.urandom(16).hex())

# 检查 Linux 熵池充足性（嵌入式/容器里排查弱随机）
# cat /proc/sys/kernel/random/entropy_avail
# 需要时：rngd / haveged 补充熵源（先确认硬件 RNG 可用）`,
          tool: "secrets、crypto.randomBytes、getrandom(2)、rng-tools、NIST RNG 测试向量",
          refs: "NIST SP 800-90A（随机数生成）、Linux random(7)"
        },
        {
...
`,
        ...
        {
          id: "blue-sample", name: "恶意样本分析：静态与动态", level: "中级",
          summary: "拿到一个可疑文件，先看它是什么（静态），再让它跑起来看它做什么（动态），最后才谈拦截。",
          keywords: ["样本分析", "静态分析", "动态分析", "沙箱", "IOC 提取", "恶意样本"],
          levels: {
            "入门": "判断一个文件是不是恶意，不能只看文件名和图标。基本套路是：先不运行它、只「看」它长什么样，再把它放到隔离环境里跑起来观察行为。",
            "初级": "静态分析看四样东西：文件类型与真实格式（改后缀骗不了人）、字符串里有没有可疑域名或命令、引入了哪些系统调用、是否带壳或加密。动态分析则记录它启动后做了什么：创建了什么进程、改了哪些注册表或文件、连了哪些外部地址。",
            "中级": "样本分析产物要能落地成**检测规则**：把外联域名/IP、落地文件路径、命令行特征提取成 IOC，再转成 SIEM 规则或 EDR 检测项。注意沙箱会「识破」：样本若检测到虚拟环境就休眠，需要等待时间、模拟交互或换环境复跑。",
            "高级": "进阶关注反分析对抗：反调试、反沙箱（检测 CPU 核数/内存/驱动名）、时间加速绕过、分段加载与内存执行（无文件落地）。对抗手段是内存取证、API 监控与内核回调，以及在真实终端上用 EDR 遥测补足沙箱看不到的部分。",
          },
          codeLang: "bash",
          code:
`# 分析一个可疑样本的最小流程（务必在隔离环境里做）
file sample.bin                       # 真实文件类型（改扩展名骗不了 file）
strings -n 8 sample.bin | head -40     # 可疑域名 / 命令 / 路径
sha256sum sample.bin                  # 先算哈希，用于情报比对
exiftool sample.bin | head -20        # 编译时间 / 语言线索
# 动态：丢进隔离机或沙箱，抓进程树与外联
#   Procmon：文件/注册表行为   Wireshark：外联   沙箱报告：自动汇总`,
          tool: "file、strings、ExifTool、Procmon、Wireshark、Cuckoo/在线沙箱",
          refs: "MITRE ATT&CK（行为映射）、PE/ELF 格式规范"
        },
        {
          id: "blue-forensics", name: "主机取证基础：时间线与证据链", level: "中级",
          summary: "出事之后要回答三件事：什么时候、做了什么、还能不能复现——靠的是时间线与证据链。",
          keywords: ["主机取证", "时间线", "证据链", "易失性顺序", "内存取证"],
          levels: {
            "入门": "取证就是「事后还原真相」：谁在什么时间做了什么。和把服务器重启一下接着用相反——很多线索重启就没了。",
            "初级": "取证有**易失性顺序**：越容易消失的越先取。顺序大致是：内存与进程 → 网络连接与路由 → 登录会话与命令历史 → 磁盘文件与日志。全程记录「谁在什么时间取了什么、哈希是多少」，这就是证据链。",
            "中级": "做**时间线**是核心方法：把文件时间戳（M/A/C/B 四种）、日志时间、进程启动时间统一到一张表，异常活动会自己浮出来。注意时区与时钟漂移，先校正再对齐；取证镜像要在写保护下操作，先做哈希再分析。",
            "高级": "进阶：内存取证找无文件落地与注入痕迹（进程内存里的可疑模块、网络连接）；日志被删时用文件系统残留（USN 日志、预取、注册表）反推；最后把发现映射到 ATT&CK 技战术，输出可复用的检测项。",
          },
          codeLang: "bash",
          code:
`# 主机取证顺序（越易失的越先取）
# 1) 内存与进程
winpmem / LiME                    # 内存镜像（先取内存，重启即失）
tasklist /v  或  ps auxf          # 进程树
# 2) 网络与连接
netstat -ano / ss -antp           # 外联与监听
# 3) 会话与历史
last / who                        # 登录记录
...
sha256sum disk.img > disk.img.sha256`,
          tool: "Volatility、winpmem/LiME、Autopsy、Plaso/log2timeline、KAPE",
          refs: "NIST SP 800-86（取证指南）、RFC 3227（易失性顺序）"
        },
      ]
    },
    /* ---------------- 移动安全 ---------------- */
...
`,
        ...
        {
          id: "ds-compliance", name: "等保与合规要求", level: "初级",
          summary: "等保不是「买设备就能过」：它是一套按等级划定的安全要求清单，重点是能不能说清、能不能查证。",
          keywords: ["等保", "网络安全法", "合规", "等级保护", "审计留痕"],
          levels: {
            "入门": "等保（网络安全等级保护）是按系统的重要程度分等级，再按等级配一套安全要求。核心不是买设备，而是「有制度、有落实、有记录」。",
            "初级": "常见的五个维度：身份与访问控制（谁能进）、审计与留痕（做了什么有记录）、边界与访问限制（不该通的就不通）、入侵防范与恶意代码（能不能发现）、数据备份与恢复（丢了能不能回来）。日志留存时间与密码强度这类是硬指标，评审会直接查。",
            "中级": "落地要点：把「制度文件」和「技术措施」对齐——每一条要求都能指出对应配置或流程。常见失分点是**审计记录不完整**（只在应用层记、系统层没记）与**权限过大**（运维账号权限没有最小化、缺少审批）。整改优先做「可查证」的部分：日志、备份、权限清单。",
            "高级": "进阶视角：把合规要求映射到技术控制项（类似 CIS/ISO 27001 的控制族），做成台账并自动化校验（配置基线扫描、日志留存轮检）；对多系统做统一「等级 + 定级报告 + 测评证据」管理，把合规从一次性测评变成持续状态。",
          },
          codeLang: "bash",
          code:
`# 合规自查的最小清单（可查证 = 有记录）
# ① 日志留存是否够（等保一般要求不少于 6 个月）
 grep -c '' /var/log/secure   /var/log/audit/audit.log
# ② 是否最小权限：列出特权账号并确认审批
awk -F: '$3==0{print $1}' /etc/passwd
# ③ 备份是否可恢复（不是「有没有备份」，是「恢复过没有」）
ls -l /backup/*.sql.gz | tail -5
# ④ 基线核查：跑一遍配置基线，留存报告作为证据
lynis audit system | tail -20`,
          tool: "Lynis、CIS-CAT、OpenSCAP、等保测评自查表",
          refs: "GB/T 22239（等保 2.0 基本要求）、网络安全法"
        },
        {
          id: "ds-thirdparty", name: "第三方与外包数据风险", level: "中级",
          summary: "数据交出去之后，责任还在你身上：第三方的数据风险要按「能收不能留、能看不能取」来管。",
          keywords: ["第三方", "外包", "供应商风险", "数据共享", "委托处理"],
          levels: {
            "入门": "业务外包、用第三方系统、把数据交给服务商处理，都会让数据离开你的边界。问题是：一旦泄露，责任和后果还是你的。",
            "初级": "先分清两类：**委托处理**（你定目的，对方按你要求处理）与**共同处理/共享**（双方都能决定用途）。前者的责任边界更清楚，所以能用委托就别用共享。交接要有合同条款与数据清单（给了哪些字段、给多少、存多久）。",
            "中级": "落地控制：① 只给必要字段（能脱敏就脱敏、能令牌化就令牌化）；② 约定删除与返还（合作结束多久内删完、怎么证明）；③ 访问最小化（对方只调接口，不落地副本）；④ 出口可观测（走数据网关统一出网，留审计记录）。",
            "高级": "进阶：把供应商纳入风险分级——接触敏感数据的定期做安全评估（问卷 + 渗透/审计报告）；在技术层做「数据不落地」设计（API 调用 + 结果脱敏返回）；对共享的数据做水印或指纹，一旦外泄可溯源是哪个合作方流出的。",
          },
          codeLang: "bash",
          code:
`# 第三方数据出口的最小核查点
# ① 出口是否统一（能不能枚举谁在往外传数据）
 grep -rE 'https?://' deploy/partners/*.yml | head   # 合作方接口清单
# ② 传出去的字段是否已最小化/脱敏
curl -s https://partner.example/api/push | jq 'keys'   # 只应出现必要字段
# ③ 出网审计是否留痕（数据网关/代理日志）
grep -c 'partner.example' /var/log/datagateway/egress.log
# ④ 删除条款是否有技术手段验证
#    要求对方提供删除证明，或在合同里约定抽查权`,
          tool: "数据网关/代理日志、DLP 出口规则、供应商安全问卷",
          refs: "《个人信息保护法》委托处理条款、ISO 27001 A.15 供应商关系"
        },
      ]
    },
    /* ---------------- 供应链安全 ---------------- */
...
`,
  ...
  tools: [
    ...
    { id:"t_netcat", name:"Netcat / Ncat", cat:"network",
      desc:"网络连通性排查与手工传数据的瑞士军刀",
      usage:"测端口是否通 → 手动发一段原始请求看响应 → 需要时做端口转发或临时传文件（配合加密通道）。",
      example:"nc -vz host 22      测端口 ｜  nc -lvnp 4444      监听 ｜  nc host 80 手工发 HTTP 请求",
      note:"仅用于你管理的网络；做转发/监听前确认不会绕过既有的边界管控。" },
    { id:"t_tcpdump", name:"tcpdump", cat:"network",
      desc:"命令行抓包，服务器上没有图形界面时的首选",
      usage:"tcpdump -i eth0 -w cap.pcap 先落盘 → 抓完拷回本地用 Wireshark 深入分析 → 排查时加 -nn 关闭解析、用 -c 限制数量。",
      example:"tcpdump -i any -nn -c 200 'tcp port 443'  # 只看 443 且不做域名解析",
      note:"生产环境抓包会占带宽与磁盘，先评估影响；流量涉及他人通信时须有授权。" },
    { id:"t_zeek", name:"Zeek", cat:"network",
      desc:"把流量变成结构化日志（连接/HTTP/DNS/证书），便于长期审计",
      usage:"部署在镜像口 → 自动生成 conn.log/http.log/dns.log → 用脚本或 SIEM 解析这些日志做异常检测。",
      example:"zeek -r cap.pcap Log::default_logdir=./logs  # 离线分析一个 pcap，产出多张日志",
      note:"日志同样属于敏感数据（含内网地址与域名），按数据分级管理并限制访问。" },
    { id:"t_trivy", name:"Trivy", cat:"cloud",
      desc:"扫容器镜像、依赖、配置与密钥的多合一扫描器",
      usage:"CI 里对镜像扫描 → 按严重度设阈值卡住流水线 → 定期对已上线的镜像重扫（新漏洞会持续披露）。",
      example:"trivy image --severity HIGH,CRITICAL myapp:1.2.3",
      note:"扫描报告可能暴露镜像里的路径与版本信息，按内部资料保管。" },
    { id:"t_kube_bench", name:"kube-bench", cat:"cloud",
      desc:"按 CIS 基线核查 Kubernetes 集群配置",
      usage:"在节点或 Pod 里跑 → 按检查项分类看失败项 → 逐条评估是否适用再整改（不是所有基线都适合你的架构）。",
      example:"kube-bench run --targets=master,node  # 输出通过/失败/警告清单",
      note:"整改前先在测试集群验证，改控制面参数可能影响业务。" },
    { id:"t_falco", name:"Falco", cat:"cloud",
      desc:"容器运行时威胁检测：谁在容器里起了 shell、读了敏感文件",
      usage:"部署为 DaemonSet → 加载社区规则 → 调优告警（容器里起 shell、写 /etc、访问容器运行时 socket 都值得告警）。",
      example:"自定义规则示例：容器内出现 /bin/sh 交互式会话 → 告警并记录父进程链。",
      note:"规则不宜一上来全开（噪声会淹没真警报），先观察再逐步收紧。" },
    { id:"t_velociraptor", name:"Velociraptor", cat:"blue",
      desc:"大规模终端取证与主动狩猎：一次问所有主机的同一问题",
      usage:"在终端部署采集器 → 用 VQL 查询跨主机取证据（近期启动项、异常外联、可疑文件）→ 结果导出做时间线。",
      example:"查询示例：列出近 24 小时新增的服务与计划任务，跨全量终端汇总",
      note:"采集范围与留存要事先报备；取证数据属敏感信息，按案件范围限制访问。" },
    { id:"t_yara", name:"YARA", cat:"blue",
      desc:"用规则匹配样本特征（字符串/字节序列/结构），适合批量初筛",
      usage:"写规则 → 对文件或内存批量匹配 → 命中后交给人工确认（规则会有误报，需配合上下文）。",
      example:"rule demo { strings: $a = cmd nocase  condition: $a }  # 命中含 cmd 的文件",
      note:"规则本身不要公开真实内部特征（可能被对手规避）；样本按恶意代码流程隔离保管。" },
    { id:"t_sigma", name:"Sigma", cat:"blue",
      desc:"把检测思路写成与厂商无关的规则，再转成各家 SIEM 语法",
      usage:"按场景写规则（如可疑提权、凭据转储）→ 用转换器生成 Splunk/Elastic 查询 → 上线后看命中量与误报再调。",
      example:"转换：sigma convert -t splunk rule.yml  → 直接得到可用的 SPL 查询",
      note:"规则上线前先在历史数据里回放，确认误报率可接受。" },
    { id:"t_frida", name:"Frida", cat:"mobile",
      desc:"动态插桩：运行时 hook 函数、看参数、改返回值",
      usage:"在已 root/越狱的测试机上跑 frida-server → 用脚本 hook 目标函数 → 观察调用参数与返回（常用于验证本地校验、密钥派生）。",
      example:"frida -U -f com.example.app -l hook.js  # 附加并注入脚本",
      note:"只对你自己的应用或明确授权的目标使用；绕过他人 App 的技术措施可能违反法律与平台条款。" },
    { id:"t_mobsf", name:"MobSF", cat:"mobile",
      desc:"移动应用自动化安全测试：静态 + 动态一体化",
      usage:"上传 APK/IPA → 看静态报告（权限、硬编码密钥、导出组件）→ 需要时用动态分析环境跑起来看行为。",
      example:"docker run -p 8000:8000 opensecurity/mobile-security-framework-mobsf",
      note:"测试对象须是你拥有或获授权的应用；报告里常含硬编码密钥，按敏感信息处理。" },
    { id:"t_jadx", name:"jadx", cat:"mobile",
      desc:"把 APK/DEX 反编译成可读 Java，快速看逻辑",
      usage:"jadx-gui 打开 APK → 按包名/字符串搜索定位关键逻辑 → 结合 Frida 动态验证静态发现的结论。",
      example:"jadx -d out/ app.apk   # 命令行反编译到 out 目录",
      note:"反编译结果仅用于授权范围内的安全评估与学习，不得用于复制他人代码。" },
    { id:"t_presidio", name:"Microsoft Presidio", cat:"datasec",
      desc:"自动识别文本里的个人信息（PII）并做脱敏",
      usage:"接入文本流 → 用预置识别器找 PII（姓名/电话/身份证/卡号）→ 按用途选择替换、掩码或假名化。",
      example:"analyzer.analyze(text='联系 138****', language='zh') → 得到实体与位置",
      note:"脱敏不可逆时要先评估是否影响业务；保留映射表就等于保留了原始数据，须单独加固。" },
    { id:"t_ranger", name:"Apache Ranger", cat:"datasec",
      desc:"大数据平台的统一权限与脱敏策略（Hive/HDFS/Kafka）",
      usage:"按库/表/列配策略 → 对敏感列挂脱敏或行级过滤 → 所有访问走统一审计。",
      example:"策略示例：列级脱敏 phone（只显示后四位）+ 行级过滤限本部门数据",
      note:"策略变更前先在测试库验证，避免误伤正常业务查询。" },
    { id:"t_openscap", name:"OpenSCAP", cat:"datasec",
      desc:"按安全基线（CIS/等保可自建）扫描主机的合规状态",
      usage:"选基线（如 CIS Level 1）→ 扫描出报告 → 按失败项逐条整改 → 定期复扫并留存报告作为合规证据。",
      example:"oscap xccdf eval --profile cis --report report.html ssg-rhel.xml",
      note:"扫描本身只读，但报告含主机配置细节，按内部资料保管。" },
    { id:"t_syft", name:"Syft", cat:"supply",
      desc:"从镜像或目录生成 SBOM（软件物料清单）",
      usage:"对镜像/源码目录跑 → 得到全量组件清单（名称/版本/许可）→ 入库留档，配合漏洞库定期比对。",
      example:"syft myapp:1.2.3 -o spdx-json > sbom.json",
      note:"SBOM 会暴露内部组件与版本，对外提供前先评估信息泄露面。" },
    { id:"t_grype", name:"Grype", cat:"supply",
      desc:"拿 SBOM 或镜像去比对漏洞库，列出受影响组件",
      usage:"对镜像或 SBOM 扫描 → 按严重度与是否有修复版本排序 → 优先处理可修复且暴露在外的组件。",
      example:"grype sbom:./sbom.json --fail-on high  # CI 里卡住高危",
      note:"漏洞库有滞后与误报，处置前先确认组件是否真的被加载/可达。" },
    { id:"t_cosign", name:"Cosign（Sigstore）", cat:"supply",
      desc:"给镜像与制品签名，部署前验签，防「被换过的镜像」",
      usage:"生成密钥或用 keyless 签名 → 推送镜像后签名 → 部署侧（准入控制）验签，未签名直接拒绝。",
      example:"cosign sign --key cosign.key myapp:1.2.3 然后 cosign verify 校验",
      note:"私钥与身份凭据要纳入密钥管理，丢失等同无法签发可信制品。" },
    { id:"t_garak", name:"Garak", cat:"aisec",
      desc:"针对大模型的漏洞扫描器：提示注入、越狱、数据泄露",
      usage:"指向被测模型接口 → 选探测器组合（注入/越狱/泄露/幻觉）→ 出报告，按失败用例设计防护。",
      example:"garak --model_type openai --model_name gpt-4o-mini --probes injection",
      note:"只测你有权限的模型与接口；测试流量可能被计费与记录。" },
    { id:"t_promptfoo", name:"Promptfoo", cat:"aisec",
      desc:"把提示词与模型的评测做成可回归的测试集",
      usage:"写测试用例（输入 + 期望断言）→ 多模型/多提示词横评 → 改提示词后一键回归，防止「改好了这个坏了那个」。",
      example:"promptfoo eval  # 按配置跑用例，输出通过率与差异对比",
      note:"用例里别放真实业务数据；评测结果可能含模型输出片段，注意留存范围。" },
    { id:"t_guardrails", name:"NeMo Guardrails", cat:"aisec",
      desc:"在模型外面加一层规则护栏，拦住越界回答与危险动作",
      usage:"定义话题边界与动作约束 → 请求先过护栏再进模型 → 命中规则时改写、拒答或转人工。",
      example:"规则示例：禁止输出可执行攻击代码 / 禁止透露系统提示词 → 命中即替换为安全回复",
      note:"护栏是纵深防御的一层，不能代替输出侧过滤与权限最小化。" },
    { id:"t_misp", name:"MISP", cat:"soccti",
      desc:"威胁情报的共享与交换平台（IOC 库）",
      usage:"导入/自建事件（域名、IP、样本哈希）→ 打标签与关联 → 通过 feed 共享给合作方或订阅外部情报。",
      example:"按事件组织 IOC，并配置 feed 自动同步与去重",
      note:"共享情报前确认脱敏与共享范围（TLP 标记），避免间接泄露自身环境信息。" },
    { id:"t_opencti", name:"OpenCTI", cat:"soccti",
      desc:"把情报做成知识图谱：攻击者、工具、漏洞之间能关联查询",
      usage:"接入 MISP/情报源 → 建立实体关系（攻击者→使用→工具→利用→漏洞）→ 按实体追查相关活动与报告。",
      example:"查询示例：某个 CVE 关联了哪些攻击者组织与恶意样本",
      note:"图谱里常有未验证情报，用于决策前需交叉验证来源可靠性。" },
    { id:"t_thehive", name:"TheHive", cat:"soccti",
      desc:"安全事件响应工单：线索、任务、时间线集中管理",
      usage:"把告警升级为案例 → 建任务清单与观察项（Observable）→ 记录处置时间线，形成可复盘的闭环。",
      example:"用 Case Template 固化常见事件（钓鱼/勒索/失陷主机）的处置步骤",
      note:"案例内容含事件细节与内部资产信息，按机密级别控制访问与导出。" }
  ],
  ...
  quizzes: [
    ...
    { id:"q_lfi", topic:"lfi", cat:"web", level:"初级", q:"缓解本地文件包含（LFI）最有效的做法是？",
      options:["过滤掉 ../ 字符串","文件名白名单 + 禁止 PHP 包装器/远程包含","把文件放到 Web 根目录","记录访问日志"], answer:1,
      explain:"字符串过滤易被变形绕过；白名单 + 关闭 allow_url_include 才是根本。" },
    { id:"q_blockmode", topic:"blockmode", cat:"crypto", level:"初级", q:"以下哪种分组模式同时提供机密性与完整性（AEAD 认证加密）？",
      options:["ECB","CBC","GCM","CTR（无 MAC）"], answer:2,
      explain:"GCM 是 AEAD，内建完整性校验；ECB/CBC/CTR 仅提供机密性，需额外 MAC。" },
    { id:"q_osint", topic:"osint", cat:"pentest", level:"入门", q:"下列属于「被动」开源情报（OSINT）收集的是？",
  ...
  // ---- Web 安全 · 入门 ----
  { id:"xw1", topic:"sqli", cat:"web", level:"入门", q:"以下哪种做法最可能引入 SQL 注入？", options:["使用参数化查询","将用户输入直接拼接进 SQL 字符串","对输入做白名单校验","使用存储过程且参数绑定"], answer:1, explain:"拼接不可信输入到查询字符串是 SQL 注入的根因；参数化查询可从根本上阻断。" },
  { id:"xw2", topic:"sqli", cat:"web", level:"入门", q:"SQL 注入中，UNION 注入的主要目的是？", options:["删除数据库","把额外查询结果拼接到原结果集中回显","提升数据库权限","绕过登录"], answer:1, explain:"UNION SELECT 让攻击者把自定义查询的结果并入原页面响应，常用于拖库。" },
  { id:"xw3", topic:"sqli", cat:"web", level:"入门", q:"“盲注”（Blind SQLi）与常规注入的区别是？", options:["盲注无法利用","响应不直接回显数据，需靠布尔/时间差异推断","盲注只发生在 NoSQL","盲注不需要构造 payload"], answer:1, explain:"盲注时页面不返回数据本身，攻击者通过真假条件或睡眠延时来“问”数据库。" },
  { id:"xw4", topic:"xss", cat:"web", level:"入门", q:"反射型 XSS 的传播方式是？", options:["存储在服务器数据库","通过链接/参数把恶意脚本“反射”到响应中诱用户点击","写入 cookie","仅在本地文件"], answer:1, explain:"反射型依赖把恶意脚本放进请求（如 URL），服务器原样返回到受害者浏览器执行。" },
  { id:"xw5", topic:"xss", cat:"web", level:"入门", q:"存储型 XSS 与反射型 XSS 的主要区别？", options:["存储型把恶意脚本存到服务端，用户访问即触发","反射型更危险","存储型不执行","两者完全一样"], answer:0, explain:"存储型payload进入数据库/页面，任何访客都会被触发，影响面更大。" },
  { id:"xw6", topic:"xss", cat:"web", level:"入门", q:"DOM 型 XSS 的源头通常在？", options:["服务端模板拼接","客户端 JS 读取不可信来源(location.hash/document.URL)并写入 DOM","数据库存储","HTTP 头"], answer:1, explain:"DOM XSS 完全发生在客户端，服务端未输出恶意脚本，而是由前端 JS 不安全地操作 DOM 造成。" },
  { id:"xw7", topic:"web-http-flow", cat:"web", level:"入门", q:"为降低 XSS 影响，敏感 cookie 应设置？", options:["HttpOnly","仅 Secure 即可","SameSite=None","Path=/"], answer:0, explain:"HttpOnly 禁止 JS 读取该 cookie，可阻止 XSS 偷会话。" },
  { id:"xw8", topic:"xss", cat:"web", level:"入门", q:"CSP（内容安全策略）主要用于？", options:["加速页面","限制可加载/执行的资源来源，缓解 XSS 与数据注入","加密传输","防 CSRF"], answer:1, explain:"CSP 通过白名单约束脚本/资源来源，是重要的 XSS 纵深防御手段。" },
  { id:"xw9", topic:"cors", cat:"web", level:"入门", q:"CSRF 攻击成功依赖浏览器的哪项行为？", options:["自动执行 JS","自动携带目标站 cookie 发起跨站请求","同源策略","CORS 预检"], answer:1, explain:"浏览器会自动附带目标站 cookie，使受害者在不知情时以自己身份发请求。" },
  { id:"xw10", topic:"csrf", cat:"web", level:"入门", q:"防御 CSRF 最有效组合是？", options:["图形验证码 + 短信","Anti-CSRF Token + SameSite Cookie","禁用 JS","更换 IP"], answer:1, explain:"Token 保证请求出自本人页面，SameSite 限制跨站携带 cookie，二者叠加更稳。" },
  { id:"xw11", topic:"web-http-flow", cat:"web", level:"入门", q:"SameSite=Strict 对 Cookie 的影响是？", options:["任何请求都带","跨站请求不携带该 Cookie","仅 HTTPS 带","永不失效"], answer:1, explain:"Strict 最严格：只要请求发起方与目标站不同站，就不带该 cookie。" },
  { id:"xw12", topic:"xxe", cat:"web", level:"入门", q:"XXE 漏洞根因是？", options:["JSON 解析","XML 解析器允许解析外部实体(DTD/ENTITY)","HTML 编码","SQL 拼接"], answer:1, explain:"启用外部实体解析后，攻击者可读取文件或发起 SSRF。" },
  { id:"xw13", topic:"ssrf", cat:"web", level:"入门", q:"XXE 常被用来做 SSRF，典型是？", options:["读取本地文件 file:// 或访问内网","修改 DNS","提升权限","删除日志"], answer:0, explain:"外部实体可指向 file:// 或内网地址，从而读文件或访问内部服务。" },
  { id:"xw14", topic:"ssrf", cat:"web", level:"入门", q:"SSRF（服务端请求伪造）的防御核心是？", options:["禁用 HTTPS","对目标地址做白名单/禁止访问内网与云元数据","增大超时","启用 gzip"], answer:1, explain:"SSRF 是服务器代发请求，必须约束目标地址，禁止内网与云元数据地址。" },
  { id:"xw15", topic:"ssrf", cat:"web", level:"入门", q:"云环境 SSRF 常试图访问哪个地址读取实例元数据/临时凭证？", options:["8.8.8.8","169.254.169.254","127.0.0.1:80","example.com"], answer:1, explain:"169.254.169.254 是云实例的链路本地元数据服务，常含临时凭证，是 SSRF 的高价值目标。" },
  { id:"xw16", topic:"upload", cat:"web", level:"入门", q:"文件上传漏洞防御不包括？", options:["校验文件类型与内容","限制执行权限(存到非可执行目录)","信任客户端文件名","重命名随机化"], answer:2, explain:"客户端文件名完全可控，绝不能信任；应服务端校验类型/内容并随机命名。" },
  { id:"xw17", topic:"lfi", cat:"web", level:"入门", q:"路径遍历(../)利用目的是？", options:["提升网速","跳出 Web 根目录读取/执行任意文件","注入 SQL","绕过 CSP"], answer:1, explain:"../ 序列可回溯目录，读取或包含本不应暴露的文件。" },
  { id:"xw18", topic:"sqli", cat:"web", level:"入门", q:"开放重定向(open redirect)常用于？", options:["提权","钓鱼跳转窃取凭证/令牌","SQL 注入","XSS"], answer:1, explain:"开放重定向常被放进钓鱼链接，诱使用户以为在访问可信站点。" },
  { id:"xw19", topic:"clickjack", cat:"web", level:"入门", q:"点击劫持(clickjacking)防御用？", options:["X-Frame-Options / CSP frame-ancestors 禁止被 iframe 嵌套","CORS","HttpOnly","HSTS"], answer:0, explain:"阻止站点被嵌入恶意 iframe，可防用户误点隐藏按钮。" },
  { id:"xw20", topic:"csrf", cat:"web", level:"入门", q:"HSTS 的作用是？", options:["强制 HTTPS、防 SSL 剥离/降级","防 XSS","防 CSRF","加密 cookie"], answer:0, explain:"HSTS 让浏览器只走 HTTPS，缓解降级与 SSL stripping。" },
  // ---- Web 安全 · 初级 ----
  { id:"xw21", topic:"cmdinj", cat:"web", level:"初级", q:"命令注入(OS command injection)防御应优先？", options:["转义所有字符","避免调用 shell、用参数数组/白名单","加长超时","禁用网络"], answer:1, explain:"最佳是根本不拼 shell；必须调用时用参数数组且白名单校验。" },
  { id:"xw22", topic:"sqli", cat:"web", level:"初级", q:"NoSQL 注入(如 MongoDB)常利用？", options:["union","操作符如 $ne/$gt 改变查询逻辑(绕过认证)","order by","limit"], answer:1, explain:"把输入变成查询操作符对象，可构造恒真条件绕过登录等逻辑。" },
  { id:"xw23", topic:"auth", cat:"web", level:"初级", q:"二阶(Second-Order)注入指？", options:["两次扫描","恶意数据先被存储、后续在另一处被拼入命令/查询执行","二级缓存","两阶段认证"], answer:1, explain:"数据先“安全”入库，之后在另一处被取出拼入上下文才触发，易被忽略。" },
  { id:"xw24", topic:"web-http-flow", cat:"web", level:"初级", q:"报错注入利用？", options:["数据库报错信息回显数据","cookie","HTTP 头","CSS"], answer:0, explain:"利用数据库报错把内部数据带回页面，是信息回显型注入。" },
  { id:"xw25", topic:"api-sec", cat:"web", level:"初级", q:"功能级越权(BFLA)例子？", options:["改 URL 里的 id","修改 role/admin 参数调用本无权用的管理接口","改密码","改邮箱"], answer:1, explain:"BFLA 是横向/纵向的功能越权，如普通用户改参数调用管理员接口。" },
  { id:"xw26", topic:"auth", cat:"web", level:"初级", q:"会话固定(session fixation)指？", options:["会话随机生成","攻击者先种入已知 session id 诱用户使用，从而冒充","会话加密","会话过期"], answer:1, explain:"攻击者预先设定会话标识并诱使受害者使用，登录后攻击者即可凭该 id 冒充。" },
  { id:"xw27", topic:"web-http-flow", cat:"web", level:"初级", q:"Cookie 的 Secure 属性表示？", options:["仅 HTTPS 传输","仅同源","不可读","永不过期"], answer:0, explain:"Secure 要求 cookie 只能通过 HTTPS 发送，降低明文泄露风险。" },
  { id:"xw28", topic:"csrf", cat:"web", level:"初级", q:"Cookie 的 SameSite 属性用于？", options:["控制跨站请求是否携带","加密","压缩","存储大小"], answer:0, explain:"SameSite 决定跨站请求是否附带该 cookie，是防御 CSRF 的重要手段。" },
  { id:"xw29", topic:"jwt", cat:"web", level:"初级", q:"JWT 弱密钥/共享密钥可被？", options:["压缩","离线爆破(字典)伪造令牌","DNS 污染","CSRF"], answer:1, explain:"HS256 用对称密钥，若密钥弱可被字典爆破，从而伪造任意身份令牌。" },
  { id:"xw30", topic:"sqli", cat:"web", level:"初级", q:"JWT 的 kid 参数若未校验可被用于？", options:["路径遍历/SQL 注入来定位签名密钥","加速","压缩","加密"], answer:0, explain:"kid 指向密钥文件/数据库，未校验时可借路径遍历或注入篡改验签所用的密钥。" },
  { id:"xw31", topic:"cors", cat:"web", level:"初级", q:"CORS 设置 Access-Control-Allow-Origin: * 且带凭据时？", options:["允许任意源带凭据","浏览器不允许 * 与凭据同时使用，需显式源","提升性能","防 XSS"], answer:1, explain:"带凭据时不能用通配符，必须返回具体的请求源，否则浏览器拒绝。" },
  { id:"xw32", topic:"cors", cat:"web", level:"初级", q:"反射 Origin 到 CORS 头(任意源)会导致？", options:["性能提升","跨源读取本应受限的资源(信息泄露)","防 CSRF","加密"], answer:1, explain:"把请求者 Origin 原样回显等于对任意源开放，敏感接口会被跨站读取。" },
  { id:"xw33", topic:"cache-poison", cat:"web", level:"初级", q:"子资源完整性(SRI)用途？", options:["校验第三方资源哈希防被篡改","加速","压缩","加密 cookie"], answer:0, explain:"SRI 用哈希校验 CDN 脚本/样式是否被替换，防止供应链投毒。" },
  { id:"xw34", topic:"cors", cat:"web", level:"初级", q:"同源策略(SOP)主要限制？", options:["跨源读取资源","同站脚本","本地存储","cookie 大小"], answer:0, explain:"SOP 限制网页跨源读取响应，CORS 是在其之上的受控放宽。" },
  { id:"xw35", topic:"graphql", cat:"web", level:"初级", q:"GraphQL 常见风险包括？", options:["内省暴露 schema、深度/批查询导致 DoS","SQL 注入","XSS","CSRF"], answer:0, explain:"默认内省暴露全量 schema，且嵌套/批量查询可被用于资源耗尽型 DoS。" },
  { id:"xw36", topic:"lfi", cat:"web", level:"初级", q:"RFI(远程文件包含)与 LFI 区别？", options:["RFI 可加载远程攻击者控制的文件并执行","LFI 只读取","RFI 更安全","无区别"], answer:0, explain:"RFI 把远程恶意文件包含进来执行，危害更大；LFI 通常只能读本地文件。" },
  // ---- Web 安全 · 中级 ----
  { id:"xw37", topic:"smuggling", cat:"web", level:"中级", q:"HTTP 请求走私(CL/TE)利用？", options:["前后端对 Content-Length/Transfer-Encoding 解析不一致造成请求拼接","DNS","CSRF","XSS"], answer:0, explain:"前后端解析差异可把两个请求“粘”在一起，绕过安全控制或污染他人请求。" },
  { id:"xw38", topic:"cache-poison", cat:"web", level:"中级", q:"HTTP 响应拆分(CRLF 注入)可造成？", options:["缓存投毒/头注入/ XSS","提速","压缩","加密"], answer:0, explain:"注入 CRLF 可在响应中插入额外头或拆分响应，进而 XSS/缓存投毒。" },
  { id:"xw39", topic:"cache-poison", cat:"web", level:"中级", q:"主机头注入(Host header)常用于？", options:["密码重置链接中毒/缓存投毒","XSS","CSRF","SQLi"], answer:0, explain:"服务端若用 Host 头拼重置链接，攻击者可令受害者收到指向恶意域的链接。" },
  ...
  { id:"xw41", topic:"cache-poison", cat:"web", level:"中级", q:"缓存投毒(cache poisoning)依赖？", options:["未规范化的输入被用作缓存键 + 未校验的响应","HTTPS","CSRF","XSS"], answer:0, explain:"若不可信输入进入缓存键且响应未校验，攻击者可让缓存向其他用户返回恶意内容。" },
  ...
  { id:"xw43", topic:"xss", cat:"web", level:"中级", q:"防 XSS 的“输出编码”应按？", options:["统一转义","按输出上下文(HTML/JS/URL/属性)分别编码","只转义 <","只转义引号"], answer:1, explain:"不同上下文需不同编码规则，错误上下文编码会漏防或被绕过。" },
  { id:"xw44", topic:"csrf", cat:"web", level:"中级", q:"OAuth 授权码流程用 PKCE 主要防止？", options:["授权码拦截后被人拿去兑换令牌","CSRF","XSS","暴力破解密码"], answer:0, explain:"PKCE 用一次性挑战值绑定授权码与请求方，防止授权码被截获冒用。" },
  { id:"xw45", topic:"web-authn", cat:"web", level:"中级", q:"刷新令牌(refresh token)泄露后果？", options:["仅影响本次会话","攻击者可长期获取新访问令牌","无影响","仅泄露用户名"], answer:1, explain:"刷新令牌有效期长，泄露意味着持续可用，应妥善存储并支持吊销。" },
  // ---- 二进制 · 入门 ----
  { id:"xb1", topic:"stack", cat:"binary", level:"入门", q:"栈溢出的根本原因是？", options:["缺少边界检查写入超出缓冲区","编译器错误","CPU 故障","网络延迟"], answer:0, explain:"向定长缓冲区写入超过其容量的数据会覆盖相邻栈内容（含返回地址）。" },
  { id:"xb2", topic:"fmt", cat:"binary", level:"入门", q:"格式化字符串漏洞中 %x 用于？", options:["写入","读取栈上数据(泄露)","执行","删除"], answer:1, explain:"%x 按格式从栈上取并打印数据，可泄露栈内存；%n 才可写入。" },
  { id:"xb3", topic:"rop", cat:"binary", level:"入门", q:"NX/DEP 的作用是？", options:["数据页不可执行，阻止在堆/栈执行 shellcode","随机化地址","加密","限速"], answer:0, explain:"NX 让数据区域不可执行，使传统“写 shellcode 再跳过去”失效，催生 ROP。" },
  { id:"xb4", topic:"mitigations", cat:"binary", level:"入门", q:"ASLR 的作用是？", options:["随机化内存地址布局，增加预测难度","禁用执行","加密","压缩"], answer:0, explain:"每次加载基址随机，攻击者在无信息泄露时难以预测函数/库地址。" },
  { id:"xb5", topic:"stack", cat:"binary", level:"入门", q:"栈 Canary 的作用是？", options:["检测栈溢出是否覆盖了返回地址(哨兵值)","加速","加密","压缩"], answer:0, explain:"在返回地址前放随机哨兵，函数返回前检查被改则中止，防返回地址被覆盖。" },
  { id:"xb6", topic:"mitigations", cat:"binary", level:"入门", q:"PIE 指？", options:["代码基址随机化(位置无关可执行)","数据加密","禁用执行","堆随机"], answer:0, explain:"PIE 让代码段基址也随机化，与 ASLR 配合提升利用难度。" },
  { id:"xb7", topic:"mitigations", cat:"binary", level:"入门", q:"RELRO(FULL)的作用是？", options:["使 GOT 重定位后只读，防 GOT 覆盖","加密","随机化","压缩"], answer:0, explain:"FULL RELRO 在启动时完成重定位并把 GOT 设为只读，阻止改写函数指针。" },
  { id:"xb8", topic:"race", cat:"binary", level:"入门", q:"条件竞争(TOCTOU)常见在？", options:["检查与使用之间的时间窗口","网络","加密","编译"], answer:0, explain:"先检查权限/状态，再使用时已被另一线程改变，造成越权或绕过。" },
  // ---- 二进制 · 初级 ----
  { id:"xb9", topic:"stack", cat:"binary", level:"初级", q:"堆溢出常覆写？", options:["堆块元数据(size/fd/bk)实现任意写","栈返回地址","寄存器","内核"], answer:0, explain:"堆块头含 size 与前后向指针，覆写它们可制造任意地址写，是堆利用核心。" },
  { id:"xb10", topic:"uaf", cat:"binary", level:"初级", q:"Use-After-Free(UAF)指？", options:["释放后未置空又被使用，可能复用已分配块","未分配","重复分配","内存泄漏"], answer:0, explain:"释放后指针未清空，再次使用可能操作被重新分配的同块内存，造成类型混淆/劫持。" },
  { id:"xb11", topic:"stack", cat:"binary", level:"初级", q:"double free 指？", options:["多次 free 同一指针，破坏堆结构","一次 free","未 free","栈溢出"], answer:0, explain:"重复释放同一块会让空闲链表出现重复节点，进而可被用于任意写。" },
  { id:"xb12", topic:"rop", cat:"binary", level:"初级", q:"ROP(返回导向编程)用于绕过？", options:["NX:复用已存在代码片段(gadget)拼出逻辑","ASLR","Canary","PIE"], answer:0, explain:"NX 下不能执行注入代码，ROP 改借程序中已有的 ret 结尾小片段串起攻击逻辑。" },
  ...
  { id:"xb14", topic:"mitigations", cat:"binary", level:"初级", q:"GOT 覆盖利用前提是？", options:["GOT 可写(非 FULL RELRO)时改写函数指针劫持控制流","ASLR","Canary","PIE"], answer:0, explain:"部分 RELRO 下 GOT 仍可写，覆写某函数项即可在调用它时跳到攻击者地址。" },
  ...
  { id:"xb17", topic:"rop", cat:"binary", level:"初级", q:"shellcode 要成功执行通常需要？", options:["可写且可执行的页(NX 下更难)","仅可写","仅可读","仅网络"], answer:0, explain:"NX 下数据页不可执行，需配合内存权限修改或 ROP 才能运行注入代码。" },
  ...
  // ---- 二进制 · 中级 ----
  { id:"xb19", topic:"mitigations", cat:"binary", level:"中级", q:"checksec 工具可查看？", options:["Canary/NX/PIE/RELRO 等保护","网络","日志","密码"], answer:0, explain:"checksec 汇总二进制开启的缓解措施，是漏洞利用前的第一步侦察。" },
  { id:"xb20", topic:"rop", cat:"binary", level:"中级", q:"ROP 中 gadget 指？", options:["以 ret 结尾的短指令序列，用于拼装逻辑","漏洞","shellcode","堆块"], answer:0, explain:"gadget 是程序中已有的小指令段，多个串联即可在不写可执行代码的前提下完成攻击。" },
  { id:"xb21", topic:"sandbox", cat:"binary", level:"中级", q:"seccomp 是？", options:["限制进程可用 syscall 的系统过滤器(沙箱)","加密","随机化","压缩"], answer:0, explain:"seccomp 收窄进程系统调用面，即使被攻破也难做敏感操作，是沙箱基础。" },
  { id:"xb22", topic:"uaf", cat:"binary", level:"中级", q:"glibc tcache 因是单链表，使得？", options:["UAF/double free 更易被利用","更慢","更安全","加密"], answer:0, explain:"tcache 缺少充分的一致性校验，UAF/double free 在它上面更易转化为任意写。" },
  { id:"xb23", topic:"stack", cat:"binary", level:"中级", q:"栈溢出利用需要先确定？", options:["返回地址相对缓冲区的偏移(offset)","网络端口","密码","文件名"], answer:0, explain:"只有知道从缓冲区到返回地址的填充长度，才能精确覆盖返回地址跳到目标。" },
  { id:"xb24", topic:"rop", cat:"binary", level:"中级", q:"信息泄露(如泄露 libc 地址)对 ROP 的意义？", options:["可计算基址绕过 ASLR 调用 gadget","无意义","加速网络","加密"], answer:0, explain:"ASLR 随机化基址，泄露一个 libc 地址就能算出 system 等函数真实地址。" },
  { id:"xb25", topic:"uaf", cat:"binary", level:"中级", q:"类型混淆(type confusion)常导致？", options:["把对象当另一类型解释，造成越界/UAF 类利用","编译错误","网络","加密"], answer:0, explain:"类型信息被破坏后，对同一块内存按错误结构解读，可越界读写或触发 UAF。" },
  { id:"xb26", topic:"heap", cat:"binary", level:"中级", q:"fastbin dup 是？", options:["堆利用技术：利用 double free 在 fastbin 制造重复块","网络","加密","编译"], answer:0, explain:"借助 fastbin 的重复释放制造两个指向同块的指针，进而控制空闲链表。" },
  { id:"xb27", topic:"heap", cat:"binary", level:"中级", q:"unlink 是？", options:["经典堆利用：通过伪造 chunk 元数据在空闲时写任意地址","XSS","CSRF","SQLi"], answer:0, explain:"unlink 合并相邻空闲块时会写相邻指针，伪造元数据即可任意地址写。" },
  { id:"xb28", topic:"fmt", cat:"binary", level:"中级", q:"栈 canary 若被泄露(如格式化字符串)会？", options:["失效，溢出可不被发现","更安全","加密","加速"], answer:0, explain:"canary 一旦被读出，攻击者可在溢出时原样填回，使检测失效。" },
  // ---- 密码学 · 入门 ----
  { id:"xc1", topic:"sym", cat:"crypto", level:"入门", q:"DES 的主要缺陷？", options:["56 位密钥过短，已被 brute-force","太快","分组太大","需要 IV"], answer:0, explain:"DES 仅 56 位密钥，现代算力可穷举，已不安全的。" },
  { id:"xc2", topic:"sym", cat:"crypto", level:"入门", q:"3DES 现状？", options:["有效 112 位、慢、新系统不推荐","最安全","无需密钥","无 IV"], answer:0, explain:"3DES 有效强度约 112 位且性能差，NIST 已逐步弃用，新系统用 AES。" },
  { id:"xc3", topic:"sym", cat:"crypto", level:"入门", q:"AES 支持的分组长度是？", options:["128 位","64 位","256 位(仅)","512 位"], answer:0, explain:"AES 固定 128 位分组；密钥才分 128/192/256。" },
  { id:"xc4", topic:"sym", cat:"crypto", level:"入门", q:"AES 密钥长度可选？", options:["128/192/256 位","仅 128","仅 56","仅 64"], answer:0, explain:"AES 支持三种密钥长度，均为安全强度足够的选项。" },
  ...
  { id:"xc6", topic:"sym", cat:"crypto", level:"入门", q:"SHA-1 现况？", options:["已可构造碰撞，弃用","最安全","需 IV","对称"], answer:0, explain:"SHA-1 碰撞已被证实，证书与签名场景应升级到 SHA-256 及以上。" },
  { id:"xc7", topic:"crypto-sign", cat:"crypto", level:"入门", q:"数字签名用？", options:["私钥签名、公钥验证","对称密钥","哈希即可","无密钥"], answer:0, explain:"签名由私钥产生、公钥验证，提供来源认证与不可否认。" },
  { id:"xc8", topic:"asym", cat:"crypto", level:"入门", q:"MAC 与数字签名区别在于？", options:["MAC 基于对称密钥、双方共享；签名用非对称且可公开验证","无区别","MAC 用公钥","签名用对称"], answer:0, explain:"MAC 的验证方也持有同一密钥，无法向第三方证明；签名可公开验证。" },
  { id:"xc9", topic:"crypto-hash-store", cat:"crypto", level:"入门", q:"口令存储应加 salt 是为了？", options:["防彩虹表、使相同口令密文不同","加速","缩短","加密传输"], answer:0, explain:"随机 salt 让相同口令产生不同哈希，并使预计算彩虹表失效。" },
  { id:"xc10", topic:"crypto-hash-store", cat:"crypto", level:"入门", q:"存储口令推荐？", options:["bcrypt/Argon2/scrypt 等慢哈希+盐","MD5 单次","明文","Base64"], answer:0, explain:"慢哈希拖慢暴力/字典攻击，加盐防彩虹表，是当前最佳实践。" },
  { id:"xc11", topic:"rand", cat:"crypto", level:"入门", q:"生成令牌/密钥应使用？", options:["CSPRNG(如 /dev/urandom)","rand()","时间种子","PID"], answer:0, explain:"密码学安全随机数不可预测，rand()/时间/pid 都可被猜解。" },
  // ---- 密码学 · 初级 ----
  { id:"xc12", topic:"blockmode", cat:"crypto", level:"初级", q:"ECB 模式问题？", options:["相同明文块→相同密文块，泄露结构","需 IV","最快","最安全"], answer:0, explain:"ECB 无随机化，相同明文块产生相同密文块，会暴露图像/数据模式。" },
  { id:"xc13", topic:"blockmode", cat:"crypto", level:"初级", q:"CBC 模式 IV 应？", options:["随机且不可预测","固定为 0","公开明文","省略"], answer:0, explain:"IV 必须随机不可预测，否则相同明文首块会暴露关联。" },
  { id:"xc14", topic:"blockmode", cat:"crypto", level:"初级", q:"CBC 填充预言(padding oracle)可？", options:["通过填充校验差异解密密文","提速","压缩","加密"], answer:0, explain:"若服务对填充错误返回不同响应，攻击者可逐字节恢复明文，无需密钥。" },
  { id:"xc15", topic:"blockmode", cat:"crypto", level:"初级", q:"CTR/GCM 的 nonce 绝对不能重用，否则？", options:["可恢复明文甚至伪造(灾难性)","无影响","更安全","加速"], answer:0, explain:"流密码式 nonce 重用会让同一密钥流异或，直接泄露明文并可伪造。" },
  { id:"xc16", topic:"blockmode", cat:"crypto", level:"初级", q:"RC4 现状？", options:["存在统计偏置，已不安全","最安全","需 IV","对称最好"], answer:0, explain:"RC4 密钥流有可 exploited 的偏置，已被淘汰，不应再使用。" },
  { id:"xc17", topic:"asym", cat:"crypto", level:"初级", q:"RSA 小公钥指数 e=3 且仅用裸加密小消息会被？", options:["直接开立方根恢复明文(需适当填充)","提速","压缩","加密"], answer:0, explain:"小 e 且无填充时，密文可能本身就是明文的小幂次，开根即得明文。" },
  { id:"xc18", topic:"blockmode", cat:"crypto", level:"初级", q:"RSA 应使用哪种填充？", options:["OAEP(PKCS#1 v2)","无填充(Textbook)","PKCS#1 v1.5(易 BLEICHEN)","自定义 XOR"], answer:0, explain:"OAEP 提供语义安全；v1.5 有 BLEICHEN 等攻击，教科书式无填充更危险。" },
  { id:"xc19", topic:"asym", cat:"crypto", level:"初级", q:"两 RSA 模数共享一个素因子时，可？", options:["求 GCD 分解两模数","提速","压缩","加密"], answer:0, explain:"共享一个素数则两模数 GCD 即该素数，进而分解出私钥。" },
  { id:"xc20", topic:"crypto-sign", cat:"crypto", level:"初级", q:"前向保密(forward secrecy)指？", options:["会话密钥不依赖长期私钥，泄露长期密钥也不能解密旧会话","更快","无密钥","对称"], answer:0, explain:"每次会话用临时密钥，长期私钥泄露也无法解密既往通信内容。" },
  { id:"xc21", topic:"hash", cat:"crypto", level:"初级", q:"HMAC 是？", options:["基于哈希的消息认证码","对称加密","数字签名","随机数"], answer:0, explain:"HMAC 用密钥与哈希构造 MAC，提供完整性+认证，且抗长度扩展。" },
  { id:"xc22", topic:"hash", cat:"crypto", level:"初级", q:"长度扩展攻击影响？", options:["Merkle-Damgård 哈希(MD5/SHA1/SHA2)，HMAC 免疫","AES","RSA","ECC"], answer:0, explain:"M-D 结构允许在已知哈希后追加数据并算出新哈希；HMAC 通过嵌套结构免疫。" },
  // ---- 密码学 · 中级 ----
  { id:"xc23", topic:"tls", cat:"crypto", level:"中级", q:"TLS 的 Heartbleed 是？", options:["OpenSSL 心跳扩展越界读，泄露内存","加密弱","降级","MITM"], answer:0, explain:"心跳请求未校验长度，可读取进程内存，泄露私钥与会话数据。" },
  { id:"xc24", topic:"ecc", cat:"crypto", level:"中级", q:"POODLE 攻击针对？", options:["强制降级到 SSLv3 利用 CBC 弱点","AES","RSA","ECC"], answer:0, explain:"POODLE 诱使客户端用已不安全的 SSLv3，再利用 CBC 处理缺陷解密字节。" },
  { id:"xc25", topic:"asym", cat:"crypto", level:"中级", q:"ECDSA 若随机数 k 重用会？", options:["可恢复私钥","提速","压缩","加密"], answer:0, explain:"同一 k 签两份不同消息会联立方程解出私钥，是真实重大事故根源。" },
  { id:"xc26", topic:"sym", cat:"crypto", level:"中级", q:"DHE/ECDHE 提供？", options:["临时密钥交换，支持前向保密","对称加密","静态密钥","无密钥"], answer:0, explain:"临时(Ephemeral)密钥每次协商新密钥，实现前向保密。" },
  { id:"xc27", topic:"tls", cat:"crypto", level:"中级", q:"证书固定(cert pinning)用途？", options:["绑定预期公钥/证书，防伪造 CA 中间人","加速","压缩","加密 cookie"], answer:0, explain:"pinning 只信任预定证书/公钥，即使被植入恶意 CA 也无法中间人。" },
  { id:"xc28", topic:"asym", cat:"crypto", level:"中级", q:"RSA 私钥 d 过小时可用？", options:["Wiener 攻击恢复私钥","提速","压缩","加密"], answer:0, explain:"d 过小（与 N 相比）时连分数攻击可恢复私钥，应使用足够大的 d。" },
  { id:"xc29", topic:"tls", cat:"crypto", level:"中级", q:"TLS 中间人成功常因？", options:["证书校验缺失/被禁或伪造 CA 被信任","HTTPS","HSTS","pinning"], answer:0, explain:"只要客户端不严格校验证书或信任了攻击者 CA，就能透明中间人解密。" },
  { id:"xc30", topic:"sym", cat:"crypto", level:"中级", q:"对称加密 vs 非对称加密，非对称主要缺点？", options:["运算慢、适合小数据/密钥交换","不需要密钥","最安全","最快"], answer:0, explain:"非对称运算开销大，通常用于协商对称密钥或签名，而非大量数据加密。" },
  // ---- 渗透测试 · 入门 ----
  { id:"xp1", topic:"recon", cat:"pentest", level:"入门", q:"被动侦察指？", options:["不接触目标(WHOIS/证书透明/搜索引擎缓存)","端口扫描","漏洞利用","提权"], answer:0, explain:"被动侦察尽量不触碰目标，降低被发现与封禁风险。" },
  { id:"xp2", topic:"recon", cat:"pentest", level:"入门", q:"主动侦察指？", options:["直接扫描/探测目标(可能留下痕迹)","只查公开资料","社工","报告"], answer:0, explain:"主动侦察会向目标发流量（扫描/探测），效率高但有痕迹。" },
  { id:"xp3", topic:"recon", cat:"pentest", level:"入门", q:"nmap -sS 是？", options:["SYN 半开扫描","全连接","UDP","版本"], answer:0, explain:"-sS 只完成三次握手前两步便重置，隐蔽且快，是最常用扫描方式。" },
  { id:"xp4", topic:"recon", cat:"pentest", level:"入门", q:"nmap -sV 用于？", options:["探测服务/版本","操作系统","只 ping","清理"], answer:0, explain:"-sV 主动探测 banner 以识别服务类型与版本，辅助匹配漏洞。" },
  { id:"xp5", topic:"recon", cat:"pentest", level:"入门", q:"目录爆破(gobuster/ffuf)用于？", options:["发现隐藏路径/文件","加密","提权","报告"], answer:0, explain:"用字典枚举路径，常能找到后台、备份、接口等未链接资产。" },
  { id:"xp6", topic:"recon", cat:"pentest", level:"入门", q:"子域枚举常用？", options:["字典/证书透明(DNS)/搜索引擎","仅 ping","仅 nmap","仅社工"], answer:0, explain:"证书透明日志、DNS 字典与搜索引擎是子域发现的常用手段。" },
  { id:"xp7", topic:"pt-process", cat:"pentest", level:"入门", q:"漏洞扫描器(Nessus/OpenVAS)应在？", options:["明确授权范围内使用","任意目标","生产无告知","公网随意"], answer:0, explain:"未经授权的扫描可能违法或影响业务，必须在书面授权范围内进行。" },
  { id:"xp8", topic:"recon", cat:"pentest", level:"入门", q:"渗透测试方法论顺序通常？", options:["侦察→扫描→利用→提权→持久化→报告","报告→利用→侦察","只利用","只扫描"], answer:0, explain:"标准流程从信息收集到利用再到后渗透与交付报告，循序渐进。" },
  ...
  // ---- 渗透测试 · 初级 ----
  { id:"xp11", topic:"privesc", cat:"pentest", level:"初级", q:"Linux 提权常见途径？", options:["sudo 配置错误/SUID/ cron/内核漏洞","换浏览器","重装","关防火墙"], answer:0, explain:"滥用错误 sudo 规则、SUID 二进制、定时任务与内核漏洞是常见提权点。" },
  { id:"xp12", topic:"privesc", cat:"pentest", level:"初级", q:"Windows 提权常见途径？", options:["未引号路径服务/令牌 impersonation/ GPP 密码","换桌面","重装","关杀软"], answer:0, explain:"不当服务路径、令牌模拟与组策略首选项里存的密码都是经典提权面。" },
  ...
  { id:"xp15", topic:"lateral", cat:"pentest", level:"初级", q:"横向移动常用？", options:["SMB/PsExec/ Pass-the-Hash/ WMI","重装","关机","报告"], answer:0, explain:"在内网用共享/远程管理协议与哈希复用，从一台主机移动到另一台。" },
  ...
  { id:"xp18", topic:"social", cat:"pentest", level:"初级", q:"钓鱼(phishing)属于？", options:["社会工程，诱导目标执行/泄露","扫描","加密","内核"], answer:0, explain:"钓鱼利用心理与信任诱使目标点击/提供凭证，是社会工程典型手段。" },
  { id:"xp19", topic:"pt-process", cat:"pentest", level:"初级", q:"C2(命令与控制)框架如？", options:["Cobalt Strike/Metasploit(授权内)","浏览器","编辑器","防火墙"], answer:0, explain:"C2 框架用于在被控主机与攻击端间建立受控通道，须在授权范围内使用。" },
  { id:"xp20", topic:"report", cat:"pentest", level:"初级", q:"红队与蓝队区别？", options:["红队攻、蓝队防，对抗式演练","都是防守","都是扫描","都是报告"], answer:0, explain:"红队模拟攻击者，蓝队防守检测响应，联合演练提升真实防护能力。" },
  // ---- 渗透测试 · 中级 ----
  ...
  { id:"xp22", topic:"lateral", cat:"pentest", level:"中级", q:"SIEM 用途？", options:["集中日志与安全事件关联分析","加密","扫描","提权"], answer:0, explain:"SIEM 汇聚多源日志并做关联，帮助发现横向移动等攻击链。" },
  { id:"xp23", topic:"ad", cat:"pentest", level:"中级", q:"域渗透中 BloodHound 用于？", options:["可视化 AD 权限关系，找最短提权路径","扫描端口","加密","报告"], answer:0, explain:"BloodHound 以图展示 AD 对象关系，快速定位到域管的攻击路径。" },
  { id:"xp24", topic:"report", cat:"pentest", level:"中级", q:"渗透报告应包含？", options:["风险等级/复现步骤/影响/修复建议","只给分数","只给密码","只给 IP"], answer:0, explain:"好报告要让客户能复现并修复，需含严重度、证据、影响与整改建议。" },
  { id:"xp25", topic:"pt-process", cat:"pentest", level:"中级", q:"范围界定(scoping)重要性？", options:["明确授权边界，避免越权/违法","加快","加密","扫描"], answer:0, explain:"清晰的范围与授权是合法测试前提，越界可能构成违法或影响业务。" },
  { id:"xp26", topic:"social", cat:"pentest", level:"中级", q:"社会工程手法包括？", options:["pretexting/ baiting/ 钓鱼 等","端口扫描","加密","内核"], answer:0, explain:"社会工程靠人为弱点而非技术漏洞，包含伪装、诱饵、钓鱼等。" },
  ...
  { id:"xp28", topic:"scan", cat:"pentest", level:"中级", q:"漏洞验证(PoC)与利用区别？", options:["PoC 证明存在且不造成破坏，利用可能深入","一样","PoC 更危险","利用更安全"], answer:0, explain:"PoC 仅需证明可行性并尽量不破坏系统，正式利用才会进一步深入。" },
  { id:"xp29", cat:"pentest", level:"中级", q:"WAF 属于？", options:["防御层，过滤恶意 Web 流量","攻击工具","扫描器","加密"], answer:0, explain:"WAF 在应用前过滤恶意请求，是 Web 防护的重要一层（非万能）。" },
    { id:"xp30", topic:"lateral", cat:"pentest", level:"中级", q:"日志分析可发现？", options:["异常登录/横向移动痕迹","加密","扫描端口","内核版本"], answer:0, explain:"登录时间异常、非常见源 IP 的横向连接等是入侵在日志中的典型信号。" },

  // ===================== 高级(CTF)档位：真实 CVE 分析 / 综合利用链 / CTF 专项 =====================
  // ---- 高级 · 真实 CVE 漏洞分析 ----
  { id:"av1", topic:"tls", cat:"crypto", level:"高级", q:"CVE-2014-0160（Heartbleed）的漏洞本质是？", options:["OpenSSL TLS 心跳扩展未校验长度导致越界读内存","SSH 弱口令","SQL 注入","XSS"], answer:0, explain:"心跳请求声称的长度大于实际负载，服务端按声称长度回显，越界读取进程内存（含私钥）。", hint:"关注“声称的长度”与实际负载长度不一致。" },
  { id:"av2", topic:"cmdinj", cat:"web", level:"高级", q:"Shellshock（CVE-2014-6271）的触发面是？", options:["Bash 环境变量中以 `() {` 开头却仍执行尾随命令","内核提权","浏览器漏洞","DNS 投毒"], answer:0, explain:"CGI 等会把 HTTP 头写入环境变量再交给 bash，尾随命令被当作函数体执行，造成 RCE。", hint:"CGI 脚本会把 HTTP 头塞进环境变量传给 bash。" },
  { id:"av3", topic:"bin-kernel", cat:"binary", level:"高级", q:"Dirty COW（CVE-2016-5195）属于？", options:["Linux 内核写时复制(COW)竞态导致的本地提权","远程 RCE","XSS","密码学缺陷"], answer:0, explain:"竞态让只读映射被写成可写，攻击者可改 /etc/passwd 等拿到 root。", hint:"目标是写原本只读的系统文件。" },
  { id:"av4", topic:"rop", cat:"binary", level:"高级", q:"EternalBlue（MS17-010）利用的组件是？", options:["Windows SMBv1 的漏洞","OpenSSL","Apache","Nginx"], answer:0, explain:"SMBv1 处理畸形报文时的漏洞，可被用于远程代码执行，WannaCry 即借其传播。", hint:"与 WannaCry 勒索蠕虫同源。" },
  ...
  { id:"av6", topic:"xss", cat:"web", level:"高级", q:"Log4Shell（CVE-2021-44228）的触发点是？", options:["Log4j 2 的 JNDI 查找 `${jndi:ldap://...}` 解析不可信日志","SSH 配置错误","DNS 污染","XSS"], answer:0, explain:"任何被记录的不可信字符串里的 JNDI 占位符都会被解析，从远程加载类实现 RCE。", hint:"日志内容里出现该占位符就会被解析。" },
  { id:"av7", topic:"sqli", cat:"web", level:"高级", q:"Spring4Shell（CVE-2022-22965）的利用关键在？", options:["通过数据绑定访问 getClass().classLoader 修改 Tomcat 写入 webshell","SQL 注入","XSS","CSRF"], answer:0, explain:"Spring Bean 的数据绑定越过了对象边界，可借 classLoader 写文件实现 RCE。", hint:"数据绑定越过了对象属性边界。" },
  { id:"av8", topic:"sqli", cat:"web", level:"高级", q:"Drupalgeddon2（CVE-2018-7600）属于？", options:["Drupal 请求参数处理导致的远程代码执行","XSS","CSRF","SQL 注入"], answer:0, explain:"未过滤的数组/属性被当作命令执行，攻击者可直接 RCE。", hint:"与“未过滤的数组/属性被当作命令执行”有关。" },
  { id:"av9", topic:"xss", cat:"web", level:"高级", q:"Struts2 S2-045（CVE-2017-5638）的根因是？", options:["Jakarta Multipart 解析器的 OGNL 表达式注入","CSRF","XSS","SSRF"], answer:0, explain:"上传/错误页处理时执行了攻击者可控的 OGNL 表达式，导致 RCE。", hint:"错误页处理执行了可控的 OGNL。" },
  ...
  { id:"av11", topic:"rop", cat:"binary", level:"高级", q:"Meltdown（CVE-2017-5754）与 Spectre 主要区别？", options:["直接跨越内核/用户态边界读内核内存","只影响浏览器","仅网络层","XSS"], answer:0, explain:"Meltdown 利用权限检查被推迟，用户态可直接读内核内存，需 KPTI 缓解。", hint:"需 KPTI/隔离页表缓解。" },
  ...
  { id:"av13", topic:"priv-esc", cat:"pentest", level:"高级", q:"PrintNightmare（CVE-2021-34527）利用的是？", options:["Windows 打印后台处理程序 RPRN RPC 未授权提权/RCE","浏览器漏洞","DNS 投毒","SQL 注入"], answer:0, explain:"通过 RPC 添加打印机驱动加载 DLL，实现提权与远程代码执行。", hint:"通过 RPC 添加打印机驱动加载 DLL。" },
  { id:"av14", topic:"xss", cat:"web", level:"高级", q:"ProxyLogon（CVE-2021-26855）链的起点通常是？", options:["Exchange 预认证 SSRF 抵达内部服务再序列化 RCE","钓鱼邮件","XSS","CSRF"], answer:0, explain:"先用 SSRF 绕过认证访问内部接口，再借序列化等拿到 RCE。", hint:"先“绕过认证”访问内部接口。" },
  { id:"av15", topic:"tls", cat:"crypto", level:"高级", q:"POODLE（CVE-2014-3566）迫使客户端降级到？", options:["SSLv3 再利用其 CBC 处理弱点","TLS1.3","AES-GCM","RSA"], answer:0, explain:"降级到 SSLv3 后利用 CBC 实现缺陷逐字节恢复明文。", hint:"旧协议 CBC 实现存在可攻击点。" },
  { id:"av16", topic:"tls", cat:"crypto", level:"高级", q:"FREAK（CVE-2015-0204）利用的是？", options:["可降级到出口级(export-grade)弱 RSA 密钥协商","Heartbeat","XSS","DNS 污染"], answer:0, explain:"中间人让服务端使用短密钥，随后可离线分解恢复会话密钥。", hint:"中间人让服务端用弱密钥。" },
  { id:"av17", topic:"sym", cat:"crypto", level:"高级", q:"ROBOT（CVE-2017-6168 等）是？", options:["RSA PKCS#1 v1.5 填充的计时/报错预言机攻击","AES 侧信道","XSS","SQL 注入"], answer:0, explain:"利用解密“预言机”（响应差异）辅助恢复明文或签名。", hint:"解密“预言机”可辅助解密。" },
  { id:"av18", topic:"deser", cat:"web", level:"高级", q:"Log4Shell 的最佳修复思路是？", options:["升级 Log4j2 并禁用 JNDI 远程查找/格式消息","仅关日志","更换数据库密码","仅靠 WAF 即可"], answer:0, explain:"根因在 JNDI 查找，应升级并关闭远程查找；WAF 只能拦已知 payload，不解决根因。", hint:"WAF 只拦已知 payload，根因在 JNDI 查找。" },
  // ---- 高级 · 综合利用链(exploit chain) ----
  { id:"av19", topic:"sandbox", cat:"binary", level:"高级", q:"一个典型的浏览器利用链顺序是？", options:["渲染进程漏洞(RCE)→沙箱逃逸→提权到系统","提权→沙箱逃逸→RCE","反序三步","仅 XSS"], answer:0, explain:"先在被攻击进程拿代码执行，再逃逸沙箱、提权，形成完整链。", hint:"先在渲染进程拿代码执行，再逃沙箱。" },
  { id:"av20", topic:"ad", cat:"pentest", level:"高级", q:"Web 应用 RCE → 域控 的常见链是？", options:["Web RCE 读凭据→Pass-the-Hash/票据→Kerberoasting→域控","直接 XSS","仅 SQL 注入","仅钓鱼"], answer:0, explain:"从主机落到身份，再用 Windows 认证链横向到域控。", hint:"从“机”到“身份”再到“域”。" },
  { id:"av21", topic:"lateral", cat:"pentest", level:"高级", q:"钓鱼邮件→内网横向 的下一步通常是？", options:["在主机落 C2 信标→侦察→凭据窃取→横向移动","直接 DDoS","仅发勒索信","关闭防火墙"], answer:0, explain:"先建立据点(foothold)，再在内网扩展与移动。", hint:"先 establish foothold 再 move。" },
  ...
  { id:"av24", topic:"av-bypass", cat:"pentest", level:"高级", q:"“左移”防御利用链的思路是？", options:["在开发/构建期消除漏洞根源，减少可被利用的起点","只靠 WAF","只靠 EDR","只靠审计日志"], answer:0, explain:"减少可被利用的环节数，比事后拦截更根本。", hint:"减少可被利用的环节数。" },
  { id:"av25", topic:"priv-esc", cat:"pentest", level:"高级", q:"持久化(persistence)在利用链中的位置？", options:["提权/立足之后，保证能重新进入","第一步","仅侦察阶段","报告阶段"], answer:0, explain:"拿到权限后建立后门，保证失陷主机可被再次访问。", hint:"在拿到权限后建立“后门”。" },
  { id:"av26", topic:"ssrf", cat:"web", level:"高级", q:"针对“SSRF→云元数据→临时凭证”链，防御应？", options:["禁止实例访问元数据/启用 IMDSv2/网络隔离","只用 HTTPS","加 WAF","更换密码"], answer:0, explain:"断掉“拿凭证”这一环（强制 PUT 令牌、禁止链路本地访问）。", hint:"断掉“拿凭证”这一环。" },
  { id:"av27", topic:"pt-web-exp", cat:"pentest", level:"高级", q:"利用链分析里“攻击面(attack surface)”指？", options:["所有可达且可被滥用入口的集合，链往往从这些入口开始","仅开放端口","仅网页","仅 API"], answer:0, explain:"入口越多，链越容易起手；收敛攻击面是降风险的关键。", hint:"入口越多链越容易起。" },
  { id:"av28", topic:"ad", cat:"pentest", level:"高级", q:"红队评估利用链时关注“爆破半径(blast radius)”是为了？", options:["评估单点失陷后的影响范围，优先修复高杠杆环节","提速","压缩","加密"], answer:0, explain:"判断哪个环节最致命，优先加固以缩短潜在影响。", hint:"判断哪个环节最致命。" },
  { id:"av29", topic:"priv-esc", cat:"pentest", level:"高级", q:"防御利用链时“最小权限(least privilege)”的作用是？", options:["让单点失陷难以横向/提权，缩短链","提速","压缩","加密"], answer:0, explain:"即使某一环被突破，缺乏额外权限也让后续链难以延续。", hint:"让单点失陷难以横向/提权。" },
  ...
  { id:"av31", topic:"rop", cat:"binary", level:"高级", q:"64 位程序开启 NX+PIE+Canary+FULL RELRO，存在格式化字符串泄露与栈溢出，正确思路？", options:["先用格式化串泄露 libc 基址与 canary，再用 ROP(ret2libc)拿 shell","直接覆盖 GOT","直接跳 system","无需任何泄露"], answer:0, explain:"FULL RELRO 下 GOT 只读不可改，只能靠泄露地址后 ROP 调用 libc 函数。", hint:"FULL RELRO 不能改 GOT，靠 ROP。" },
  { id:"av32", topic:"rop", cat:"binary", level:"高级", q:"ret2csu 在 64 位利用中用于？", options:["利用 __libc_csu_init 的 gadget 设置寄存器后调用任意函数","加密","压缩","提速"], answer:0, explain:"当找不到直接 gadget 时，可借这段通用 gadget 布置 rdi/rsi/rdx 等再 call。", hint:"找不到直接 gadget 时借这段通用 gadget。" },
  { id:"av33", topic:"heap", cat:"binary", level:"高级", q:"tcache poisoning 的目标是？", options:["制造重叠块以覆写 __free_hook/目标指针实现任意写","提速","压缩","加密"], answer:0, explain:"通过构造重复节点，使 free 后写入的地址可控，从而改写关键函数指针。", hint:"控制 free 后写入的地址。" },
  { id:"av34", topic:"uaf", cat:"binary", level:"高级", q:"给定“使用后才释放”的 UAF，常见做法是？", options:["释放后保留指针，再次分配同尺寸块复用该内存改结构","立即置空指针","不利用","只读不改"], answer:0, explain:"复用已释放块可控制其中的类型/函数指针，造成类型混淆或劫持。", hint:"复用已释放块控制类型/函数指针。" },
  { id:"av35", topic:"ssti", cat:"web", level:"高级", q:"Jinja2 SSTI 读 flag 常用思路？", options:["借 `''.__class__.__mro__[1].__subclasses__()` 找到危险类(如 subprocess/os)执行命令","直接读文件","XSS","CSRF"], answer:0, explain:"从基础类型向上找可调用危险方法的类，进而执行系统命令。", hint:"从基础类型向上找可调用危险方法的类。" },
  ...
  { id:"av37", topic:"proto-poll", cat:"web", level:"高级", q:"原型链污染(prototype pollution)利用？", options:["覆写 Object.prototype 属性(如 __proto__.isAdmin=true)影响逻辑判定","XSS","CSRF","SQL 注入"], answer:0, explain:"通过 `.__proto__` 等键写进全局原型，影响后续所有对象的属性读取。", hint:"通过 `.__proto__` 等键写进全局原型。" },
  ...
  { id:"av39", topic:"deser", cat:"web", level:"高级", q:"Java 反序列化 gadget chain(如 CommonsCollections)目的？", options:["通过可控属性链最终调用 Runtime.exec 实现 RCE","只读取","只存储","只加密"], answer:0, explain:"反序列化时自动调用 setter/读属性触发链条，末端执行命令。", hint:"反序列化时自动调用触发链，末端执行命令。" },
  { id:"av40", topic:"asym", cat:"crypto", level:"高级", q:"同一明文用相同小公钥指数 e 对多个不同模数 N 加密(广播)，可用？", options:["中国剩余定理(CRT/Håstad)恢复明文","MD5","XSS","AES"], answer:0, explain:"同消息多模数下，联立同余可在不知私钥时解出明文（Håstad 广播攻击）。", hint:"同消息多模数 → 联立同余。" },
  { id:"av41", topic:"blockmode", cat:"crypto", level:"高级", q:"CTR 模式两密文 nonce 相同且已知其中一份明文，可？", options:["异或两份密文得两份明文异或，再结合已知明文恢复另一份","无解","提速","压缩"], answer:0, explain:"流密码 keystream 复用 = 明文异或，已知一份即可解另一份。", hint:"流密码 keystream 复用 = 明文异或。" },
  { id:"av42", topic:"sym", cat:"crypto", level:"高级", q:"ECB cut-and-paste 利用前提是？", options:["明文结构已知且按固定块对齐，可复制已知密文块拼接到自己数据","随机 IV","AEAD","流式加密"], answer:0, explain:"相同明文块=相同密文块，可像拼图一样重组出权限提升的密文。", hint:"相同明文块=相同密文块。" },
  { id:"av43", topic:"blockmode", cat:"crypto", level:"高级", q:"padding oracle 在 CTF 中逐字节恢复需？", options:["从末块往前逐字节调整 IV 使填充为 0x01/0x02… 观察响应差异","提速","压缩","加密"], answer:0, explain:"利用填充校验是否正确的“预言机”逐字节反推明文。", hint:"利用填充校验是否正确的“预言机”。" },
  { id:"av44", topic:"hash", cat:"crypto", level:"高级", q:"hash 长度扩展攻击(MD5/SHA1/SHA2)要求已知？", options:["原消息长度与原哈希(无需原消息内容)，即可在其后追加计算新哈希","明文","密钥","IV"], answer:0, explain:"Merkle-Damgård 压缩状态可接续，已知长度与哈希即可算 extension。", hint:"Merkle-Damgård 状态可接续。" },
  { id:"av45", topic:"race", cat:"binary", level:"高级", q:"逆向中 anti-debug 常见手段？", options:["ptrace 自检/定时器/断点检测阻止动态调试","只加密","只压缩","只提速"], answer:0, explain:"检测自身是否被调试，命中则走错误分支或退出。", hint:"检测自己是否被调试。" },
  ...
  { id:"np1", topic:"net-packet", cat:"network", level:"入门", q:"以下哪种协议默认以明文传输凭据，风险最高？", options:["HTTPS","SSH","Telnet","SFTP"], answer:2, explain:"Telnet 明文传输账号口令与全部会话内容，应改用 SSH 等加密协议。" },
  { id:"np2", topic:"net-proto", cat:"network", level:"入门", q:"TCP/IP 模型中，IP 协议主要负责？", options:["端到端可靠传输","寻址与路由","应用层加密","端口复用"], answer:1, explain:"IP 负责寻址与路由，可靠性由 TCP 提供，二者分层不同。" },
  ...
  { id:"np4", topic:"port-scan", cat:"network", level:"初级", q:"SYN 半开扫描（nmap -sS）相比全连接扫描的特点是？", options:["必须完成三次握手","不完成握手，更快且更隐蔽","只能扫描 UDP","需要目标应用层认证"], answer:1, explain:"只发 SYN、收到 SYN/ACK 即回 RST，不在目标留下完整连接记录。" },
  { id:"np5", topic:"port-scan", cat:"network", level:"初级", q:"nmap 中 -sV 参数的作用是？", options:["探测服务名称与版本","识别操作系统","执行漏洞利用脚本","指定端口范围"], answer:0, explain:"-sV 做服务与版本探测；-O 才是系统识别，-sC 执行默认脚本。" },
  { id:"np6", topic:"port-scan", cat:"network", level:"初级", q:"对大型目标做资产测绘时，合理的执行顺序是？", options:["先快速全端口探测，再对开放端口做服务与版本识别","对全部端口都做深度版本探测","只扫常见 100 个端口就够了","先跑漏洞利用脚本"], answer:0, explain:"先粗后细控制时间成本；只扫常见端口会漏掉跑在非常规端口上的服务。" },
  { id:"np7", topic:"arp-dns", cat:"network", level:"中级", q:"在交换网络中实施中间人（MITM）最常用的手段是？", options:["ARP 欺骗，把网关 IP 映射到攻击者 MAC","爆破 Wi-Fi 口令","伪造 HTTPS 证书即可","必须物理接触交换机"], answer:0, explain:"伪造 ARP 应答即可让流量经过攻击者；HTTPS 只能保护内容，不能阻止流量被转发。" },
  { id:"np8", topic:"net-dhcp", cat:"network", level:"中级", q:"防御 ARP 欺骗最有效的手段是？", options:["交换机开启 DHCP Snooping + 动态 ARP 检测（DAI）","启用 HTTPS","关闭 ICMP","更换交换机品牌"], answer:0, explain:"在接入层校验 IP-MAC 绑定并丢弃伪造 ARP 应答；HTTPS 不能阻止 ARP 欺骗本身。" },
  { id:"np9", topic:"net-dns-sec", cat:"network", level:"中级", q:"DNS 缓存投毒的核心目的是？", options:["把域名解析结果改为攻击者控制的地址","加密 DNS 查询","提升解析速度","隐藏服务器真实 IP"], answer:0, explain:"污染递归解析器缓存后可劫持大量用户访问；DNSSEC 与随机化查询 ID/端口可缓解。" },
  ...
  { id:"np11", topic:"net-lateral", cat:"network", level:"高级", q:"内网横向移动中「凭据复用」最常见的来源是？", options:["本地管理员口令在多台机器上相同","浏览器书签","DNS 缓存","交换机 MAC 地址表"], answer:0, explain:"统一镜像带来统一本地管理员口令，攻陷一台等于攻陷一片；应用 LAPS 随机化本地口令。" },
  { id:"np12", topic:"net-lateral", cat:"network", level:"高级", q:"降低内网横向移动风险最有效的架构手段是？", options:["管理与特权分层 + 最小权限（LAPS、跳板机）","关闭 445 端口即可","全员改用 PIN 码","把口令长度提到 20 位"], answer:0, explain:"横向移动依赖可复用的高权限凭据与信任关系，分层管理直接切断复用链。" },
  { id:"np13", topic:"net-proto", cat:"network", level:"高级", q:"Kerberoasting 的攻击对象是？", options:["带 SPN 的服务账户所请求的 TGS 票据","域用户明文口令","域控注册表文件","DNS 区域文件"], answer:0, explain:"任意域用户都能请求带 SPN 账户的 TGS，离线爆破其弱口令；应使用长随机口令与服务账户托管。" },
  { id:"np14", topic:"ad-pentest", cat:"network", level:"高级", q:"AS-REP Roasting 成立的前提是账户？", options:["关闭了 Kerberos 预认证（不要求预认证）","口令长度超过 14 位","启用了 NTLM","加入了信任域"], answer:0, explain:"无预认证时可直接请求 AS-REP 并离线爆破；应确保所有账户开启预认证。" },
  { id:"np15", topic:"ad-pentest", cat:"network", level:"高级", q:"域环境里常见的权限维持手法是？", options:["伪造 Kerberos 票据（金票据 / 银票据）","修改 DNS 转发","安装杀毒软件","禁用主机防火墙"], answer:0, explain:"金票据伪造 TGT、银票据伪造 TGS，可长期以高权限访问；处置需重置 krbtgt 等关键账号。" },
  { id:"np16", topic:"fw-bypass", cat:"network", level:"中级", q:"利用分片（fragmentation）绕过检测的原理是？", options:["把载荷切分，使特征无法在单个报文内匹配","加密流量无法被解密","改用 IPv6 即可绕过","降低发包速率"], answer:0, explain:"IDS 若不做报文重组就匹配不到跨包特征，而目标主机重组后仍能正常处理。" },
  { id:"np17", topic:"net-devices", cat:"network", level:"中级", q:"针对基于特征的 IDS，最通用的规避思路是？", options:["改变编码/大小写/等价语法，使特征失配但目标仍能解析","关闭自身防火墙","更换代理 IP","提高带宽"], answer:0, explain:"URL 编码、内联注释、大小写混合都是常见做法；防御方需先规范化再匹配。" },
  { id:"np18", topic:"net-dns-sec", cat:"network", level:"中级", q:"出口限制严格的环境中，C2 流量最常伪装成？", options:["HTTPS/DNS 等常见协议","原始 IP 协议 0","大流量 UDP 直播","超大 ICMP 包"], answer:0, explain:"混入常见协议的 C2 最难被简单封禁拦住，需结合 TLS 指纹与行为分析检测。" },
  { id:"cp1", topic:"cloud-services", cat:"cloud", level:"入门", q:"在 IaaS 模式下，属于「客户」安全责任的是？", options:["物理机房安全","宿主机硬件维护","客户数据与访问配置","云厂商网络设备固件"], answer:2, explain:"IaaS 下客户负责镜像以上部分（系统补丁、数据、配置、身份）；物理与虚拟化层由厂商负责。" },
  ...
  { id:"cp3", topic:"cloud-storage-sec", cat:"cloud", level:"初级", q:"云上「配置错误」成为最常见风险的主要原因是？", options:["安全边界从网络转向身份与配置，默认配置常过宽","云厂商不提供安全能力","云上无法审计","云网络不加密"], answer:0, explain:"云上一切由 API 配置驱动：一个公开的对象存储或过宽角色就等于敞开的门。" },
  { id:"cp4", topic:"cloud-monitor", cat:"cloud", level:"初级", q:"最小权限原则的核心是？", options:["只授予完成任务所需的最小权限并定期收敛","所有服务共用管理员密钥","给所有用户只读权限","关闭云审计日志"], answer:0, explain:"最小权限 + 定期复核 + 权限边界，能显著降低凭据泄露后的影响面。" },
  { id:"cp5", topic:"iam", cat:"cloud", level:"中级", q:"云上长期访问密钥（AK/SK）泄露最危险之处在于？", options:["长期有效、可直接调用 API 且难以自动轮换","只能读不能写","必须配合口令才能使用","无法在云环境之外使用"], answer:0, explain:"长期密钥泄露后可被持续滥用；应改用角色与临时凭证（STS），并开启密钥轮换与异常调用检测。" },
  { id:"cp6", topic:"iam", cat:"cloud", level:"初级", q:"更安全的云上身份方案是？", options:["使用角色 / 临时凭证（STS）替代长期密钥","把密钥写入镜像方便程序读取","用邮件传递密钥","关闭 MFA"], answer:0, explain:"临时凭证自动过期且可限定权限范围，暴露风险显著低于长期密钥。" },
  ...
  { id:"cp8", topic:"container-escape", cat:"cloud", level:"高级", q:"把 docker.sock 挂载进容器内的后果是？", options:["容器内可驱动 Docker 守护进程，等同获得宿主机 root","没有任何影响","只影响容器网络","只影响镜像构建速度"], answer:0, explain:"可借此创建特权容器并挂载宿主机根目录，是最常见的容器逃逸路径之一。" },
  { id:"cp9", topic:"container-escape", cat:"cloud", level:"高级", q:"内核漏洞导致容器逃逸的根本原因是？", options:["容器与宿主机共享同一内核，隔离依赖内核机制","容器内没有文件系统","镜像未签名","未启用 HTTPS"], answer:0, explain:"隔离靠 namespace/cgroup 等内核能力，内核漏洞即可突破；需及时打补丁并使用安全运行时。" },
  { id:"cp10", topic:"cloud-image-govern", cat:"cloud", level:"中级", q:"Kubernetes Pod 安全加固的基线手段是？", options:["限制特权、根文件系统只读、丢弃多余 capabilities、禁用 hostPath","只设置资源限额","只加注解说明","只修改镜像 tag"], answer:0, explain:"Pod Security 的基线/受限策略能挡掉绝大多数提权姿势，privileged 与 hostPath 最危险。" },
  { id:"cp11", topic:"k8s", cat:"cloud", level:"高级", q:"Kubernetes 中 RBAC 权限过宽的典型表现是？", options:["把 cluster-admin 或通配 verbs/resources 授予普通工作负载","未开启审计日志","使用 Helm 部署","部署在公有云上"], answer:0, explain:"通配权限让一个被攻陷的 Pod 能控制整个集群；应按需最小授权并禁止自动挂载 SA Token。" },
  ...
  { id:"cp13", topic:"serverless", cat:"cloud", level:"中级", q:"FaaS 中函数执行角色权限过大的典型风险是？", options:["函数被利用后可调用账号内其他云资源","函数无法被触发","冷启动变慢","只能读取日志"], answer:0, explain:"事件注入或依赖投毒后攻击者会继承函数角色权限；应按函数粒度最小授权。" },
  { id:"cp14", topic:"serverless", cat:"cloud", level:"中级", q:"Serverless 场景中的「事件注入」指的是？", options:["把恶意内容放入触发函数的事件数据，被拼接到命令/查询中执行","修改函数源码","伪造函数名称","关闭函数触发器"], answer:0, explain:"对象存储键名、HTTP 参数等事件字段都可能携带注入载荷，必须校验并参数化处理。" },
  { id:"cp15", topic:"serverless", cat:"cloud", level:"中级", q:"Serverless 的依赖风险为何格外突出？", options:["依赖来自公共仓库、随函数打包且更新滞后","函数不联网","函数没有日志","函数无网络访问"], answer:0, explain:"被投毒或含漏洞的依赖会随每次调用执行；应锁定版本、扫描依赖并定期重建。" },
  { id:"cp16", topic:"metadata", cat:"cloud", level:"中级", q:"SSRF 打云元数据服务时的典型目标地址是？", options:["169.254.169.254（IMDS）","127.0.0.1","8.8.8.8","224.0.0.1"], answer:0, explain:"IMDS 可返回实例临时凭证，进而横向控制云资源；应强制 IMDSv2 并限制跳数。" },
  { id:"cp17", topic:"metadata", cat:"cloud", level:"中级", q:"IMDSv2 相比 v1 的核心改进是？", options:["需先通过 PUT 获取 token 且限制跳数，显著抬高 SSRF 利用门槛","改为加密传输","变为只读接口","仅支持 IPv6"], answer:0, explain:"token 机制与 hop limit 让简单 SSRF 无法直接读取凭证，是必须开启的加固项。" },
  { id:"cp18", topic:"metadata", cat:"cloud", level:"中级", q:"发现云上临时凭证泄露后的处置顺序应是？", options:["先禁用或轮换凭证并查调用记录，再评估影响范围","先重装受影响主机","先通知全体员工","先删除全部日志"], answer:0, explain:"止血（断可用性）与留证（操作审计）并行；删除日志会破坏溯源链。" },
  { id:"bp1", topic:"siem", cat:"blue", level:"初级", q:"SIEM 的核心能力是？", options:["多源日志集中存储 + 关联分析与告警","直接阻断攻击流量","替代防火墙","自动安装补丁"], answer:0, explain:"SIEM 负责汇聚、关联、检索与告警；阻断属于 IPS/EDR/防火墙的职责。" },
  { id:"bp2", topic:"blue-log", cat:"blue", level:"初级", q:"决定检测能力上限最关键的因素是？", options:["日志源覆盖范围与保留期","界面是否美观","存储设备品牌","告警条数多少"], answer:0, explain:"没有日志就没有检测与溯源；先保证认证、边界、关键服务器日志接入且保留期足够。" },
  { id:"bp3", topic:"siem", cat:"blue", level:"初级", q:"下列最应优先接入 SIEM 的日志是？", options:["认证/域控、边界设备与关键服务器的安全日志","打印机耗材状态","桌面壁纸变更记录","应用界面点击流"], answer:0, explain:"优先接入高价值安全事件源，最快形成有效检测能力。" },
  { id:"bp4", topic:"ids", cat:"blue", level:"中级", q:"基于特征的 IDS 的固有短板是？", options:["难以发现未知攻击与变形绕过","完全无法处理流量","需要人工逐包点击确认","只能检测加密流量"], answer:0, explain:"特征库只覆盖已知模式，需配合行为/异常检测与威胁情报。" },
  { id:"bp5", topic:"ids", cat:"blue", level:"中级", q:"IPS 与 IDS 的关键差别是？", options:["IPS 串接在链路中并可主动阻断","IPS 只记录日志","IDS 可以阻断流量","两者完全等价"], answer:0, explain:"IPS 在线串接能阻断但存在误杀风险；IDS 旁路只告警，不影响业务链路。" },
  { id:"bp6", topic:"ids", cat:"blue", level:"中级", q:"降低 IDS 误报率的常见做法是？", options:["规则调优 + 白名单 + 结合情报与行为基线","直接关闭告警","把阈值调到不再报警","删除所有高危规则"], answer:0, explain:"误报治理靠上下文与基线，而不是关告警——关掉等于放弃检测。" },
  { id:"bp7", topic:"threat-intel", cat:"blue", level:"中级", q:"TLS 普及后，识别 C2 流量可行的信号是？", options:["TLS 指纹（JA3/JA4）、心跳周期性、DNS 异常与情报匹配","直接读取明文 URL","只看 IP 归属地","看报文长度是否为偶数"], answer:0, explain:"加密环境下要转向元数据与行为特征，并配合威胁情报做匹配。" },
  { id:"bp8", topic:"ids", cat:"blue", level:"中级", q:"数据外传（exfiltration）的典型网络特征是？", options:["对单一目标的长时间稳定上传，常在非工作时段","短时广播风暴","少量 ICMP 回显请求","普通的域名 A 记录查询"], answer:0, explain:"稳定速率 + 长时长 + 非工作时段是经典外传画像，应建立流量基线并对偏离告警。" },
  { id:"bp9", topic:"ids", cat:"blue", level:"中级", q:"检测 Beacon 心跳最有效的方法是？", options:["统计固定间隔的小流量会话并做周期性分析","只看端口号","只看目的 IP 数量","只看总带宽"], answer:0, explain:"心跳的规律性是最强特征；结合 jitter 分析与情报可显著提高准确率。" },
  { id:"bp10", topic:"edr", cat:"blue", level:"高级", q:"勒索事件发生后，第一批动作的正确顺序是？", options:["隔离受影响主机与网段防扩散，同时保留现场与取证数据","立即全盘格式化","立即支付赎金","先重启所有服务器"], answer:0, explain:"先遏制、再取证、后恢复；格式化与盲目重启都会破坏证据并可能加速加密。" },
  { id:"bp11", topic:"edr", cat:"blue", level:"高级", q:"为什么不建议第一时间重启或断电？", options:["会丢失内存证据，并可能中断尚未完成的取证与遏制","会损坏硬盘磁头","会导致机房断电","没有任何影响"], answer:0, explain:"内存中有加密密钥、进程与网络连接线索；应隔离并先做内存镜像再处置。" },
  { id:"bp12", topic:"ir", cat:"blue", level:"高级", q:"恢复阶段最关键的验证是？", options:["确认备份未被污染且能干净还原（离线 / 不可变备份）","备份文件个数","备份软件品牌","备份压缩率"], answer:0, explain:"勒索常先破坏在线备份；恢复前必须在隔离环境验证还原结果的可用与洁净。" },
  { id:"bp13", topic:"threat-intel", cat:"blue", level:"初级", q:"IOC 与 IOA 的区别是？", options:["IOC 是静态指标（哈希/IP/域名），IOA 是攻击行为与手法特征","两者完全相同","IOA 只用于文件检测","IOC 只用于网络设备"], answer:0, explain:"IOC 见效快但易过期，IOA 难规避；两者结合效果最好。" },
  { id:"bp14", topic:"threat-intel", cat:"blue", level:"初级", q:"把威胁情报用于检测最直接的方式是？", options:["将 IOC 导入检测设备或日志检索做匹配","直接阻断所有外网访问","打印情报报告存档","关闭防火墙"], answer:0, explain:"情报必须落成可执行规则（匹配、检索、告警），否则只是阅读材料。" },
  { id:"bp15", topic:"threat-intel", cat:"blue", level:"中级", q:"威胁情报的「可执行性」指的是？", options:["能直接转成检测/阻断规则并可验证有效性","描述文字足够长","来自知名厂商","配有图表"], answer:0, explain:"可执行情报包含上下文（TTP、影响面）并能落成规则，这才是可用情报。" },
  { id:"bp16", topic:"edr", cat:"blue", level:"初级", q:"EDR 与传统杀毒软件的核心区别是？", options:["基于行为与遥测持续检测与响应，而非仅靠特征匹配","只查本地病毒库","只做磁盘加密","只做补丁管理"], answer:0, explain:"EDR 关注进程行为链与响应动作（隔离、取证），对未知威胁更有效。" },
  { id:"bp17", topic:"edr", cat:"blue", level:"初级", q:"终端遥测中对溯源最有价值的数据是？", options:["进程树与命令行、父子进程关系","桌面壁纸","屏幕分辨率","输入法设置"], answer:0, explain:"命令行与进程父子关系能还原完整攻击链，是 EDR 溯源的核心数据。" },
  { id:"bp18", topic:"edr", cat:"blue", level:"中级", q:"EDR 的「隔离主机」动作的价值是？", options:["切断攻击者横向移动与 C2 通道，同时保留取证数据","删除主机全部文件","自动重装操作系统","关闭全部日志"], answer:0, explain:"网络隔离是遏制与取证的平衡做法；直接删除或重装会毁掉证据。" },

  // ---- 移动安全（v1.5.1 第二批：新增领域）----
  { id:"mob1", topic:"mobile-attack-surface", cat:"mobile", level:"入门", q:"为什么说「客户端不可信」？", options:["因为手机性能不足","因为 App 装在做题者/攻击者可控的设备上，可被逆向与改包","因为移动网络不安全","因为安卓开源"], answer:1, explain:"客户端代码与数据都在用户设备上，可被反编译、调试与篡改；安全校验必须放在服务端。" },
  { id:"mob2", topic:"mobile-attack-surface", cat:"mobile", level:"入门", q:"移动应用安全评估的三条主线通常指？", options:["客户端、数据存储、通信与接口","界面、性能、兼容性","安装、卸载、升级","广告、推送、统计"], answer:0, explain:"客户端（逆向/调试）、本地数据、通信与接口是移动安全的三个主要风险面。" },
  { id:"mob3", topic:"apk-sign", cat:"mobile", level:"初级", q:"重打包（改包）后必须做的一件事是？", options:["重新签名","重新申请包名","重新上架应用商店","重装系统"], answer:0, explain:"改动内容会让原签名失效，必须用自己的密钥重新签名，否则无法安装。" },
  { id:"mob4", topic:"apk-sign", cat:"mobile", level:"初级", q:"应用签名校验的作用是？", options:["保证安装包未被篡改、且来自该开发者","加密应用数据","提升运行速度","限制安装设备数量"], answer:0, explain:"签名是完整性与来源的凭证；改包后签名必然不一致。" },
  { id:"mob5", topic:"apk-sign", cat:"mobile", level:"初级", q:"AndroidManifest 中 exported=true 的组件意味着？", options:["可被其它应用调用，需要额外权限校验","只在应用内可用","必须系统签名","仅调试模式可见"], answer:0, explain:"导出组件是对外入口，若缺少权限/调用方校验就可能被越权调用。" },
  { id:"mob6", topic:"component-export", cat:"mobile", level:"初级", q:"用 adb 验证组件是否可被外部拉起，常用命令是？", options:["adb shell am start -n 包名/组件","adb install","adb logcat","adb reboot"], answer:0, explain:"am start 可直接拉起指定 Activity；同理 am broadcast、content query 验证广播与 Provider。" },
  { id:"mob7", topic:"webview", cat:"mobile", level:"中级", q:"WebView 中 addJavascriptInterface 的风险是？", options:["网页中的 JS 可调用原生对象的方法，页面可控时可能越权读写本地数据","会拖慢页面加载","会禁用 JavaScript","只影响渲染样式"], answer:0, explain:"桥接暴露了原生能力，若加载的 URL 可控即可被恶意页面利用；应严格白名单并最小化暴露。" },
  { id:"mob8", topic:"webview", cat:"mobile", level:"中级", q:"WebView 加固的正确做法是？", options:["严格控制加载域名白名单、关闭 file 协议、只暴露必要接口并校验参数","开启全部 JS 能力方便开发","允许加载任意 URL","把敏感方法都挂到桥对象上"], answer:0, explain:"WebView 要按不可信输入处理：白名单 + 最小暴露 + 参数校验。" },
  { id:"mob9", topic:"local-storage", cat:"mobile", level:"入门", q:"下列哪项最容易导致本地敏感数据泄露？", options:["把令牌写入日志、SharedPreferences 明文存储并开启 allowBackup","使用 Keystore 保存密钥","关闭敏感日志","数据不落地"], answer:0, explain:"明文落盘 + 日志输出 + 可备份，三者叠加会让凭据暴露；应尽量不落地并用系统密钥库。" },
  { id:"mob10", topic:"local-storage", cat:"mobile", level:"入门", q:"检测 App 是否把敏感信息写进日志，常用命令是？", options:["adb logcat | grep -i token","adb install","adb shell am start","adb shell dumpsys cpuinfo"], answer:0, explain:"logcat 过滤关键字（token/password/secret）能快速发现敏感日志。" },
  { id:"mob11", topic:"ssl-pinning", cat:"mobile", level:"初级", q:"证书固定（Certificate Pinning）的作用是？", options:["把信任范围收窄到指定证书/公钥，降低自签根证书代理抓包的成功率","加密本地文件","提升下载速度","防止应用被重打包"], answer:0, explain:"Pinning 让仅凭系统信任链的中间人失效；但可被运行时 hook 绕过，服务端仍需鉴权。" },
  { id:"mob12", topic:"ssl-pinning", cat:"mobile", level:"初级", q:"在授权测试中遇到证书固定导致抓包失败，常见处理方式是？", options:["用 Frida hook 掉 X509TrustManager/CertificatePinner 等校验点","直接放弃测试","关闭手机 Wi-Fi","改用 HTTP 明文接口"], answer:0, explain:"动态 hook 校验函数是标准绕过手段；注意这是测试手段，不代表线上可被轻易利用。" },
  { id:"mob13", topic:"mob-reverse", cat:"mobile", level:"中级", q:"Frida 在移动测试中的主要用途是？", options:["运行时 hook 函数、修改返回值、观察真实行为","静态反编译 dex","打包签名","抓取网络包"], answer:0, explain:"Frida 是动态分析核心工具；静态反编译用 jadx/Ghidra，抓包用代理。" },
  { id:"mob14", topic:"root-detect", cat:"mobile", level:"中级", q:"App 检测到调试器就退出，这类防护的定位是？", options:["提高分析成本，但可被绕过，不能作为唯一防线","彻底阻断逆向","等同服务端鉴权","属于加密手段"], answer:0, explain:"反调试/反 Root 属军备竞赛，只能提高门槛；真正的防线是服务端校验与数据最小化。" },
  { id:"mob15", topic:"root-detect", cat:"mobile", level:"中级", q:"绕过 Root 检测最常用的工程化做法是？", options:["用 Magisk 的隐藏能力（DenyList/Zygisk）并配合 hook 返回假值","卸载所有安全软件","改用 iPhone","关闭开发者选项"], answer:0, explain:"隐藏 Root 痕迹 + hook 检测点为假值，是标准组合拳；但仍可能被云端完整性校验发现。" },
  { id:"mob16", topic:"root-detect", cat:"mobile", level:"中级", q:"对抗「越狱/Root 检测」时最关键的评估产出是？", options:["绕过之后攻击者究竟能获得什么权限与数据","检测代码有多少行","使用了哪种编程语言","厂商名称"], answer:0, explain:"绕过本身不是目的，要评估由此带来的真实越权后果与影响面。" },
  { id:"mob17", topic:"app-hardening", cat:"mobile", level:"高级", q:"脱壳（dump dex）通常发生在什么时候？", options:["应用启动、壳完成解密、真实代码已加载到内存之后","安装过程中","下载 APK 之前","卸载之后"], answer:0, explain:"抽取式壳在运行时才解密出真实代码，必须在内存中 dump 再修复 dex。" },
  { id:"mob18", topic:"mob-reverse", cat:"mobile", level:"高级", q:"OLLVM 控制流平坦化给逆向带来的主要困难是？", options:["打乱控制流，使逻辑难以还原，需要去平坦化分析","加密网络流量","隐藏应用图标","禁用调试端口"], answer:0, explain:"控制流平坦化会显著增加静态分析成本，常需配合动态 trace 与脚本化还原。" },
  { id:"mob19", topic:"ios-basics", cat:"mobile", level:"初级", q:"iOS 应用包（IPA）解包后应优先关注什么？", options:["Info.plist 中的 ATS 例外、URL Scheme 与权限配置","图片资源数量","字体大小","应用图标样式"], answer:0, explain:"ATS 放宽与 URL Scheme 暴露是 iOS 侧常见风险点，先看配置再看二进制。" },
  { id:"mob20", topic:"ios-basics", cat:"mobile", level:"初级", q:"iOS 侧存储敏感数据更推荐的方式是？", options:["Keychain（并正确设置访问控制）","明文写入 plist","写进 NSUserDefaults","写进日志"], answer:0, explain:"Keychain 由系统加密保管；plist/UserDefaults 明文易被读取，日志更会直接泄露。" },
  { id:"mob21", topic:"mobile-api", cat:"mobile", level:"中级", q:"移动端接口最常见的越权类型是？", options:["水平越权：改 id 就能看/改他人数据","接口不支持 HTTPS","接口返回 JSON","接口使用 REST 风格"], answer:0, explain:"服务端只校验登录、不校验数据归属，是移动接口最典型的漏洞。" },
  { id:"mob22", topic:"apk-sign", cat:"mobile", level:"中级", q:"为什么「客户端加密/签名」不能替代服务端校验？", options:["因为客户端逻辑可被逆向与重放，攻击者能绕过或复现签名流程","因为加密速度慢","因为会增大包体","因为不受开发者控制"], answer:0, explain:"客户端签名只能抬高改包成本，无法阻止脚本化调用与重放；鉴权与风控必须在服务端。" },
  { id:"mob23", topic:"mobile-api", cat:"mobile", level:"中级", q:"评估移动接口是否可被批量滥用，最直接的验证是？", options:["对同一接口高频重复调用，观察是否有频率限制与风控拦截","查看接口返回码个数","统计接口文档页数","检查是否用了 CDN"], answer:0, explain:"缺少限流与风控会导致枚举/撞库/羊毛；需实测确认。" },
  { id:"mob24", topic:"mob-permission", cat:"mobile", level:"高级", q:"移动恶意样本最常申请的敏感权限/能力是？", options:["无障碍服务、短信与通话权限","蓝牙配对","屏幕亮度调节","振动"], answer:0, explain:"无障碍服务可读屏与自动点击、短信权限可窃取验证码，二者是移动恶意的常见组合。" },
  { id:"mob25", topic:"mobile-malware", cat:"mobile", level:"高级", q:"分析移动恶意样本时提取 IOC 的主要来源是？", options:["静态字符串中的域名/URL 与运行时抓包得到的 C2","应用图标的配色","安装包体积","开发者签名算法"], answer:0, explain:"域名、URL、C2 地址是 IOC 核心，静态提取与动态抓包互相补充。" },
  { id:"mob26", topic:"mob-reverse", cat:"mobile", level:"高级", q:"多阶段载荷（下载器 + 二次加载）对分析的影响是？", options:["核心恶意逻辑不在初始包中，需要动态跟踪后续加载","样本无法运行","不需要分析网络行为","只能在静态阶段发现"], answer:0, explain:"二次加载要求动态分析跟踪落地文件与解密过程，否则无法看清真实行为。" },

  // ---- 数据安全与隐私 / 供应链安全（v1.5.1 第三批）----
  { id:"ds1", topic:"ds-lifecycle", cat:"datasec", level:"入门", q:"数据分类分级的首要作用是？", options:["决定该数据需要多强的保护与审批流程","提升数据库性能","减少存储成本","方便做报表"], answer:0, explain:"分级是数据安全的地基：加密强度、访问审批、脱敏与留存策略都由等级驱动。" },
  { id:"ds2", topic:"ds-lifecycle", cat:"datasec", level:"入门", q:"数据生命周期通常不包括下列哪一项？", options:["生物特征采集认证","采集","存储与使用","销毁"], answer:0, explain:"生命周期一般指采集—传输—存储—使用—共享—销毁；生物特征认证属于身份技术，不在其列。" },
  { id:"ds3", topic:"ds-gateway", cat:"datasec", level:"初级", q:"发现某数据库列名含 phone、id_card 等关键字，下一步应该？", options:["抽样核查内容并纳入分级与脱敏范围","直接删除这些列","忽略，列名不代表什么","关闭数据库审计"], answer:0, explain:"列名只是线索，需抽样验证准确率后纳入分级，并驱动加密/脱敏/审计。" },
  { id:"ds4", topic:"ds-mask", cat:"datasec", level:"初级", q:"给测试环境提供生产数据时，正确做法是？", options:["先脱敏或使用合成数据","直接复制生产库","只改表名","只做权限限制"], answer:0, explain:"测试环境安全水位低，直接复制生产数据会放大泄露面；应脱敏或使用合成数据。" },
  { id:"ds5", topic:"ds-dlp", cat:"datasec", level:"中级", q:"DLP 上线初期最稳妥的策略是？", options:["先观察记录、统计误报，再逐步收紧阻断","直接全量阻断所有外发","只做终端不做网络","只对高管启用"], answer:0, explain:"直接阻断会引发业务绕过；观察—收敛—常态化是已被验证的路径。" },
  { id:"ds6", topic:"ds-dlp", cat:"datasec", level:"中级", q:"下列哪种做法最可能让 DLP 失效？", options:["员工把文件压缩加密后通过个人网盘外发","使用公司邮箱发送","在终端复制文件","打印文件"], answer:0, explain:"压缩加密与个人通道会绕过基于内容识别的 DLP；需结合终端管控、通道封禁与审计。" },
  { id:"ds7", topic:"ds-mask", cat:"datasec", level:"初级", q:"脱敏后仍需评估「重标识风险」的原因是？", options:["保留生日、邮编等准标识符组合后仍可能反推出个人身份","脱敏会降低性能","脱敏成本高","法规禁止脱敏"], answer:0, explain:"掩码不等于匿名，准标识符组合可重标识；需做 k-匿名等风险评估。" },
  { id:"ds8", topic:"ds-mask", cat:"datasec", level:"初级", q:"关于脱敏与假名化，说法正确的是？", options:["假名化数据在合规上通常仍属个人信息","假名化等同于完全匿名","脱敏可以随意恢复","掩码后即可公开"], answer:0, explain:"假名化只是替换标识，借助映射表仍可还原，因此仍受个人信息保护规则约束。" },
  { id:"ds9", topic:"ds-encrypt", cat:"datasec", level:"中级", q:"密钥管理中最容易被忽视但最致命的问题是？", options:["密钥与数据放在一起，或硬编码在代码/配置里","密钥长度不够","使用 AES 而非国密","未使用 HSM"], answer:0, explain:"密钥泄露等于没加密；密钥必须与数据分离并放入 KMS/HSM，禁止硬编码。" },
  { id:"ds10", topic:"ds-encrypt", cat:"datasec", level:"中级", q:"信封加密（Envelope Encryption）的核心思想是？", options:["用数据密钥加密数据，再用主密钥加密数据密钥，密钥分层便于轮换与控制","对文件加密两次","用两个相同密钥","先压缩再加密"], answer:0, explain:"分层密钥让数据密钥可频繁轮换，而主密钥始终留在 KMS/HSM 中。" },
  { id:"ds11", topic:"ds-access", cat:"datasec", level:"初级", q:"数据权限治理中「权限复核」主要解决什么问题？", options:["长期累积的过期授权（如离职人员、岗位变更后仍保留的权限）","数据库性能","备份速度","日志体积"], answer:0, explain:"权限只增不减会形成隐性高权限；定期复核与回收是核心动作。" },
  ...
  { id:"ds13", topic:"ds-audit", cat:"datasec", level:"中级", q:"数据审计日志至少要记录？", options:["账号、时间、来源、对象、动作与影响范围","只有操作时间","只有账号名","只有错误信息"], answer:0, explain:"缺任何一项都难以定责或评估影响范围；日志本身还需防篡改与集中留存。" },
  { id:"ds14", topic:"ds-audit", cat:"datasec", level:"中级", q:"审计中发现某账号在离职前一周大量导出数据，最应优先做什么？", options:["立即冻结该账号并固定证据、核查导出范围","先通知本人解释","等离职当天再看","只删除日志"], answer:0, explain:"离职前集中导出是典型内部威胁信号，先止血与留证，再评估影响与定责。" },
  { id:"ds15", topic:"ds-privacy-law", cat:"datasec", level:"入门", q:"个人信息保护中的「告知同意」要求？", options:["事前告知用途并取得同意，且同意可撤回","只要不泄露就无需告知","用户注册即代表同意一切","口头约定即可"], answer:0, explain:"告知同意是合规基础：用途明确、同意有效、可撤回，且敏感信息需单独同意。" },
  { id:"ds16", topic:"ds-minimize", cat:"datasec", level:"入门", q:"「最小必要」原则的含义是？", options:["只收集实现功能所必需的个人信息，并只保留必要期限","尽量少收但可无限期保存","只要用户同意就能随便收","只在节假日收集"], answer:0, explain:"最小必要同时约束字段范围与留存期限，收多了就是风险。" },
  ...
  { id:"ds18", topic:"ds-ai-data", cat:"datasec", level:"初级", q:"用户行使「删除权」时，工程上必须保证？", options:["主库、副本、备份与第三方同步链路都能删除或匿名化","只删主库即可","只需在产品界面隐藏","延后一年处理"], answer:0, explain:"数据分散在多处，删除请求必须在全链路生效，否则合规上不成立。" },
  { id:"ds19", topic:"ds-cross-border", cat:"datasec", level:"高级", q:"数据出境合规的第一步通常是？", options:["做数据出境盘点：系统、字段、接收方、国家地区与用途","直接签标准合同","先上线再补材料","关闭所有跨境访问"], answer:0, explain:"没有清单就无法判断路径与必要性；盘点后本地化或选择相应合规路径。" },
  { id:"ds20", topic:"ds-pii", cat:"datasec", level:"高级", q:"下列哪种情形通常必须单独取得同意？", options:["向第三方提供个人信息或处理敏感个人信息","统计站点访问量","记录系统错误","压缩日志"], answer:0, explain:"对外提供与敏感个人信息处理属单独同意场景，需单独告知并留痕。" },
  { id:"ds21", topic:"ds-leak-response", cat:"datasec", level:"高级", q:"数据泄露事件处置的正确顺序是？", options:["隔离与止血 → 固定证据 → 评估影响 → 依法通报与告知 → 整改复盘","先对外道歉再慢慢查","先删除日志保护隐私","先通知媒体"], answer:0, explain:"顺序错了会导致证据丢失或影响扩大；留证与通报时限都要满足合规要求。" },
  { id:"ds22", topic:"ds-leak-response", cat:"datasec", level:"高级", q:"判断泄露影响范围时，最关键的评估维度是？", options:["数据类型与等级、可识别性（能否重标识）与涉及人数","日志文件大小","服务器数量","事件持续秒数"], answer:0, explain:"影响评估要围绕数据敏感度与可识别性，才能决定通报范围与补救措施。" },
  { id:"ds23", topic:"ds-insider", cat:"datasec", level:"中级", q:"防御内部人员权限滥用最有效的一组措施是？", options:["最小权限 + 行为基线告警 + 高敏操作双人复核","全员只读","关闭所有审计日志","只依赖员工自觉"], answer:0, explain:"技术收敛 + 行为发现 + 流程制衡三者缺一不可。" },
  { id:"ds24", topic:"ds-leak-scene", cat:"datasec", level:"中级", q:"离职流程中与数据安全最相关的一项动作是？", options:["当天回收全部账号与权限并检查近期导出记录","退还门禁卡","交接文档","回收电脑"], answer:0, explain:"账号与权限回收不及时是内部威胁的主要窗口；同时核查有无异常导出。" },
  { id:"ds25", topic:"ds-backup", cat:"datasec", level:"高级", q:"备份「不可变（WORM）」的价值是？", options:["即使管理凭据被窃取，攻击者也无法在保留期内删除或覆盖备份","节省存储空间","提高备份速度","减少备份数量"], answer:0, explain:"勒索攻击常先毁备份；不可变或离线副本是最后的恢复保证。" },
  { id:"ds26", topic:"ds-backup", cat:"datasec", level:"高级", q:"验证备份是否真的可用的正确方式是？", options:["定期做恢复演练并记录实测 RTO/RPO","看备份文件大小是否正常","看备份任务是否显示成功","看备份软件版本"], answer:0, explain:"未经验证的备份不算备份；演练要覆盖最坏场景并记录恢复耗时与数据丢失窗口。" },
  { id:"sc1", topic:"sc-concept", cat:"supply", level:"入门", q:"软件供应链安全主要关注的是？", options:["构成软件的依赖、构建与分发链路是否可信","服务器机房温湿度","员工考勤","显示器品牌"], answer:0, explain:"供应链风险分布在依赖、构建工具、制品与分发通道，而非单一节点。" },
  { id:"sc2", topic:"sc-what", cat:"supply", level:"入门", q:"为什么一次供应链攻击能影响大量单位？", options:["被攻击的组件/更新通道被广泛复用，形成一对多的放大效应","因为攻击者数量多","因为网络带宽大","因为用户不看新闻"], answer:0, explain:"公共依赖与更新通道的复用性放大了影响面，效率极高。" },
  { id:"sc3", topic:"sc-sbom", cat:"supply", level:"初级", q:"SBOM 的作用是？", options:["记录软件由哪些组件与版本构成，便于快速判断是否受新漏洞影响","提升编译速度","压缩安装包","加密源代码"], answer:0, explain:"SBOM 是影响面判定的基础；没有它，0day 出现时只能靠猜。" },
  { id:"sc4", topic:"sc-sbom", cat:"supply", level:"初级", q:"SBOM 常见的两种格式是？", options:["SPDX 与 CycloneDX","JSON 与 XML","ZIP 与 TAR","CSV 与 YAML"], answer:0, explain:"SBOM 标准化格式主要是 SPDX 与 CycloneDX，便于工具链互通。" },
  { id:"sc5", topic:"sc-dependency", cat:"supply", level:"初级", q:"依赖漏洞响应中，优先级排序应考虑？", options:["漏洞可利用性、是否可达、是否暴露在公网","依赖包的下载量","依赖包的名字长度","引入时间先后"], answer:0, explain:"按可利用性与实际暴露面排序，才能把有限人力用在真实风险上。" },
  { id:"sc6", topic:"sc-dependency", cat:"supply", level:"初级", q:"锁文件（lockfile）在依赖管理中的作用是？", options:["固定依赖树的具体版本，保证构建可重复并便于审计","加快下载速度","减小仓库体积","自动修复漏洞"], answer:0, explain:"lockfile 锁定传递依赖版本，是构建一致性与依赖审计的前提。" },
  { id:"sc7", topic:"sc-sca", cat:"supply", level:"中级", q:"SCA 工具最常见的落地问题是？", options:["告警过多且无人判定，最终被忽略","扫描速度太慢","不支持中文","占用磁盘过大"], answer:0, explain:"SCA 的价值取决于闭环：有人判定、有 SLA、有修复验证，否则只是噪音。" },
  { id:"sc8", topic:"sc-sca", cat:"supply", level:"中级", q:"把 SCA 接入 CI 时较稳妥的策略是？", options:["先对新增依赖做阻断门禁，存量按 SLA 专项清理","一次性阻断所有历史漏洞","只在发布前手工扫一次","只在本地开发机扫描"], answer:0, explain:"避免因存量债导致流水线瘫痪，同时保证新引入不再恶化。" },
  { id:"sc9", topic:"sc-typosquat", cat:"supply", level:"中级", q:"典型的依赖投毒手法不包括？", options:["给依赖包增加单元测试","抢注废弃包名","仿冒相似包名（typosquatting）","在安装脚本中执行恶意代码"], answer:0, explain:"增加单元测试是正常开发行为；其余三项都是投毒常见手法。" },
  { id:"sc10", topic:"sc-what", cat:"supply", level:"中级", q:"降低依赖投毒风险的有效措施是？", options:["使用私有命名空间与内部代理，禁用或审核安装脚本","只使用下载量最高的包","关闭所有依赖更新","只写自己的代码不用开源"], answer:0, explain:"来源管控 + 脚本管控能显著降低被投毒的概率与影响。" },
  { id:"sc11", topic:"sc-build", cat:"supply", level:"高级", q:"构建链安全中最需要限制的是？", options:["构建机的权限（尤其是发布与签名凭证）","构建机的 CPU 型号","构建机的网络带宽","构建机的机箱颜色"], answer:0, explain:"构建机一旦可被 PR 操纵又能拿到发布凭证，就能产出带后门的正式版本。" },
  { id:"sc12", topic:"sc-concept", cat:"supply", level:"高级", q:"可复现构建（Reproducible Build）的价值是？", options:["相同源码与依赖能产出相同哈希，可验证产物未被动手脚","构建速度更快","安装包更小","支持更多系统"], answer:0, explain:"可复现构建让「产物是否被篡改」可被独立验证，是供应链可信的关键能力。" },
  { id:"sc13", topic:"sc-signing", cat:"supply", level:"高级", q:"制品签名的正确用法是？", options:["发布时签名，部署或使用时强制验证签名","只签名不验证","把私钥放在构建脚本里","签名后允许随意替换制品"], answer:0, explain:"签名与验证必须成对存在；不验证的签名只提供心理安慰。" },
  ...
  { id:"sc15", topic:"sc-sca", cat:"supply", level:"中级", q:"CI/CD 流水线中「表达式注入」的典型成因是？", options:["把外部输入（issue 标题、PR 描述）直接拼进脚本执行","使用 YAML 语法","使用缓存","并行执行任务"], answer:0, explain:"外部输入参与脚本拼接会导致命令注入；应作为环境变量传递并避免拼接。" },
  { id:"sc16", topic:"sc-build", cat:"supply", level:"中级", q:"PR 触发的流水线不应具备的权限是？", options:["读取生产发布/签名凭证","读取仓库源码","运行单元测试","上传测试报告"], answer:0, explain:"PR 来自外部贡献者时风险最高；发布凭证必须与 PR 构建隔离。" },
  { id:"sc17", topic:"sc-secrets", cat:"supply", level:"初级", q:"发现密钥已提交到 Git 仓库，正确处置顺序是？", options:["先轮换/作废密钥，再清理历史与副本，然后查审计日志","先删文件再慢慢换密钥","只删文件即可","把仓库设为私有即可"], answer:0, explain:"密钥一旦提交即视为泄露，必须优先轮换；仅删除文件无法阻止历史与 fork 中的读取。" },
  { id:"sc18", topic:"sc-secrets", cat:"supply", level:"初级", q:"防止密钥进入代码库的有效手段是？", options:["提交前与 CI 中做密钥扫描，并把密钥改为密钥库注入","禁止使用数据库密码","要求开发者手动检查","只在生产环境扫描"], answer:0, explain:"pre-commit + CI 扫描形成双重拦截，密钥库注入消除硬编码。" },
  { id:"sc19", topic:"sc-license", cat:"supply", level:"中级", q:"开源组件引入评审中，最需要法务关注的是？", options:["许可证类型（如 GPL 的传染性）与产品分发方式的兼容性","代码行数","Star 数量","文档语言"], answer:0, explain:"许可证不合规可能导致产品被迫开源或面临法律风险，需在引入前评估。" },
  ...
  { id:"sc21", topic:"sc-vendor", cat:"supply", level:"初级", q:"供应商合同中最应明确的与安全相关的条款是？", options:["数据范围与用途限定、事件通报时限、审计权与退出销毁","付款周期","品牌露出","会议频率"], answer:0, explain:"合同是约束供应商安全行为的主要手段，需覆盖数据、通报、审计与退出。" },
  { id:"sc22", topic:"sc-vendor", cat:"supply", level:"初级", q:"对接触高敏数据的供应商，管理力度应？", options:["更严格：准入评估、定期复核、必要时现场检查","与其他供应商相同","可以放宽以节省成本","只要求口头承诺"], answer:0, explain:"按数据等级分级管理供应商是通行做法，高敏场景需更强约束。" },
  { id:"sc23", topic:"sc-attack", cat:"supply", level:"入门", q:"SolarWinds 事件暴露的核心问题是？", options:["软件更新通道被植入后门，下游客户大规模受影响","机房消防不合格","员工口令太短","备份未加密"], answer:0, explain:"该事件说明更新与分发通道是供应链的高价值攻击点，需要签名与来源验证。" },
  { id:"sc24", topic:"sc-attack", cat:"supply", level:"入门", q:"Log4Shell 事件给我们的主要启示是？", options:["广泛使用的组件漏洞会瞬间波及海量系统，需要 SBOM 与快速影响面判定","应避免使用任何开源","应关闭日志功能","应禁用 Java"], answer:0, explain:"问题不在开源本身，而在于缺乏依赖台账与响应机制导致无法快速判定与修复。" },
  { id:"sc25", topic:"sc-response", cat:"supply", level:"高级", q:"供应链事件响应的首要能力是？", options:["在短时间内判定「哪些制品与在线实例受影响」","先对外发布公告","先起诉供应商","先重装所有服务器"], answer:0, explain:"影响面判定决定止血与沟通节奏；这依赖 SBOM、制品台账与资产清单的日常建设。" },
  { id:"sc26", topic:"sc-what", cat:"supply", level:"高级", q:"检验供应链响应能力的有效方式是？", options:["定期演练：模拟某依赖 0day，测量从情报到判定的耗时","只做文档评审","只在年底总结","等真实事件发生"], answer:0, explain:"演练是唯一能暴露判定链路断点的办法；文档与流程未经演练往往不可用。" },
  ...
  { id:"ai2", topic:"aisec-landscape", cat:"aisec", level:"入门", q:"评估 LLM 应用时，第一条主线应该是？", options:["画出数据流：输入 → 检索 → 模型 → 工具 → 输出","先看模型的参数量","先看 UI 配色","先看部署机器配置"], answer:0, explain:"沿数据流逐段找「可被操纵」与「有副作用」的点，是 LLM 安全评估的基本方法。" },
  { id:"ai3", topic:"aisec-prompt-injection", cat:"aisec", level:"初级", q:"间接提示注入（Indirect Prompt Injection）指？", options:["恶意指令藏在模型会读取的外部内容里（网页/文档/邮件）","用户直接在对话框里骂模型","模型自身出现幻觉","提示词过长被截断"], answer:1, explain:"间接注入把攻击载荷放进被检索或解析的内容，是 LLM 应用最实际、最危险的攻击面。" },
  { id:"ai4", topic:"aisec-agent-tools", cat:"aisec", level:"初级", q:"防御提示注入最有效的方向是？", options:["把不可信内容降权为纯数据，并收敛工具权限与副作用","写更严厉的系统提示词","禁止用户上传文件","把模型换成更大的"], answer:0, explain:"提示词层面的加固可被绕过，结构隔离与权限最小化才可靠；高风险动作还需人工确认。" },
  { id:"ai5", topic:"aisec-data-leak", cat:"aisec", level:"中级", q:"多租户 LLM 应用中，最关键的隔离检查是？", options:["检索是否强制按租户/用户过滤，且过滤条件不由模型决定","是否使用向量数据库","是否使用流式输出","是否做了结果缓存"], answer:0, explain:"检索层若缺少强制过滤，A 用户可检索到 B 用户私有文档，形成跨租户数据泄露。" },
  { id:"ai6", topic:"aisec-jailbreak", cat:"aisec", level:"中级", q:"越狱（Jailbreak）防护的现实目标是？", options:["提高绕过成本并限制后果，而非彻底消除","完全杜绝任何违规输出","禁止用户提问","把温度参数设为 0"], answer:0, explain:"越狱无法根除；有效做法是输入检测 + 输出过滤 + 高风险动作确认与审计，并把拦截率作为指标。" },
  { id:"ai7", topic:"aisec-rag-risks", cat:"aisec", level:"中级", q:"RAG 知识库投毒的风险在于？", options:["恶意内容进入知识库后被检索进上下文，牵引模型输出","向量检索速度下降","知识库占用磁盘","检索结果排序变慢"], answer:0, explain:"知识库对模型是「可信感很强」的信息源，入库审核与权限控制是必要防线。" },
  { id:"ai8", topic:"aisec-rag-risks", cat:"aisec", level:"中级", q:"RAG 回答的引用可追溯性为什么重要？", options:["能核对原文以判断回答依据是否被污染或篡改","可以提升回答速度","可以减少 token 消耗","可以美化界面"], answer:0, explain:"引用溯源让用户能验证答案依据，是抵御知识库投毒与幻觉的重要手段。" },
  { id:"ai9", topic:"aisec-agent-tools", cat:"aisec", level:"高级", q:"Agent 工具调用最大的风险来源是？", options:["模型决定调用什么、参数是什么，而工具可能有副作用与过大权限","工具接口写得不够优雅","调用日志太多","工具返回 JSON 格式"], answer:0, explain:"Agent 的风险集中在「有副作用的动作 + 过大的权限 + 缺少确认」，需按副作用分级管控。" },
  { id:"ai10", topic:"ai-agent-guard", cat:"aisec", level:"高级", q:"对「发邮件」这类外发型工具，合理控制是？", options:["收件人白名单 + 人工确认 + 全量审计","允许任意收件人以提升效率","只在日志里记录","由模型自行判断是否安全"], answer:0, explain:"外发动作不可撤回，必须用白名单与确认限制影响面，并保留审计证据。" },
  { id:"ai11", topic:"aisec-output-handling", cat:"aisec", level:"初级", q:"把模型输出直接插入页面 HTML 会导致？", options:["XSS：模型输出内容被当作代码执行","页面加载变慢","字体渲染异常","SEO 排名下降"], answer:0, explain:"模型输出属于不可信内容，必须按用户输入处理：转义、白名单净化或结构化校验。" },
  { id:"ai12", topic:"aisec-output-handling", cat:"aisec", level:"初级", q:"处理模型输出的正确姿势是？", options:["当不可信输入处理：编码、参数化、结构校验","直接 eval 执行","直接拼进 SQL","直接写入 shell 脚本"], answer:0, explain:"任何「执行」路径都必须经过显式白名单与结构校验，避免二次漏洞。" },
  { id:"ai13", topic:"aisec-model-supply", cat:"aisec", level:"高级", q:"从公共仓库下载模型权重前，最应做的动作是？", options:["校验来源与哈希/签名，优先使用仅数据格式避免反序列化执行","先跑一遍看效果","改个文件名更安全","放到生产目录再验证"], answer:0, explain:"权重文件可能被投毒，pickle 类格式甚至可直接执行代码；校验与安全格式是基本要求。" },
  { id:"ai14", topic:"aisec-agent-tools", cat:"aisec", level:"高级", q:"推理环境的加固建议是？", options:["禁出网、最小挂载、非特权用户运行","以 root 运行方便调试","开放全部网络权限","挂载宿主机全部目录"], answer:0, explain:"推理环境一旦被利用，会成为跳板；隔离与最小权限能限制影响范围。" },
  { id:"ai15", topic:"aisec-privacy", cat:"aisec", level:"初级", q:"把真实客户数据直接贴进外部模型对话，主要风险是？", options:["等同于向第三方提供个人信息，可能违反合规要求","会消耗更多 token","回答质量下降","模型会记住并公开"], answer:0, explain:"提示词往往夹带业务数据，需按数据等级评估可用模型、脱敏或改用私有部署。" },
  { id:"ai16", topic:"aisec-privacy", cat:"aisec", level:"初级", q:"下列哪项属于「提示词数据自查」应覆盖的内容？", options:["是否含个人信息、密钥、内部地址等敏感数据","提示词的字数是否整齐","是否使用了 emoji","段落缩进是否规范"], answer:0, explain:"上线前要审查发送内容的数据等级，并确认供应商的留存与训练条款。" },
  { id:"ai17", topic:"aisec-defense", cat:"aisec", level:"中级", q:"四层护栏通常指？", options:["输入过滤、能力收敛、输出校验、流程确认","加密、备份、审计、监控","防火墙、WAF、IDS、EDR","开发、测试、预发、生产"], answer:0, explain:"输入/能力/输出/流程四层叠加，其中「能力收敛 + 流程确认」对限制后果最有效。" },
  { id:"ai18", topic:"aisec-eval", cat:"aisec", level:"中级", q:"护栏效果应该怎么衡量？", options:["用红队样本集统计拦截率与漏拦率并做回归","统计代码行数","统计拦截次数越多越好","看部署了几个组件"], answer:0, explain:"护栏必须可度量、可回归，否则无法判断改进是否有效，也容易误拦正常业务。" },
  { id:"ai19", topic:"aisec-eval", cat:"aisec", level:"高级", q:"LLM 应用红队评估的产出应包含？", options:["最小复现步骤、实际结果、影响判定与修复位置","仅一份风险清单","仅截图","仅评分"], answer:0, explain:"可复现的证据链才能推动工程修复，并沉淀为回归用例。" },
  { id:"ai20", topic:"aisec-eval", cat:"aisec", level:"高级", q:"为什么要把红队样本集沉淀为回归资产？", options:["改提示词或换模型版本后需要重新验证是否回归","可以提升模型效果","可以减少服务器成本","可以用来训练模型"], answer:0, explain:"提示词与模型频繁变更，只有回归资产能保证防护不会悄悄退化。" },
  { id:"ai21", topic:"aisec-cost-dos", cat:"aisec", level:"中级", q:"LLM 应用的成本型 DoS 通常表现为？", options:["超长输入或循环调用导致账单飙升与服务不可用","磁盘写满","CPU 温度过高","网络丢包"], answer:0, explain:"入口限长限频、工具循环设上限、预算熔断是必要控制。" },
  { id:"ai22", topic:"aisec-agent-tools", cat:"aisec", level:"中级", q:"限制 Agent 无限循环的常见做法是？", options:["设置工具调用轮次上限并对相同调用去重","提高温度参数","增大上下文长度","更换模型"], answer:0, explain:"轮次上限 + 去重能避免 Agent 陷入递归调用，造成成本与副作用失控。" },
  { id:"ai23", topic:"aisec-governance", cat:"aisec", level:"入门", q:"AI 治理中，「提示词与工具配置」应如何管理？", options:["纳入版本管理，变更需评审与留痕","由个人保存在本地文档","随改随用无需记录","只记录最终版本"], answer:0, explain:"提示词与工具权限等价于生产配置，必须版本化、可评审、可回滚。" },
  { id:"ai24", topic:"aisec-governance", cat:"aisec", level:"入门", q:"AI 应用上线变更留痕至少应记录？", options:["变更内容、提交与评审人、回归结果、生效时间与回滚方式","服务器型号","开发者电脑品牌","代码行数"], answer:0, explain:"留痕是为了事后可追溯与快速回滚，是治理的基本要求。" },
  { id:"ai25", topic:"aisec-governance", cat:"aisec", level:"高级", q:"把 LLM 应用纳入企业安全体系的关键是？", options:["接入既有日志、SIEM、应急与变更管理，而非独立小系统","单独采购一个 AI 安全产品即可","只做内部自查","只在出事时上报"], answer:0, explain:"AI 应用的风险最终要在统一的安全运营与治理框架里闭环。" },
  { id:"ai26", topic:"aisec-agent-tools", cat:"aisec", level:"高级", q:"评估 LLM 应用时，「影响面优先」的含义是？", options:["先收紧有副作用的动作（确认+审计+限额），再优化提示与检测","先优化提示词再考虑权限","先做性能优化","先做界面体验"], answer:0, explain:"副作用动作是唯一不可逆的风险，先限制它能让整体风险迅速可控。" },
  { id:"ct1", topic:"soc-basics", cat:"soccti", level:"入门", q:"威胁情报的分层中，给管理层看的是？", options:["战略情报（趋势与风险）","战术情报（IOC 列表）","原始日志","漏洞扫描报告"], answer:0, explain:"战略情报服务决策与资源投入；战术情报给检测设备，运营情报给日常安全运营。" },
  { id:"ct2", topic:"soc-basics", cat:"soccti", level:"入门", q:"威胁情报「没人用」的最常见原因是？", options:["层级与消费方错配，或缺少可执行落地","情报数量太少","情报格式太新","情报太贵"], answer:0, explain:"先明确谁用它做什么决策，再决定收集什么，否则收藏即终点。" },
  { id:"ct3", topic:"soc-ioc-ioa", cat:"soccti", level:"初级", q:"IOC 与 TTP 相比，主要短板是？", options:["易过期且易被规避（换域名/哈希即可）","无法自动化匹配","不能写进规则","只能用于邮件"], answer:0, explain:"IOC 见效快但时效短；TTP 与行为规则更难被绕过，价值更持久。" },
  { id:"ct4", topic:"soc-ioc-ioa", cat:"soccti", level:"初级", q:"IOC 落地前必须评估？", options:["误伤风险（共享 IP、CDN、云服务）与有效期","文件大小","颜色编码","订阅价格"], answer:0, explain:"不做误伤评估直接封禁，容易造成业务中断；同时要管理失效与下架。" },
  { id:"ct5", topic:"soc-soc-ops", cat:"soccti", level:"初级", q:"SOC 一线值班最需要的支持是？", options:["清晰的告警分级、剧本与升级路径","更多仪表盘","更快的机器","更多告警来源"], answer:0, explain:"流程与剧本决定处置效率；告警越多而流程不清只会加剧积压。" },
  { id:"ct6", topic:"soc-soc-ops", cat:"soccti", level:"初级", q:"值班交接中必须包含的内容是？", options:["未闭环告警状态与临时抑制规则及其到期时间","个人作息安排","设备采购计划","年度预算"], answer:0, explain:"未闭环事项与临时抑制最容易在交接中丢失，导致漏处理或长期误抑制。" },
  { id:"ct7", topic:"soc-detection", cat:"soccti", level:"中级", q:"检测规则上线前的必要动作是？", options:["用历史数据回归：确认该报的报、不该报的不报","直接上线观察","仅评审语法","仅统计规则条数"], answer:0, explain:"未回归的规则上线即噪音；回归数据是规则质量的基本保障。" },
  { id:"ct8", topic:"soc-detection", cat:"soccti", level:"中级", q:"衡量检测覆盖度时，容易出现的偏差是？", options:["只看「有没有规则」，忽视是否真能检出变种","统计规则行数太少","使用 ATT&CK 编号","按日志源分类"], answer:0, explain:"规则存在不等于有效；需用真实样本与演练结果验证覆盖是否真实。" },
  { id:"ct9", topic:"soc-triage", cat:"soccti", level:"中级", q:"告警研判的四个核心问题不包括？", options:["告警的颜色是否醒目","资产是否关键","行为是否已知良性","是否有成功迹象"], answer:0, explain:"研判关注资产重要性、行为性质与证据，与视觉呈现无关。" },
  { id:"ct10", topic:"soc-detection", cat:"soccti", level:"中级", q:"降低误报最有效的前置动作是？", options:["与变更系统、扫描器、备份等已知行为联动做白名单","直接关闭噪音规则","提高阈值到不再报警","只保留高危规则"], answer:0, explain:"多数误报来自已知的合法行为；把变更与工具行为纳入上下文可显著降噪。" },
  { id:"ct11", topic:"soc-hunting", cat:"soccti", level:"高级", q:"威胁狩猎的起点通常是？", options:["基于 ATT&CK 或情报提出假设，再去找能验证的数据","等告警触发","随机浏览日志","购买新设备"], answer:0, explain:"狩猎是假设驱动的主动搜索；数据缺口本身就是重要结论。" },
  { id:"ct12", topic:"soc-hunting", cat:"soccti", level:"高级", q:"狩猎没有发现任何异常，应该怎么处理？", options:["记录结论与数据缺口，补齐日志与检测项","认为白做了没有价值","停止后续狩猎","删除相关查询"], answer:0, explain:"「未发现」也是结论，常暴露日志缺失；补数据能提升后续检测与狩猎能力。" },
  { id:"ct13", topic:"soc-ir-flow", cat:"soccti", level:"中级", q:"应急响应六阶段的正确顺序是？", options:["准备 → 识别 → 遏制 → 根除 → 恢复 → 复盘","识别 → 恢复 → 遏制 → 复盘 → 根除 → 准备","遏制 → 识别 → 复盘 → 根除 → 恢复 → 准备","准备 → 遏制 → 识别 → 根除 → 复盘 → 恢复"], answer:0, explain:"先识别再遏制，根除后恢复，最后复盘沉淀；顺序错乱会导致证据丢失或攻击反复。" },
  { id:"ct14", topic:"soc-baseline", cat:"soccti", level:"中级", q:"应急中的「遏制」需要平衡的两件事是？", options:["快速止血与保留证据（内存与日志）","成本与性能","界面与体验","人员排班与休假"], answer:0, explain:"直接关机或重装会毁掉证据；正确做法是网络隔离并先做内存/日志固定。" },
  { id:"ct15", topic:"soc-forensics", cat:"soccti", level:"高级", q:"取证工作中「证据完整性」的核心要求是？", options:["先固定再做分析，并记录取证链与哈希","尽快分析尽快结案","只保留结论","在原始系统上直接操作"], answer:0, explain:"证据必须可追溯、可复现，否则结论无法采信，也可能被反取证手段破坏。" },
  { id:"ct16", topic:"soc-net-forensics", cat:"soccti", level:"高级", q:"面对日志被清理的情况，可行的验证思路是？", options:["多源交叉（EDR/流量/认证/云审计）与集中化日志留存","相信单一日志结论","直接结案","只问当事人"], answer:0, explain:"单源被破坏时需靠多源交叉；集中化与只追加存储能显著提高抗销毁能力。" },
  { id:"ct17", topic:"soc-mitre", cat:"soccti", level:"初级", q:"ATT&CK 最有价值的用法是？", options:["对齐检测覆盖、情报与复盘语言，找出检测缺口","替代防火墙","自动处置告警","生成漏洞报告"], answer:0, explain:"ATT&CK 是共同语言：用技术编号串联情报、规则、演练与复盘。" },
  { id:"ct18", topic:"soc-mitre", cat:"soccti", level:"初级", q:"ATT&CK 覆盖矩阵中，优先补齐的组合是？", options:["有日志但无规则、以及关键技术无日志","已有规则的技术再多写几条","把编号抄全","美化图表"], answer:0, explain:"优先补「差一点就能检出」和「根本没数据」的缺口，收益最大。" },
  { id:"ct19", topic:"soc-platform", cat:"soccti", level:"中级", q:"情报订阅源管理最关键的两个维度是？", options:["可信度与可执行性（能否落到规则或处置）","价格与界面","数量与格式","更新频率与体积"], answer:0, explain:"来源可信度决定是否可依赖，可执行性决定能否产生价值；其余为次要维度。" },
  { id:"ct20", topic:"soc-alert", cat:"soccti", level:"中级", q:"情报富化（Enrichment）的典型用途是？", options:["在告警里自动补上资产、情报与历史行为，帮助快速研判","提升告警数量","减少日志存储","加快扫描速度"], answer:0, explain:"富化让一线在最短时间拿到决策所需上下文，是 SOC 提效的关键动作。" },
  { id:"ct21", topic:"soc-metrics", cat:"soccti", level:"中级", q:"下列哪个指标最容易被误用？", options:["关闭告警的数量（关闭多不代表防得好）","MTTD","MTTR","告警准确率"], answer:0, explain:"关闭数量可通过盲目关闭刷高，属于典型的「有害指标」；应关注准确率与漏检复盘。" },
  { id:"ct22", topic:"soc-detection", cat:"soccti", level:"中级", q:"漏检事件（该报未报）发生后最重要的动作是？", options:["转化为新规则或补日志，并纳入回归验证","追究值班人员责任即可","删除相关数据","更换 SIEM 产品"], answer:0, explain:"漏检的价值在于补齐检测能力；仅追责不改进会让同类事件重复发生。" },
  { id:"ct23", topic:"soc-attribution", cat:"soccti", level:"高级", q:"归因结论的正确使用方式是？", options:["标注置信度与依据，并明确它对防御决策的影响","作为唯一行动依据","对外公开确定结论","据此直接反击"], answer:0, explain:"归因是低置信推断，存在假旗与工具外流导致的误判；应服务于加固与监控决策。" },
  { id:"ct24", topic:"soc-attribution", cat:"soccti", level:"高级", q:"归因中需要警惕的干扰因素是？", options:["对手故意留下的假线索（假旗）与工具买卖导致的特征重叠","情报订阅过多","日志格式不统一","人员轮班"], answer:0, explain:"基础设施与工具会被共享或出售，单靠特征重叠容易误判。" },
  { id:"ct25", topic:"soc-baseline", cat:"soccti", level:"入门", q:"建设检测能力最先要解决的问题是？", options:["关键日志有没有采集、是否完整、保留多久","先买最贵的 SIEM","先招更多人","先做可视化大屏"], answer:0, explain:"没有日志就没有检测与溯源；日志覆盖与保留期决定能力上限。" },
  { id:"ct26", topic:"soc-baseline", cat:"soccti", level:"入门", q:"下列最应优先接入的日志源是？", options:["认证/域控与边界设备、关键服务器","打印机耗材状态","会议室预定系统","员工餐厅消费记录"], answer:0, explain:"优先接高价值安全事件源，能最快形成有效检测能力。" },

  // ---- 入门档补充（v1.5.1 第五批）----
  { id:"wb1", topic:"web-http-flow", cat:"web", level:"入门", q:"浏览器向服务器发一次请求，响应里用哪个头告诉浏览器要设置会话 Cookie？", options:["Set-Cookie","Content-Type","User-Agent","Referer"], answer:0, explain:"Set-Cookie 用于下发 Cookie；HttpOnly/Secure/SameSite 决定它的安全属性。" },
  { id:"wb2", topic:"web-owasp10", cat:"web", level:"入门", q:"OWASP Top 10 的正确用途是？", options:["作为常见风险检查清单，提示评估方向","替代所有安全测试","证明系统已安全","替代威胁建模"], answer:0, explain:"Top 10 是高频风险的清单，利于沟通与查漏，但不能替代系统性测试与建模。" },
  { id:"bb1", topic:"stack", cat:"binary", level:"入门", q:"栈溢出漏洞之所以危险，关键原因是栈上存放着？", options:["函数返回地址，被覆盖可劫持控制流","图片资源","网络配置","日志文件"], answer:0, explain:"覆盖返回地址即可改变程序执行路径；现代系统靠 ASLR/NX/Canary 提高利用难度。" },
  { id:"bb2", topic:"bin-tools", cat:"binary", level:"入门", q:"逆向分析时，快速定位关键逻辑的常用起点是？", options:["用 strings 找提示语/错误信息，再查交叉引用","直接读二进制机器码","先跑压力测试","先做网络扫描"], answer:0, explain:"字符串是定位函数的最快线索，配合交叉引用能迅速缩小分析范围。" },
  { id:"cb1", topic:"sym", cat:"crypto", level:"入门", q:"TLS 这类实际协议通常如何组合加密算法？", options:["用非对称算法协商会话密钥，再用对称算法加密数据","全程只用非对称加密","全程只用对称加密","不加密，只做签名"], answer:0, explain:"非对称解决密钥分发，对称保证性能，二者配合是主流方案。" },
  { id:"cb2", topic:"crypto-hash-store", cat:"crypto", level:"入门", q:"存储用户口令的正确做法是？", options:["使用 bcrypt/argon2 等慢哈希并加盐","MD5 单次哈希","AES 加密后存库","Base64 编码后存库"], answer:0, explain:"慢哈希 + 每用户盐显著提高离线破解成本；可逆加密与编码都不算保护。" },
  { id:"pb1", topic:"pt-process", cat:"pentest", level:"入门", q:"渗透测试开始前最重要的一件事是？", options:["取得书面授权并明确范围与禁止动作","准备好扫描器","找好代理 IP","先跑一遍自动化工具"], answer:0, explain:"没有授权的测试就是攻击；范围、时间窗与禁止动作必须在动手前确认。" },
  { id:"pb2", topic:"recon", cat:"pentest", level:"入门", q:"信息收集阶段之所以优先做被动收集，是因为？", options:["不接触目标，降低被发现与封禁风险","被动收集更准确","主动收集违法","被动收集不用授权"], answer:0, explain:"被动收集（公开信息、证书日志）不留痕迹，适合先摸清资产范围。" },
  { id:"nb1", topic:"net-lateral", cat:"network", level:"入门", q:"VLAN 在网络中的作用是？", options:["做网络隔离，限制广播域与横向访问","提高带宽","加密流量","替代防火墙"], answer:0, explain:"VLAN 用于分区管理；划分不当会让内网横向移动变得更容易。" },
  { id:"nb2", topic:"net-packet", cat:"network", level:"入门", q:"用 Wireshark 判断某协议是否明文传输，最直接的方法是？", options:["跟随 TCP 流查看内容，看是否可读","看包的数量","看端口号是否常见","看抓包文件大小"], answer:0, explain:"Follow TCP/HTTP Stream 能直接看到载荷内容，明文则账号口令可见。" },
  { id:"cl1", topic:"cloud-storage-sec", cat:"cloud", level:"入门", q:"云上「对象存储」的典型安全风险是？", options:["存储桶被配置为公开可读写","磁盘容量不足","网络延迟升高","镜像构建变慢"], answer:0, explain:"公开桶是云上最常见的数据泄露原因之一，需做配置巡检。" },
  { id:"cl2", topic:"cloud-account", cat:"cloud", level:"入门", q:"云账号保护的第一优先级动作是？", options:["主/根账号开启 MFA 且不日常使用","先买更多实例","先建更多子账号","先开启 CDN"], answer:0, explain:"主账号权限最大，一旦失守影响全局；MFA + 不日常使用是底线要求。" },
  { id:"bl1", topic:"blue-role", cat:"blue", level:"入门", q:"蓝队工作的核心目标是？", options:["缩短发现与响应时间，并持续改进检测能力","比红队更早拿到旗标","采购更多安全设备","减少安全预算"], answer:0, explain:"蓝队价值体现在 MTTD/MTTR 与检测覆盖的持续提升，而非设备数量。" },
  { id:"bl2", topic:"blue-log", cat:"blue", level:"入门", q:"分析登录日志时，判断暴力破解最关键的线索是？", options:["同一来源多次失败后出现成功登录","登录时间在白天","用户使用了浏览器","请求来自内网"], answer:0, explain:"失败堆积后成功意味着可能已猜中口令，需立即核查与处置。" },
  { id:"mb1", topic:"mob-anatomy", cat:"mobile", level:"入门", q:"Android APK 中声明权限与对外组件的位置是？", options:["AndroidManifest.xml","build.gradle","strings.xml","图标资源目录"], answer:0, explain:"配置清单是移动安全评估的第一站：权限与 exported 组件都在这里声明。" },
  { id:"mb2", topic:"mob-permission", cat:"mobile", level:"入门", q:"判断 App 是否过度收集信息，最直接的依据是？", options:["申请的权限与实际功能是否匹配","App 的安装包大小","App 的更新频率","App 的评分高低"], answer:0, explain:"权限与功能不匹配是过度收集的典型信号，还要结合隐私政策与实际行为核查。" },
  { id:"db1", topic:"ds-pii", cat:"datasec", level:"入门", q:"下列属于「敏感个人信息」的是？", options:["人脸信息与医疗记录","公司公开宣传册","产品说明书","开源许可证文本"], answer:0, explain:"生物特征、医疗、金融、行踪等一旦泄露危害更大，属敏感个人信息，需更强保护。" },
  { id:"db2", topic:"ds-leak-scene", cat:"datasec", level:"入门", q:"现实中数据泄露最常见的成因是？", options:["配置错误与权限过大，而非被高级攻击者攻破","黑客技术太强","硬件故障","备份太多"], answer:0, explain:"公开桶、无口令共享链接、离职未收权限这类「自己放出去」的情况占多数。" },
  { id:"sb1", topic:"sc-what", cat:"supply", level:"入门", q:"软件供应链安全要回答的第一个问题是？", options:["我的系统由哪些组件与版本构成","我的服务器性能如何","我的界面是否美观","我的代码行数多少"], answer:0, explain:"先有组成清单（SBOM/依赖清单），才谈得上漏洞响应与影响判定。" },
  { id:"sb2", topic:"sc-what", cat:"supply", level:"入门", q:"在开源组件引入环节，较稳妥的做法是？", options:["通过内部代理仓库引入、锁定版本并对新增依赖评审","直接连公网仓库随意安装","只用下载量最高的包","禁止使用任何开源"], answer:0, explain:"来源可控 + 版本锁定 + 引入评审，能把风险挡在门口。" },
  { id:"ab1", topic:"ai-how", cat:"aisec", level:"入门", q:"大模型「幻觉」指的是？", options:["生成了听起来合理但并不正确的内容","模型运行变慢","提示词被截断","网络连接失败"], answer:0, explain:"模型按概率生成文本，事实性内容必须核实，关键判定应放在外部校验。" },
  { id:"ab2", topic:"ai-usage", cat:"aisec", level:"入门", q:"使用 AI 工具时，最基本的安全习惯是？", options:["不贴敏感数据、核实关键结论、限制其操作权限","尽量多贴数据以提升效果","完全信任输出","让它自动执行所有操作"], answer:0, explain:"数据最小化、结果核实、权限收敛是 AI 使用的三条基本纪律。" },
  { id:"sb3", topic:"soc-detection", cat:"soccti", level:"入门", q:"安全团队中「检测工程」主要负责？", options:["把日志与情报转成可用的检测规则并维护质量","采购设备","写合规报告","管理门禁"], answer:0, explain:"检测工程负责规则编写、回归与覆盖度管理，是发现能力的核心岗位。" },
  { id:"sb4", topic:"soc-alert", cat:"soccti", level:"入门", q:"收到告警后，判断是否需要升级的关键组合是？", options:["高权限账号 + 核心资产 + 有成功迹象","告警颜色 + 时间 + 来源端口","告警数量 + 设备型号 + 部门","处理人 + 值班表 + 天气"], answer:0, explain:"三者叠加意味着潜在影响最大，应优先升级处置。" },
  { id:"wb3", topic:"web-http-flow", cat:"web", level:"入门", q:"Cookie 的 HttpOnly 属性主要作用是？", options:["禁止 JavaScript 读取该 Cookie，降低 XSS 窃取会话的风险","加密 Cookie 内容","让 Cookie 永不过期","限制 Cookie 体积"], answer:0, explain:"HttpOnly 让脚本读不到会话 Cookie，是 XSS 的纵深防御手段之一。" },
  { id:"wb4", topic:"web-owasp10", cat:"web", level:"入门", q:"判断一个 Web 功能是否存在越权，最直接的做法是？", options:["替换参数中的 id/编号，看能否访问他人数据","观察页面配色","测量响应时间","检查是否有验证码"], answer:0, explain:"这类以对象编号直接引用资源的功能是越权高发区，需验证服务端是否校验数据归属。" },
  { id:"bb3", topic:"rop", cat:"binary", level:"入门", q:"NX（栈不可执行）保护的目的是？", options:["阻止在栈上执行注入的代码，迫使攻击者转向 ROP 等手段","加密二进制文件","压缩程序体积","加快启动速度"], answer:0, explain:"NX 提高利用门槛但不等同安全，需与其他缓解机制组合使用。" },
  { id:"bb4", topic:"bin-tools", cat:"binary", level:"入门", q:"快速查看二进制文件中的可读字符串，常用命令是？", options:["strings","top","ping","df"], answer:0, explain:"strings 是最快的摸底手段，常能直接暴露提示语、路径与内嵌地址。" },
  { id:"cb3", topic:"sym", cat:"crypto", level:"入门", q:"对称加密使用 ECB 模式的核心问题是？", options:["相同明文块生成相同密文块，泄露数据结构","速度太慢","密钥太长","无法处理中文"], answer:0, explain:"ECB 缺乏随机化，经典例子是加密后的图片仍能看出轮廓；应改用带 IV 的模式或 AEAD。" },
  { id:"cb4", topic:"crypto-hash-store", cat:"crypto", level:"入门", q:"「哈希不是加密」的正确含义是？", options:["哈希是单向的，无法通过哈希还原原文","哈希比加密更安全","哈希需要密钥","哈希可以被解密"], answer:0, explain:"正因单向，它适合做完整性校验与口令存储，但不适合需要还原原文的场景。" },
  { id:"pb3", topic:"report", cat:"pentest", level:"入门", q:"一份渗透测试报告中，最有价值的部分通常是？", options:["可复现的复现步骤与可落地的修复建议","测试人员数量","工具截图的多少","测试耗时长度"], answer:0, explain:"报告的价值在于让开发能复现并修复，而不是证明测试做过。" },
  { id:"pb4", topic:"pt-process", cat:"pentest", level:"入门", q:"测试中意外发现范围外的严重漏洞时，正确做法是？", options:["立即通过约定渠道通报，不擅自扩大测试范围","继续深入测试并利用","直接公开披露","当作没看到"], answer:0, explain:"越界测试同样构成未授权访问；发现问题应第一时间通报，由授权方决定后续动作。" },
  { id:"nb3", topic:"net-devices", cat:"network", level:"入门", q:"防火墙策略检查中最常见的配置问题是？", options:["放通规则过宽（如 any-any）或临时规则长期未回收","规则条数太少","设备品牌不统一","日志量太大"], answer:0, explain:"过宽与遗留的临时规则会抹掉分区隔离的效果，应定期复核并设置到期时间。" },
  { id:"nb4", topic:"net-packet", cat:"network", level:"入门", q:"抓包时能看到明文内容，说明？", options:["该流量未加密，凭据与敏感数据可能被直接读取","该协议更安全","抓包工具出错","网络带宽不足"], answer:0, explain:"明文协议在链路上可被读取，应改用加密协议或隧道。" },
  { id:"cl3", topic:"cloud-network", cat:"cloud", level:"入门", q:"云上「安全组」的作用是？", options:["控制实例可被哪些来源访问哪些端口","自动备份数据","加密云盘","统计费用"], answer:0, explain:"安全组是最基础的网络访问控制，配置过宽（如 0.0.0.0/0 放通管理端口）是常见风险。" },
  { id:"cl4", topic:"iam", cat:"cloud", level:"入门", q:"云账单突然大幅上升，安全上应优先排查？", options:["访问凭证是否泄露并被用于挖矿或资源滥用","网络是否波动","备份策略是否合理","实例规格是否够用"], answer:0, explain:"凭证泄露后的挖矿与资源滥用是账单暴涨的典型原因，应结合审计日志核查异常调用。" },
  { id:"bl3", topic:"blue-role", cat:"blue", level:"入门", q:"蓝队指标 MTTD 指的是？", options:["从事件发生到被发现所需的时间","从发现到处置完成的时间","每月告警总数","设备在线率"], answer:0, explain:"MTTD 衡量发现能力，MTTR 衡量处置效率，两者是蓝队最核心的两个指标。" },
  { id:"bl4", topic:"siem", cat:"blue", level:"入门", q:"日志集中收集的核心安全价值是？", options:["攻击者清理单台主机日志后仍可追溯","节省磁盘空间","提升查询速度","减少网络带宽"], answer:0, explain:"集中化与只追加存储能显著提高抗销毁能力，是取证与溯源的基础。" },
  { id:"mb3", topic:"ios-basics", cat:"mobile", level:"入门", q:"iOS 应用（IPA）解包后，配置信息主要位于？", options:["Info.plist","AndroidManifest.xml","build.gradle","strings.xml"], answer:0, explain:"iOS 侧的 ATS 例外、URL Scheme 等关键配置都在 Info.plist 中。" },
  { id:"mb4", topic:"mob-permission", cat:"mobile", level:"入门", q:"无障碍服务权限被恶意应用滥用的典型后果是？", options:["可读取屏幕内容并模拟点击，窃取信息或自动执行操作","仅影响电池续航","仅影响网络速度","没有任何实际影响"], answer:0, explain:"无障碍服务权限极强，是移动恶意软件窃取验证码与实施自动操作的核心手段。" },
  { id:"db3", topic:"ds-mask", cat:"datasec", level:"入门", q:"「准标识符组合」带来的隐私风险是？", options:["多个非唯一字段组合后可能重新识别到具体个人","提升数据库性能","减少存储占用","导致加密失败"], answer:0, explain:"生日、性别、邮编等组合常足以锁定个人，因此脱敏后仍需做重标识风险评估。" },
  { id:"db4", topic:"ds-leak-scene", cat:"datasec", level:"入门", q:"发现共享链接为「任何人可访问」且无有效期，正确的处置是？", options:["立即收紧为需口令或指定范围，并核查是否已被异常访问","无需处理","只修改文件标题","仅通知管理员备案"], answer:0, explain:"无口令无期限的共享链接是数据泄露的高频入口，应先收紧再核查访问记录。" },
  { id:"sb5", topic:"sc-sbom", cat:"supply", level:"入门", q:"要判断「线上是否受某依赖漏洞影响」，最依赖的前置能力是？", options:["依赖清单/SBOM 与资产台账","服务器的数量","防火墙的品牌","团队人数"], answer:0, explain:"没有组成清单与资产清单，影响面判定只能靠猜，响应速度会大幅下降。" },
  { id:"sb6", topic:"sc-concept", cat:"supply", level:"入门", q:"软件供应链四问中「谁能在其中插入东西」指向的是？", options:["代码仓库、构建与发布链路的权限管控","代码行数多少","使用的编程语言","部署在哪个机房"], answer:0, explain:"关注权限：谁有写权限、谁有发布权限、谁能读到签名密钥，决定了被污染的可能性。" },
  { id:"ab3", topic:"aisec-prompt-injection", cat:"aisec", level:"入门", q:"为什么说「上下文里给什么，模型就更容易照着走」？", options:["模型把系统提示、用户输入与外部内容都当作指令通道处理","模型具有长期记忆","模型会自动联网检索","模型会自我训练"], answer:0, explain:"这是提示注入能够成立的根本原因，也是「把外部内容降权为数据」这条防御原则的由来。" },
  ...
  { id:"ct27", topic:"soc-soc-ops", cat:"soccti", level:"入门", q:"出现「告警没人认领」的组织性根因通常是？", options:["缺乏明确的告警归属与升级路径","安全设备数量太少","人员数量太多","网络带宽不足"], answer:0, explain:"职责不清会让告警在交接中流失，需明确归属与升级规则并写入值班流程。" },
  { id:"ct28", topic:"soc-baseline", cat:"soccti", level:"入门", q:"日常安全运营中，「看得见」的前提条件是？", options:["关键系统与账号的日志已完整采集并集中留存","购买了大屏可视化工具","配置了更多告警规则","增加了值班人数"], answer:0, explain:"日志覆盖与保留是检测与溯源的地基，没有日志就只能靠猜。" },

  // ---- 领域补齐（v1.5.1 第六批）----
  { id:"nw1", topic:"net-nat", cat:"network", level:"初级", q:"把内网数据库端口直接映射到公网，主要风险是？", options:["暴露面扩大，可被直接爆破或利用已知漏洞","提升访问速度","增加带宽消耗","影响 DNS 解析"], answer:0, explain:"管理面与数据库不应直接暴露公网；应走私网、跳板或访问代理。" },
  { id:"nw2", topic:"net-nat", cat:"network", level:"初级", q:"NAT 在 IPv6 环境下失效带来的安全影响是？", options:["内网地址可直接被访问，遮蔽作用消失，暴露面管理更需显式","IPv6 更慢","IPv6 无法加密","IPv6 不支持防火墙"], answer:0, explain:"NAT 过去提供了一层「隐性遮蔽」，IPv6 下必须靠防火墙策略与显式最小放通。" },
  { id:"nw3", topic:"net-wifi", cat:"network", level:"中级", q:"企业无线安全的推荐做法是？", options:["使用 WPA2/3 企业级认证（802.1X）并把访客网与内网隔离","使用共享口令方便管理","开启 WPS 便于连接","让员工手机热点接入内网"], answer:0, explain:"企业级认证可追溯到人，访客隔离能避免「连上 Wi-Fi 就等于进内网」。" },
  ...
  { id:"nw5", topic:"net-tunnel", cat:"network", level:"高级", q:"DNS 隧道被用于隐蔽外联时，典型特征是？", options:["大量异常 DNS 查询与超长/编码子域","DNS 服务器宕机","TTL 值过小","使用了 IPv6"], answer:0, explain:"DNS 隧道把数据编码进域名，靠高频与异常长度可被识别，需结合基线检测。" },
  { id:"nw6", topic:"net-tunnel", cat:"network", level:"高级", q:"判定长连接是「正常业务」还是「隐蔽通道」，关键看？", options:["目的地是否在资产与业务白名单内、数据量是否异常","连接是否加密","端口号是否常见","是否使用 TCP"], answer:0, explain:"端口与加密都可以伪装，目的地可解释性与数据量特征更可靠。" },
  { id:"nw7", topic:"net-monitor", cat:"network", level:"中级", q:"相比全流量采集，使用 NetFlow 元数据的优势是？", options:["轻量、可长期保留，适合做基线与异常发现","可以还原完整载荷内容","能解密 TLS","不需要存储"], answer:0, explain:"元数据适合长期基线与趋势分析；需要内容回溯时再启用全流量。" },
  { id:"nw8", topic:"net-monitor", cat:"network", level:"中级", q:"加密流量普及后，网络侧检测的重点转向？", options:["元数据与指纹（JA3/JA4、证书特征、行为模式）","解密所有流量内容","只看端口号","只看包大小"], answer:0, explain:"内容不可读时，指纹与行为特征成为主要判据，并需结合情报与资产上下文。" },
  { id:"cw1", topic:"cloud-storage-sec", cat:"cloud", level:"初级", q:"云上对象存储最常见的数据泄露原因是？", options:["存储桶被设为公开可读，或策略允许任意人访问","磁盘未格式化","实例规格过小","未开启 CDN"], answer:0, explain:"公开桶是云上大规模泄露的经典原因，应默认阻止公开并定期巡检。" },
  { id:"cw2", topic:"iam", cat:"cloud", level:"初级", q:"对外分享云存储文件时，更安全的做法是？", options:["生成带有效期的签名链接而非设置公开策略","直接设为公开","把链接发到群里","复制到个人网盘"], answer:0, explain:"签名链接有时效与权限约束，避免文件长期暴露在公网。" },
  { id:"cw3", topic:"cloud-network", cat:"cloud", level:"中级", q:"安全组规则的常见高危配置是？", options:["0.0.0.0/0 放通远程管理与数据库端口","只放通 443","按源 IP 段放通","默认拒绝全部"], answer:0, explain:"管理面与数据库应仅从受控来源可达，配合跳板与私网访问。" },
  { id:"cw4", topic:"cloud-network", cat:"cloud", level:"中级", q:"云上「最小放通」原则的落地方式是？", options:["默认拒绝、只放通必要端口并限定来源范围","先全放通再慢慢收","只放通内网全部端口","按端口范围随意放通"], answer:0, explain:"先收紧再按需放通，并能说出每条规则对应的业务用途。" },
  { id:"cw5", topic:"cloud-cicd", cat:"cloud", level:"高级", q:"云上流水线使用长期密钥的主要风险是？", options:["密钥长期有效且权限常过大，泄露后可长期滥用云资源","构建速度变慢","镜像体积变大","日志量增加"], answer:0, explain:"推荐用 OIDC 换取短期凭证，并按最小权限设计部署角色。" },
  { id:"cw6", topic:"cloud-cicd", cat:"cloud", level:"高级", q:"云上部署角色的权限设计应遵循？", options:["最小权限：只允许部署所需的具体 API 与资源范围","与管理员同权限，方便排错","只限制网络访问即可","由团队成员自行决定"], answer:0, explain:"部署角色是流水线被利用后的直接后果面，必须按最小权限严格限定。" },
  { id:"cw7", topic:"cloud-monitor", cat:"cloud", level:"中级", q:"云上必设的高危告警不包括？", options:["服务器 CPU 使用率超过 50%","关闭或修改审计日志配置","创建管理员权限","网络规则放通管理端口给全网"], answer:0, explain:"CPU 告警属于运维监控；安全告警要盯权限、日志与网络策略的变更。" },
  { id:"cw8", topic:"cloud-monitor", cat:"cloud", level:"中级", q:"攻击者拿到云凭证后常做的第一件事是？", options:["尝试关闭或规避审计日志与告警","先备份数据","先升级实例规格","先通知管理员"], answer:0, explain:"因此日志需集中到独立账号且只追加，业务账号无权删除，才能保证事后可追溯。" },
  { id:"bw1", topic:"blue-tools", cat:"blue", level:"初级", q:"评估安全工具链是否「打通」的关键标准是？", options:["一条告警里能否看到资产、账号与历史行为并能直接处置","工具数量是否够多","界面是否好看","价格是否够高"], answer:0, explain:"数据与动作打通才减少人肉搬运，处置效率取决于此。" },
  { id:"bw2", topic:"siem", cat:"blue", level:"初级", q:"安全运营工具链中，工单系统的主要作用是？", options:["跟踪处置过程、留存结论并支持复盘","替代 SIEM 做日志分析","阻断攻击流量","管理资产采购"], answer:0, explain:"工单保证闭环与可追溯，也是后续优化规则的事实依据。" },
  { id:"bw3", topic:"blue-playbook", cat:"blue", level:"中级", q:"编写处置剧本时，必须明确的是？", options:["触发条件、所需数据、具体步骤、升级条件与联系人","脚本的编程语言","工具的采购渠道","团队的组织架构"], answer:0, explain:"剧本要在压力下可直接执行，因此需要具体动作与升级边界。" },
  { id:"bw4", topic:"blue-automation", cat:"blue", level:"中级", q:"剧本中最适合保留人工确认的部分是？", options:["有副作用的动作（隔离主机、封禁账号、删除邮件）","查询类动作","日志检索","生成报告"], answer:0, explain:"查询可自动化，有副作用的动作需人工确认以避免影响业务。" },
  { id:"bw5", topic:"blue-hunt-basics", cat:"blue", level:"中级", q:"威胁狩猎与「等告警」的本质区别是？", options:["狩猎由假设驱动，主动去找证据","狩猎只在下班后做","狩猎不需要数据","狩猎只用自动化工具"], answer:0, explain:"狩猎是假设驱动的主动搜索，与检测规则的被动触发互补。" },
  { id:"bw6", topic:"blue-hunt-basics", cat:"blue", level:"中级", q:"狩猎后发现「没有任何日志可查」，正确处理是？", options:["记录数据缺口并补日志/遥测，说明该路径当前不可见","结束狩猎并删除查询","认为假设不成立","改成监控 CPU"], answer:0, explain:"「看不见」本身就是结论，补数据是提升检测能力的直接收益。" },
  { id:"bw7", topic:"blue-purple", cat:"blue", level:"高级", q:"紫队演练的核心价值是？", options:["当场验证检测是否有效，并推动立即改进","评出红蓝双方胜负","产生一份报告","招募更多人员"], answer:0, explain:"紫队把攻击动作与检测能力对齐，重点在「能不能看见」而非输赢。" },
  { id:"bw8", topic:"blue-purple", cat:"blue", level:"高级", q:"紫队演练结果最应沉淀成什么？", options:["可重复的模拟动作库与检测规则回归用例","演练照片","参会名单","预算表"], answer:0, explain:"可重复才能持续回归，防止规则或模型变更后检测能力悄悄退化。" },
  { id:"cw9", topic:"pki", cat:"crypto", level:"中级", q:"证书信任链校验通常包括？", options:["签名有效、域名匹配、未过期、未被吊销","只检查是否过期","只检查域名","只检查是否为自签"], answer:0, explain:"任一项缺失都会导致信任判断错误，需完整校验。" },
  { id:"cw10", topic:"tls", cat:"crypto", level:"中级", q:"企业内代理抓包之所以可行，原理是？", options:["把自签根证书装入客户端信任库，从而签发被信任的中间证书","破解了 TLS 算法","关闭了客户端校验证书","使用了更强加密"], answer:0, explain:"因此「谁能安装根证书」是终端管控的重要边界。" },
  { id:"cw11", topic:"crypto-sign", cat:"crypto", level:"初级", q:"数字签名主要提供？", options:["完整性与来源证明，不提供机密性","机密性与压缩","加密与解密","身份与备份"], answer:0, explain:"签名证明内容未改且来自私钥持有者；保密需要额外加密。" },
  { id:"cw12", topic:"crypto-sign", cat:"crypto", level:"初级", q:"验签时最关键的前提是？", options:["签名者的公钥来自可信渠道","签名长度足够长","使用最新算法版本","签名速度足够快"], answer:0, explain:"公钥若不可信，攻击者可替换公钥后用自己私钥签名，验签形同虚设。" },
  { id:"cw13", topic:"pqc", cat:"crypto", level:"高级", q:"「先收集、后解密」威胁意味着？", options:["长期保密数据应优先迁移到抗量子方案","短期会话数据更需要保护","量子计算已经可用","加密不再必要"], answer:0, explain:"今天被截获的密文未来可能被解密，因此保密期长的数据优先级最高。" },
  { id:"cw14", topic:"crypto-pqc", cat:"crypto", level:"高级", q:"推进密码迁移时的工程能力指的是？", options:["密码敏捷性：算法与协议可快速替换而不改业务","只更换密钥长度","只升级 OpenSSL 版本","只在客户端改算法"], answer:0, explain:"敏捷性决定了未来面对算法失效时能否快速全局替换。" },
  { id:"pw1", topic:"pt-process", cat:"pentest", level:"中级", q:"把「信息泄露 + 越权 + 上传」串成攻击链的意义是？", options:["说明真实风险远高于单个漏洞的独立评级","让报告更花哨","减少测试工作量","规避授权限制"], answer:0, explain:"单点低危组合起来可能直接导致接管，报告应给出攻击链与最坏影响。" },
  { id:"pw2", topic:"pt-web-exp", cat:"pentest", level:"中级", q:"记录攻击链时应重点包含？", options:["前置条件、可复现步骤与最终影响","测试人员心情","使用工具的版本号","测试耗时"], answer:0, explain:"可复现与影响判定是说服开发修复的关键，工具细节属于附录。" },
  { id:"pw3", topic:"pt-social", cat:"pentest", level:"中级", q:"钓鱼演练中最应关注的正向指标是？", options:["上报率与上报平均耗时","点击率越高越成功","提交率越高越好","邮件投递成功率"], answer:0, explain:"演练目标是让人更快上报，点击率与提交率下降是结果而非考核指标。" },
  { id:"pw4", topic:"pt-social", cat:"pentest", level:"中级", q:"钓鱼演练设计时应当避免的是？", options:["使用造成恐慌的内容（如裁员、事故通知）","使用贴合业务的真实场景模板","提供一键上报通道","对高权限岗位单独设计场景"], answer:0, explain:"演练要避免引发恐慌或影响正常业务，同时保证有上报通道与事后培训。" },
  { id:"pw5", topic:"pt-report", cat:"pentest", level:"初级", q:"渗透报告中的修复建议应当？", options:["具体到可执行的配置或代码位置","只写「加强校验」","只引用标准编号","交给开发自行判断"], answer:0, explain:"可落地的建议才能被修复，也便于复测确认问题是否真正解决。" },
  { id:"pw6", topic:"pt-report", cat:"pentest", level:"初级", q:"渗透测试的闭环通常以什么为终点？", options:["复测确认问题已修复（或已接受风险）","提交报告即结束","付款完成","下一次测试开始"], answer:0, explain:"没有复测就没有闭环；未修复项应显式记录为已接受风险。" },
  { id:"bw9", topic:"rop", cat:"binary", level:"高级", q:"NX 保护开启后，攻击者转向 ROP 的原因是？", options:["无法在栈上执行注入代码，只能复用已有代码片段","ROP 更快","栈溢出不再可能","ASLR 失效"], answer:0, explain:"ROP 复用程序中已有的指令片段来构造逻辑，绕过「不可执行」的限制。" },
  { id:"bw10", topic:"rop", cat:"binary", level:"高级", q:"从防守角度降低 ROP 可行性的做法是？", options:["开启 PIE/NX/Canary/CET 等缓解并及时修补内存漏洞","关闭 ASLR 便于调试","静态链接所有库","减少日志输出"], answer:0, explain:"完整缓解组合能显著提高利用难度，源头仍是减少内存漏洞。" },
  { id:"mw1", topic:"mobile-attack-surface", cat:"mobile", level:"高级", q:"移动逆向中最值得优先追踪的目标是？", options:["客户端本地做的安全判断与硬编码密钥","界面布局代码","图片资源","字体文件"], answer:0, explain:"本地判断与硬编码密钥可被绕过或提取，影响最大，应优先验证并推动服务端补强。" },
  { id:"mw2", topic:"app-hardening", cat:"mobile", level:"高级", q:"完成客户端绕过验证后，报告应重点给出？", options:["服务端应如何补强（风控、限速、权限校验）","更复杂的绕过脚本","加固产品推荐","客户端混淆方案"], answer:0, explain:"客户端防护只能提高成本，真正的修复落在服务端校验与风控。" },
  { id:"dw1", topic:"ds-gateway", cat:"datasec", level:"中级", q:"数据网关解决的核心问题是？", options:["把数据库访问收敛到统一入口以做鉴权、脱敏与审计","提升数据库性能","减少存储成本","加速备份"], answer:0, explain:"散落的直连无法统一管控，收敛入口才能让权限与审计真正生效。" },
  { id:"dw2", topic:"ds-encrypt", cat:"datasec", level:"中级", q:"动态脱敏相比静态脱敏的优势是？", options:["同一份数据可按访问者角色返回不同结果（如打码）","性能更高","无需规则配置","可完全替代加密"], answer:0, explain:"动态脱敏在生产查询场景即可生效，适合「既要能用又要少暴露」的需求。" },
  { id:"sw1", topic:"sc-artifact", cat:"supply", level:"中级", q:"制品仓库开启「标签不可变」的目的是？", options:["防止同一标签被覆盖推送，保证部署来源可追溯","节省存储空间","加快拉取速度","减少镜像层数"], answer:0, explain:"允许覆盖会让「镜像内容与记录不一致」，破坏可追溯性与签名意义。" },
  { id:"sw2", topic:"sc-artifact", cat:"supply", level:"中级", q:"镜像准入控制通常要求？", options:["仅允许来自内部仓库且带签名的镜像，并限制高危漏洞","允许任意公共镜像","只检查镜像大小","只检查构建时间"], answer:0, explain:"准入把「不可信来源」挡在集群之外，是供应链防护的关键落点。" },
  { id:"aiw1", topic:"ai-prompt-eng", cat:"aisec", level:"中级", q:"判断「提示词是否被当成了安全防线」，一个实用的检验是？", options:["把提示词整段删掉，系统是否仍然安全","提示词是否足够长","是否用了英文","是否包含示例"], answer:0, explain:"安全职责应在代码层（权限、白名单、校验），提示词只负责体验与风格。" },
  { id:"aiw2", topic:"ai-prompt-eng", cat:"aisec", level:"中级", q:"提示词与工具配置为什么需要版本管理与回归？", options:["它们等价于生产配置，改动可能引入安全退化","为了好看","因为体积大","因为要付费"], answer:0, explain:"提示词或工具的细微改动可能让既有防护失效，需回归验证。" },
  { id:"tw1", topic:"soc-tabletop", cat:"soccti", level:"高级", q:"桌面推演的主要价值是？", options:["暴露流程与联系人的缺口，而非检验技术工具","评估员工绩效","替代真实演练","减少安全预算"], answer:0, explain:"推演以低成本暴露「谁在什么时候做什么」的断点，改进项需落责任与期限。" },
  { id:"tw2", topic:"soc-tabletop", cat:"soccti", level:"高级", q:"演练结束后最关键的动作是？", options:["把卡点转成改进项，明确责任人与期限并复验","发布演练通稿","统计参与人数","归档照片"], answer:0, explain:"没有改进项闭环的演练只产生文档，不产生能力。" },

  // ---- 领域补齐（v1.5.1 第七批）----
  { id:"nx1", topic:"net-vpn", cat:"network", level:"中级", q:"远程接入（VPN）安全最关键的加固项是？", options:["强制双因素认证并限定接入后可访问的网段","更换 VPN 设备品牌","提升带宽","开启更多加密套件"], answer:0, explain:"账号共享与过宽可达是远程接入的典型问题，MFA + 最小可达能显著降低风险。" },
  { id:"nx2", topic:"net-vpn", cat:"network", level:"中级", q:"为什么 VPN 设备自身漏洞值得高度关注？", options:["它暴露在公网且直通内网，一旦被利用等于直接被突破边界","它消耗带宽","它影响 DNS","它只在夜间运行"], answer:0, explain:"边界设备是高价值目标，需及时更新固件并限制管理面暴露。" },
  { id:"nx3", topic:"net-segment", cat:"network", level:"高级", q:"做网络分区时，最重要的输入是？", options:["业务之间「谁需要访问谁」的连通关系","交换机的品牌","机房的面积","员工的座位表"], answer:0, explain:"分区策略要基于真实业务依赖，否则不是放太宽就是影响业务而被迫全放通。" },
  { id:"nx4", topic:"net-segment", cat:"network", level:"高级", q:"微隔离相比网段级隔离的优势是？", options:["按身份/标签做到主机级最小可达，限制横向移动更彻底","配置更少","不需要日志","性能更好"], answer:0, explain:"微隔离粒度更细但维护成本更高，通常从流量基线自动学习逐步收敛。" },
  { id:"nx5", topic:"net-dns-sec", cat:"network", level:"中级", q:"DNS 查询日志对安全运营的价值是？", options:["可发现恶意域名解析、DNS 隧道与异常外联","提升解析速度","减少带宽","加密查询内容"], answer:0, explain:"DNS 日志是检测隐蔽通道与外联的关键数据源，应集中留存并接入检测。" },
  { id:"nx6", topic:"net-dns-sec", cat:"network", level:"中级", q:"企业启用加密 DNS（DoH）可能带来的影响是？", options:["安全侧可见性下降，需在策略上权衡或统一出口","DNS 解析变慢","无法访问外网","域名无法解析"], answer:0, explain:"DoH 能提升隐私，但会绕过企业 DNS 监控，需要策略与统一出口配合。" },
  { id:"cx1", topic:"cloud-secret", cat:"cloud", level:"中级", q:"云原生环境中存储密钥的正确做法是？", options:["使用密钥管理服务并按需注入临时凭据","写进 ConfigMap","打包进镜像","明文放环境变量"], answer:0, explain:"ConfigMap 与镜像都是明文可见的，密钥必须走密钥服务并按最小权限注入。" },
  { id:"cx2", topic:"cloud-monitor", cat:"cloud", level:"中级", q:"读取密钥的权限为什么要单独收敛？", options:["被攻陷的工作负载若可直接读密钥，等于直接提权","为了减少日志","为了节省费用","为了加快启动"], answer:0, explain:"密钥是高价值目标，权限过宽会让一次容器失陷升级为账号级失陷。" },
  { id:"cx3", topic:"cloud-image-govern", cat:"cloud", level:"中级", q:"为什么容器镜像需要定期重建？", options:["上游基础层的补丁不会自动进入已有镜像，重建才能带上修复","为了减小体积","为了更换基础镜像品牌","为了提升启动速度"], answer:0, explain:"老镜像等于长期携带旧漏洞，固定节奏重建是必要治理动作。" },
  { id:"cx4", topic:"cloud-image-govern", cat:"cloud", level:"中级", q:"镜像瘦身的安全意义是？", options:["去掉编译器与调试工具，减少被利用的工具与组件面","提升构建速度","减少镜像数量","降低推送频率"], answer:0, explain:"越少的组件意味着越少的漏洞面与更少的可用工具。" },
  { id:"cx5", topic:"cloud-multicloud", cat:"cloud", level:"高级", q:"多账号云环境统一治理的基础是？", options:["集中审计（业务账号无权删除）与组织级策略基线","统一计费","统一界面皮肤","统一实例规格"], answer:0, explain:"集中审计保证事后可追溯，组织策略保证基线不被绕过。" },
  { id:"cx6", topic:"cloud-multicloud", cat:"cloud", level:"高级", q:"多云治理最容易失控的是？", options:["身份与权限分散在各云，没有统一台账与复核","网络带宽","存储容量","镜像数量"], answer:0, explain:"身份与权限是云上安全的核心，必须建立统一台账与定期复核机制。" },
  { id:"bx1", topic:"blue-threat-model", cat:"blue", level:"中级", q:"防守方做威胁建模的首要输入是？", options:["关键资产与其暴露面","安全设备清单","团队成员数量","预算规模"], answer:0, explain:"先明确要保护什么、对手可能从哪进来，才能决定监控与加固的优先级。" },
  { id:"bx2", topic:"blue-role", cat:"blue", level:"中级", q:"威胁模型需要更新的触发条件是？", options:["上线新系统或开放新接口等攻击面变化","更换办公软件","调整排班","更换打印机"], answer:0, explain:"攻击面变了，防护与检测的重点也要跟着变，否则会出现监控盲区。" },
  { id:"bx3", topic:"blue-hunt-basics", cat:"blue", level:"中级", q:"最适合优先自动化的处置动作是？", options:["情报查询与告警富化等只读动作","隔离核心数据库服务器","批量删除账号","修改生产网络策略"], answer:0, explain:"先从只读、可回滚的动作开始，有副作用的动作保留人工确认。" },
  { id:"bx4", topic:"ids", cat:"blue", level:"中级", q:"自动化的主要风险是？", options:["把错误的判断大规模执行，影响业务","运行速度太快","占用磁盘空间","需要更多人力"], answer:0, explain:"因此需要白名单、可回滚、执行留痕与失败告警，并定期抽查执行效果。" },
  { id:"bx5", topic:"blue-report", cat:"blue", level:"初级", q:"给管理层的安全月报，重点应是？", options:["关键风险、趋势变化与需要的决策支持","告警总条数","设备型号清单","团队加班时长"], answer:0, explain:"管理层需要的是风险与决策依据，技术细节放在附录。" },
  { id:"bx6", topic:"blue-oncall", cat:"blue", level:"初级", q:"衡量安全运营趋势的合适指标组合是？", options:["MTTD/MTTR、关键资产覆盖率、漏检复盘数","告警总数与关闭数","设备数量与价格","值班人数"], answer:0, explain:"关注发现与处置效率、覆盖能力与漏检改进，而非数量堆砌。" },
  { id:"wx1", topic:"web-api-sec", cat:"web", level:"初级", q:"检测接口是否存在对象级越权，最直接的方法是？", options:["用 A 账号的令牌请求 B 账号的资源 id","查看接口文档版本号","测量响应时间","检查是否有验证码"], answer:0, explain:"对象级授权缺失是 API 最常见的高危问题，验证方式就是交叉替换资源标识。" },
  { id:"wx2", topic:"auth", cat:"web", level:"初级", q:"API 上线的必备防护不包括？", options:["接口返回尽量详细的堆栈信息便于排错","认证与授权校验","频率限制","参数类型与范围校验"], answer:0, explain:"详细错误信息会泄露内部细节，应统一化并记录到服务端日志。" },
  { id:"wx3", topic:"web-authn", cat:"web", level:"中级", q:"登录成功后重新生成会话 ID 的目的是？", options:["防止会话固定攻击（攻击者预先设定会话 ID）","提升登录速度","减少 Cookie 体积","兼容旧浏览器"], answer:0, explain:"会话固定利用登录前后会话不变的特点劫持会话，重新生成即可防御。" },
  { id:"wx4", topic:"web-authn", cat:"web", level:"中级", q:"找回密码流程的设计要点是？", options:["令牌随机、一次性、有时效，且不泄露账号是否存在","用账号名作为令牌","令牌永久有效","允许无限次尝试"], answer:0, explain:"找回流程是账号接管的高发点，令牌强度与信息泄露都要控制。" },
  { id:"wx5", topic:"web-upload", cat:"web", level:"中级", q:"文件上传功能的安全做法是？", options:["按内容白名单校验类型、随机化文件名并存到不可执行位置","只校验扩展名","保留原文件名放在网站目录","允许上传任意大小"], answer:0, explain:"扩展名可伪造、原文件名可能带路径；内容校验 + 改名 + 隔离存储是基本组合。" },
  ...
  { id:"wx7", topic:"ssrf", cat:"web", level:"中级", q:"SSRF 最危险的后果之一是？", options:["读取云实例元数据获取临时凭证，进而控制云资源","页面加载变慢","图片显示异常","Cookie 丢失"], answer:0, explain:"云上 SSRF 常升级为凭证窃取，需强制 IMDSv2 并限制出网。" },
  { id:"wx8", topic:"ssrf", cat:"web", level:"中级", q:"防御 SSRF 时，为什么「先解析后校验」很重要？", options:["可防止攻击者用域名解析到内网地址绕过字符串校验","能提升解析速度","可减少 DNS 查询","能兼容更多域名"], answer:0, explain:"只做字符串白名单会被 DNS 解析与重绑定绕过，必须对解析结果做内网地址判定。" },
  { id:"bz1", topic:"uaf", cat:"binary", level:"高级", q:"UAF（释放后使用）漏洞的本质是？", options:["指针所指向的内存已被释放并可能被复用，继续使用会操作他人数据","内存从未分配","程序无法运行","栈被清空"], answer:0, explain:"释放后内存可能被分配给别的对象，继续访问等于读写别人的数据，可被用于劫持。" },
  ...
  { id:"bz3", topic:"fuzz", cat:"binary", level:"中级", q:"模糊测试中，覆盖率的意义是？", options:["衡量探索到了多少代码路径，决定能否挖到更深的问题","衡量崩溃个数","衡量运行时间","衡量内存占用"], answer:0, explain:"没有覆盖率的增长，长时间运行也在原地打转；崩溃去重与最小化同样重要。" },
  { id:"bz4", topic:"fuzz", cat:"binary", level:"中级", q:"fuzzing 发现崩溃后，为什么要做输入最小化？", options:["便于开发复现与定位根因，并可作为回归用例","为了减小文件体积","为了提升 fuzz 速度","为了隐藏漏洞"], answer:0, explain:"最小化后的样本更易复现与调试，也适合长期回归。" },
  { id:"cy1", topic:"crypto-hash-store", cat:"crypto", level:"中级", q:"从用户口令派生加密密钥时，正确做法是？", options:["使用带盐与足够代价的 KDF（如 PBKDF2/argon2）","直接 SHA256(口令)","直接使用口令字节","Base64 编码口令"], answer:0, explain:"直接哈希无盐且过快，无法抵抗暴力与彩虹表攻击。" },
  { id:"cy2", topic:"crypto-kdf", cat:"crypto", level:"中级", q:"KDF 的「代价参数」需要随什么调整？", options:["硬件性能提升（保证破解成本不下降）","用户数量","服务器数量","网络带宽"], answer:0, explain:"硬件越来越快，代价参数需要定期评估上调以维持安全边际。" },
  { id:"cy3", topic:"crypto-tls-practice", cat:"crypto", level:"中级", q:"TLS 配置中应禁用的协议版本是？", options:["SSLv3 与 TLS 1.0/1.1","TLS 1.2","TLS 1.3","全部版本"], answer:0, explain:"老版本存在已知弱点，应只保留 1.2（尽量 1.3）并优先 AEAD 套件。" },
  { id:"cy4", topic:"crypto-sign", cat:"crypto", level:"中级", q:"「加密了但不完整」指的是？", options:["只加密未做完整性校验，密文可被篡改而不被发现","加密强度不足","密钥太短","未使用 https"], answer:0, explain:"应使用 AEAD（带认证的加密）或额外加签名/MAC，避免可篡改的密文。" },
  { id:"px1", topic:"pt-web-exp", cat:"pentest", level:"中级", q:"为什么同一业务的多端（Web/App）要一起测？", options:["各端校验强度可能不一致，宽松的一端会绕过严格的一端","为了增加工作量","因为客户要求","为了收集截图"], answer:0, explain:"多端一致性缺失是系统性风险，应作为整体评估而非单点问题。" },
  { id:"px2", topic:"pt-dual-end", cat:"pentest", level:"中级", q:"前端做的限制（隐藏按钮、金额校验）在安全上的意义是？", options:["仅用于体验优化，不能作为安全控制","等同于服务端校验","可防止越权","可防止篡改"], answer:0, explain:"客户端逻辑可被绕过，真正的校验必须在服务端完成。" },
  { id:"px3", topic:"report", cat:"pentest", level:"初级", q:"复测的正确做法是？", options:["用原复现步骤验证，并检查同类问题是否一并修复","只看开发提交的说明","只跑一遍扫描器","等下次测试再看"], answer:0, explain:"复测要给出实际结果证据，并关注同模式的其他位置。" },
  { id:"px4", topic:"report", cat:"pentest", level:"初级", q:"对确实无法修复的问题，应如何处理？", options:["登记为已接受风险，明确责任人与补偿措施","从报告中删除","标记为已修复","忽略不提"], answer:0, explain:"显式记录与补偿控制（如加监控）比隐匿更安全，也便于后续复评。" },
  { id:"mx1", topic:"mob-mdm", cat:"mobile", level:"中级", q:"MDM 落地时需要平衡的核心矛盾是？", options:["企业数据可控与员工个人隐私","设备价格与性能","系统版本与界面","网络速度与容量"], answer:0, explain:"企业数据容器化、个人数据不监控，是兼顾安全与隐私的常见做法。" },
  { id:"mx2", topic:"mob-mdm", cat:"mobile", level:"中级", q:"把设备合规状态作为访问决策输入，属于哪种思路？", options:["零信任：按身份与设备状态动态授权","传统边界防护","仅网络层控制","仅口令认证"], answer:0, explain:"零信任不因「在内网」就信任，而是每次访问都校验身份与设备健康状态。" },
  { id:"dx1", topic:"ds-tokenize", cat:"datasec", level:"高级", q:"令牌化与脱敏的关键区别是？", options:["令牌化可逆且保留格式（便于业务使用），脱敏通常只用于展示","令牌化更便宜","脱敏更安全","两者完全相同"], answer:0, explain:"令牌化适用于业务需要原值可查的场景，映射关系必须严格保管。" },
  { id:"dx2", topic:"ds-tokenize", cat:"datasec", level:"高级", q:"令牌映射表的最大风险是？", options:["成为「一把万能钥匙」，一旦泄露所有令牌都可还原","占用存储空间","查询速度慢","格式不统一"], answer:0, explain:"因此映射表需独立加密、最小访问、全量审计，并考虑高可用与隔离。" },
  { id:"sy1", topic:"sc-typosquat", cat:"supply", level:"中级", q:"依赖变化监控中，值得警惕的异常是？", options:["私有包名忽然出现在公共仓库（可能被抢注）","包体积变小","文档更新","作者增加了测试"], answer:0, explain:"公共仓库出现同名包是抢注与投毒的典型信号，需立即核查与阻断。" },
  { id:"sy2", topic:"sc-artifact", cat:"supply", level:"中级", q:"把依赖变化接入 CI 的价值是？", options:["版本异常变化可被自动拦截，避免污染进入构建","提升构建速度","减少镜像体积","降低测试成本"], answer:0, explain:"门禁能在引入环节阻断风险，比事后排查成本低得多。" },
  { id:"ay1", topic:"aisec-privacy", cat:"aisec", level:"中级", q:"AI 应用审计日志至少应记录？", options:["请求者身份、模型与版本、工具调用与结果、是否触发人工确认","只有回答内容","只有耗时","只有费用"], answer:0, explain:"缺少身份与动作记录无法复盘；同时注意脱敏与留存期限。" },
  { id:"ay2", topic:"aisec-landscape", cat:"aisec", level:"中级", q:"下列哪个信号适合作为 AI 应用的异常告警？", options:["工具调用被拒次数突增或输出敏感信息被护栏拦截","模型响应变快","用户数量增加","缓存命中率上升"], answer:0, explain:"这类信号往往意味着有人在试探边界或数据正在外泄，值得立即关注。" },
  { id:"tz1", topic:"soc-intel-priority", cat:"soccti", level:"中级", q:"情报驱动检测优先级的正确做法是？", options:["按行业与资产相关性筛选情报，再转化为待验证的检测假设","把所有 IOC 全部导入设备","只订阅数量最多的情报源","等出事再查情报"], answer:0, explain:"相关性与可执行性决定情报价值，转化为假设后需用演练或狩猎验证。" },
  { id:"tz2", topic:"soc-platform", cat:"soccti", level:"中级", q:"评估情报是否真正产生价值，应看？", options:["它转化成了多少条生效规则与多少起真实发现","订阅源数量","报告篇幅","下载次数"], answer:0, explain:"情报价值体现在落地与验证，而非收集与归档。" },

  // ---- 补题（v1.5.1 第八批：把题量拉平到每知识点 ≥3）----
  { id:"nz1", topic:"net-packet", cat:"network", level:"入门", q:"明文协议（HTTP/Telnet/FTP）在链路上传输时，同网段攻击者可以？", options:["直接读取内容与凭据","只能看到包的数量","无法获取任何信息","只能看到目标 IP"], answer:0, explain:"明文协议不做保护，抓包即可读取内容；应改用加密协议。" },
  { id:"nz2", topic:"port-scan", cat:"network", level:"初级", q:"端口扫描中最能反映「服务真实身份」的信息是？", options:["服务与版本指纹（banner/协议特征）","端口号本身","扫描耗时","目标 IP 归属地"], answer:0, explain:"端口号可随意更改，服务与版本指纹才是判断资产与漏洞面的依据。" },
  { id:"nz3", topic:"net-dhcp", cat:"network", level:"中级", q:"ARP 欺骗在内网之所以容易实施，根本原因是？", options:["ARP 协议本身不认证，任何人都能应答","交换机性能不足","IP 地址池太小","缺少 DNS 服务器"], answer:0, explain:"无认证的地址解析是设计缺陷，需靠接入层防护（DHCP Snooping + DAI）弥补。" },
  { id:"nz4", topic:"net-lateral", cat:"network", level:"高级", q:"Kerberos 预认证的作用是？", options:["要求客户端先证明身份再获取票据，避免被离线爆破","加密全部内网流量","防止横向移动","限制登录时段"], answer:0, explain:"关闭预认证会暴露 AS-REP 供离线爆破，因此必须保持开启。" },
  { id:"nz5", topic:"fw-bypass", cat:"network", level:"中级", q:"绕过基于内容的检测时，攻击者常用的思路是？", options:["让载荷在单包内无法被识别（分片、编码、等价语法）","加密后发送明文","提高发送频率","使用更大带宽"], answer:0, explain:"检测方需做规范化与重组，否则特征匹配会失效。" },
  { id:"nz6", topic:"net-devices", cat:"network", level:"入门", q:"路由器与交换机的主要分工是？", options:["路由器做跨网段转发，交换机做同网段转发","两者完全等价","交换机负责加密","路由器负责端口安全"], answer:0, explain:"理解二层与三层分工是排查网络问题与理解隔离效果的基础。" },
  ...
  { id:"nz8", topic:"net-dns-sec", cat:"network", level:"中级", q:"判断「某主机是否在扫描全网段」，最有价值的日志是？", options:["该主机的连接/拒绝日志短时间内覆盖大量目标","DNS 查询日志","CPU 使用率","磁盘写入量"], answer:0, explain:"扫描的特征是「一对多」的连接尝试，网络设备与主机的连接日志最能反映。" },
  { id:"nz9", topic:"net-ztna", cat:"network", level:"高级", q:"零信任网络与传统边界防护的核心差异是？", options:["不因「在内网」就信任，每次访问都校验身份与设备状态","完全不使用防火墙","只依赖口令认证","取消所有网络分区"], answer:0, explain:"零信任把信任从「位置」改为「持续校验」，更适合远程与多云场景。" },
  { id:"cz1", topic:"shared-resp", cat:"cloud", level:"入门", q:"云上「安全责任共担」中，客户始终负责的是？", options:["自己的数据、配置与访问权限","物理机房安全","虚拟化层实现","云厂商的硬件固件"], answer:0, explain:"无论哪种服务模型，数据与配置的责任都在客户侧，这也是云上问题的集中区。" },
  { id:"cz2", topic:"iam", cat:"cloud", level:"初级", q:"云上使用角色（Role）而非长期密钥的核心好处是？", options:["权限可精细限定且凭据可自动轮换过期","配置更少","速度更快","费用更低"], answer:0, explain:"临时凭据显著缩短泄露后的可用窗口，并便于按最小权限设计。" },
  { id:"cz3", topic:"cloud-tenant", cat:"cloud", level:"高级", q:"容器与宿主机共享内核带来的安全含义是？", options:["内核漏洞可直接导致逃逸，隔离强度低于虚拟机","容器无法运行","容器更省资源所以更安全","无需加固内核"], answer:0, explain:"因此需要及时打补丁、限制特权与危险挂载，并使用安全运行时。" },
  { id:"cz4", topic:"k8s", cat:"cloud", level:"高级", q:"Kubernetes 中「自动挂载服务账号令牌」的风险是？", options:["Pod 内可获取集群 API 凭据，被攻陷后可能直接操作集群","增加启动时间","占用内存","影响网络性能"], answer:0, explain:"不需要访问 API 的 Pod 应禁用自动挂载，并按需最小授权。" },
  { id:"cz5", topic:"serverless", cat:"cloud", level:"中级", q:"Serverless 函数被投毒依赖利用后，攻击者继承的是？", options:["函数的执行角色权限，可调用该角色被允许的云资源","宿主机 root 权限","其他租户的数据","云厂商的管理权限"], answer:0, explain:"因此函数角色必须最小化，并限制其可访问的资源范围。" },
  { id:"cz6", topic:"metadata", cat:"cloud", level:"中级", q:"云元数据服务（IMDS）的主要风险是？", options:["可被 SSRF 或误配读取，泄露实例临时凭据","导致实例重启","降低网络性能","影响磁盘 IO"], answer:0, explain:"强制 IMDSv2 并限制跳数可显著抬高利用门槛。" },
  { id:"cz7", topic:"cloud-multicloud", cat:"cloud", level:"入门", q:"云上排查「谁把桶设成公开」时，最直接的依据是？", options:["对象存储的访问策略与操作审计日志","实例的 CPU 曲线","网络带宽图","镜像构建记录"], answer:0, explain:"策略变更与操作审计能定位「谁在什么时候改了什么」。" },
  { id:"cz8", topic:"cloud-iac", cat:"cloud", level:"中级", q:"云上「配置漂移」指的是？", options:["实际运行配置偏离了既定基线（多因手工临时修改）","磁盘碎片增多","网络延迟波动","镜像体积变大"], answer:0, explain:"配置漂移会让基线失效，需用配置扫描与基础设施即代码收敛。" },
  { id:"cz9", topic:"cloud-multicloud", cat:"cloud", level:"高级", q:"多云环境下做统一事件响应时，最需要先打通的是？", options:["各云的审计日志与资产清单（统一时间线与责任人）","各云的计费系统","各云的界面风格","各云的实例规格"], answer:0, explain:"没有统一的日志与资产视图，跨云事件无法快速界定范围与定责。" },
  { id:"bz9", topic:"siem", cat:"blue", level:"入门", q:"SIEM 与「单纯的日志存储」的关键差别是？", options:["SIEM 提供关联分析与告警，而不只是保存日志","SIEM 容量更大","SIEM 更便宜","SIEM 不需要日志源"], answer:0, explain:"关联与告警才是检测价值的来源，纯存储只是数据湖。" },
  { id:"bz10", topic:"ids", cat:"blue", level:"中级", q:"基于异常的 IDS 相比基于特征的 IDS，主要特点是？", options:["能发现偏离基线的未知行为，但误报通常更高","完全不会误报","不需要基线数据","只能检测已知攻击"], answer:0, explain:"异常检测覆盖未知威胁，但需要精细的基线与管理误报。" },
  { id:"bz11", topic:"ids", cat:"blue", level:"中级", q:"分析 C2 心跳时，「jitter（抖动）」指的是？", options:["心跳间隔的随机扰动，用于规避固定周期检测","数据包大小","加密算法","端口号范围"], answer:0, explain:"加入抖动会让「固定间隔」这一特征失效，检测需转向统计学特征。" },
  { id:"bz12", topic:"ir", cat:"blue", level:"高级", q:"勒索事件中「恢复验证」为什么必须在隔离环境做？", options:["防止备份中潜藏的恶意代码或后门再次污染生产环境","为了节省时间","因为生产环境容量不足","便于生成报告"], answer:0, explain:"未验证的备份可能已被污染，隔离验证是恢复前的必要关卡。" },
  { id:"bz13", topic:"threat-intel", cat:"blue", level:"初级", q:"威胁情报中「IOA」更关注什么？", options:["攻击行为与手法特征（如何做）","文件哈希（是什么）","域名注册信息","IP 归属地"], answer:0, explain:"IOA 描述行为模式，比静态指标更难被规避。" },
  { id:"bz14", topic:"edr", cat:"blue", level:"初级", q:"EDR 的「遥测」通常包含？", options:["进程创建、命令行、网络连接、文件与注册表操作等行为数据","只有病毒库版本","只有登录时间","只有磁盘剩余空间"], answer:0, explain:"丰富的遥测是行为检测与溯源的基础。" },
  { id:"bz15", topic:"ids", cat:"blue", level:"中级", q:"把「变更系统」接入安全运营的价值是？", options:["区分「计划内变更」与「异常行为」，显著降低误报","加快网络速度","减少日志量","提升设备寿命"], answer:0, explain:"多数告警噪音来自合法变更，上下文对齐是降噪的关键。" },
  { id:"bz16", topic:"blue-playbook", cat:"blue", level:"中级", q:"处置剧本里「升级条件」的作用是？", options:["明确何时必须通知更高层级并启动应急，避免延误","记录处理人姓名","统计工作量","计算成本"], answer:0, explain:"升级条件把「什么时候该叫人」变成明确规则，减少主观犹豫。" },
  { id:"bz17", topic:"blue-hunt-basics", cat:"blue", level:"高级", q:"狩猎假设的来源通常包括？", options:["ATT&CK 技术、最新情报与本次事件复盘结论","员工的直觉","设备厂商建议","随机挑选"], answer:0, explain:"假设要有依据：对手手法、情报指向或既有事件的未解疑点。" },
  { id:"mz1", topic:"mobile-attack-surface", cat:"mobile", level:"入门", q:"移动安全评估通常从哪三条线同时展开？", options:["客户端、本地数据与通信/接口","界面、性能与兼容性","安装、卸载与升级","广告、推送与统计"], answer:0, explain:"三条线互相印证，单看一条容易漏掉跨层组合风险。" },
  { id:"mz2", topic:"apk-sign", cat:"mobile", level:"初级", q:"重打包后应用无法覆盖安装在原应用上，原因是？", options:["签名不同，系统拒绝对不同签名的应用执行覆盖升级","包名变了","版本号变了","缺少权限声明"], answer:0, explain:"签名是升级链路的信任基础，改包必然破坏这条链路。" },
  { id:"mz3", topic:"ssl-pinning", cat:"mobile", level:"初级", q:"用 adb 拉取应用私有目录数据通常需要什么前提？", options:["设备 root 或应用为 debuggable","安装 Burp 证书","开启飞行模式","应用处于前台"], answer:0, explain:"私有目录受系统保护，需 root 或可调试应用才能直接读取。" },
  { id:"mz4", topic:"webview", cat:"mobile", level:"中级", q:"WebView 加载的 URL 若来自深链接参数，主要风险是？", options:["URL 可控可导致加载攻击者页面并调用 JS 桥","页面加载变慢","字体显示异常","无法返回上一页"], answer:0, explain:"不可信的 URL 会把手里的原生能力交给外部页面，必须严格白名单校验。" },
  { id:"mz5", topic:"local-storage", cat:"mobile", level:"入门", q:"检查应用是否把敏感信息写入日志，最直接的方式是？", options:["用 logcat 过滤 token/password 等关键字","查看应用图标","测量启动耗时","检查安装包大小"], answer:0, explain:"日志是常见的敏感信息泄露渠道，上线前应做关键字审查。" },
  { id:"mz6", topic:"ssl-pinning", cat:"mobile", level:"初级", q:"证书固定被绕过在报告中的正确表述是？", options:["客户端防护可被绕过，真正防线应在服务端鉴权与风控","说明服务端不安全","建议取消 HTTPS","必须改用私有协议"], answer:0, explain:"客户端防护提高成本但不构成保证，结论要指向服务端与风控设计。" },
  { id:"mz7", topic:"dynamic-debug", cat:"mobile", level:"中级", q:"应用被反调试卡住时，分析者的常见处理是？", options:["定位检测函数并 hook 返回假值或直接绕过检测分支","重装系统","放弃动态分析","换一台手机即可绕过"], answer:0, explain:"反调试本质是提高成本，定位检测点后 hook 或 patch 是标准做法。" },
  { id:"mz8", topic:"root-detect", cat:"mobile", level:"中级", q:"Magisk 隐藏（DenyList/Zygisk）能绕过部分 Root 检测，但仍可能被什么发现？", options:["云端完整性校验（如 Play Integrity）","本地日志","路由器日志","应用商店评论"], answer:0, explain:"完整性校验上移到云端后，本地隐藏不足以通过验证。" },
  { id:"mz9", topic:"app-hardening", cat:"mobile", level:"高级", q:"抽取式壳的脱壳时机是？", options:["壳完成解密、真实代码进入内存之后","应用安装时","下载安装包时","应用卸载时"], answer:0, explain:"只有在内存中代码已解密才能 dump，并需要修复 dex 结构。" },
  { id:"mz10", topic:"apk-sign", cat:"mobile", level:"初级", q:"iOS 的 ATS（App Transport Security）放宽配置带来的风险是？", options:["可能允许明文 HTTP 通信，导致数据可被窃听","应用无法联网","证书校验加强","性能下降"], answer:0, explain:"ATS 例外会削弱传输安全，评估时应重点核查例外范围与必要性。" },
  { id:"mz11", topic:"jailbreak-detect", cat:"mobile", level:"中级", q:"越狱检测被绕过后的评估重点应是？", options:["绕过之后攻击者能获取哪些数据与能力（Keychain、内存、hook）","检测代码行数","检测用了哪些 API","绕过的速度"], answer:0, explain:"结论要落到真实影响面，而不是停留在「能绕过」本身。" },
  { id:"mz12", topic:"mobile-api", cat:"mobile", level:"中级", q:"移动接口最常见的两类问题是？", options:["对象级越权与缺乏频率限制","不支持 JSON","不使用 HTTPS","不返回状态码"], answer:0, explain:"越权与限流缺失是移动接口最典型的高危问题。" },
  { id:"mz13", topic:"mobile-malware", cat:"mobile", level:"高级", q:"静态提取移动恶意样本 IOC 时，优先关注？", options:["硬编码的域名/URL 与可疑权限组合","应用图标配色","安装包体积","签名算法"], answer:0, explain:"域名与权限组合能快速指向 C2 与恶意行为，是 IOC 提取的重点。" },
  { id:"mz14", topic:"mob-anatomy", cat:"mobile", level:"入门", q:"APK 配置清单中 exported=true 的组件意味着？", options:["可被其它应用调用，需要校验权限或调用方","仅调试可见","必须系统签名","应用启动后才生效"], answer:0, explain:"导出组件是对外入口，缺少校验即可被越权调用。" },
  { id:"mz15", topic:"mob-permission", cat:"mobile", level:"入门", q:"看到一个手电筒应用申请「读取短信」权限，合理判断是？", options:["权限与功能不匹配，存在过度收集嫌疑","属于正常需求","说明应用功能强大","说明系统版本过低"], answer:0, explain:"权限与功能不匹配是过度收集与恶意行为的典型信号。" },
  { id:"mz16", topic:"app-hardening", cat:"mobile", level:"高级", q:"移动逆向的核心产出应该是？", options:["关键逻辑的可复现绕过证据与服务端补强建议","一份混淆后的源码","新的绕过脚本","加固方案"], answer:0, explain:"逆向服务于风险判定与修复，最终要指向服务端加固。" },
  { id:"mz17", topic:"mob-mdm", cat:"mobile", level:"中级", q:"MDM 的「远程擦除企业数据」与「擦除整机」的关键区别是？", options:["前者只清企业数据容器，不影响员工个人数据","前者速度更快","后者无法执行","两者完全相同"], answer:0, explain:"区分企业数据与个人数据是 MDM 可被员工接受的前提。" },

  // ---- 补齐题量（v1.5.1 第九批：达成每知识点 ≥3 题）----
  { id:"dt1", topic:"ds-classify", cat:"datasec", level:"入门", q:"数据分类分级后，控制措施应当？", options:["与等级绑定（等级越高，加密/审批/审计越严）","所有等级一视同仁","只做文档记录即可","只对机密级做任何处理"], answer:0, explain:"分级的意义就是让投入与风险匹配；否则分级只是纸面工作。" },
  { id:"dt2", topic:"ds-classify", cat:"datasec", level:"初级", q:"分类分级规则上线后，最需要持续关注的是？", options:["识别准确率与漏报（如备注字段里混入手机号）","规则条数","界面美观度","编写规则的人数"], answer:0, explain:"漏报会让敏感数据逃过分级，需要抽样验证并迭代规则。" },
  { id:"dt3", topic:"ds-dlp", cat:"datasec", level:"中级", q:"DLP 观察期的主要产出是？", options:["真实的敏感数据流向与误报统计，用于制定收策略","拦截数量","审计报告页数","员工排名"], answer:0, explain:"先看清现状再收紧，否则会引发业务绕过。" },
  { id:"dt4", topic:"ds-mask", cat:"datasec", level:"初级", q:"脱敏后仍保留「后四位」的做法，适用于？", options:["业务需要核对但不需要完整值的场景（如客服核验）","需要完整还原的场景","公开披露场景","所有场景"], answer:0, explain:"保留少量片段兼顾可用性与降低暴露，但仍属个人信息需受控。" },
  { id:"dt5", topic:"ds-encrypt", cat:"datasec", level:"中级", q:"密钥轮换的主要目的是？", options:["限制密钥泄露后的影响窗口，并满足合规要求","提升加解密速度","减小密钥体积","减少备份数量"], answer:0, explain:"轮换要能不停机（新旧并存过渡），否则业务会抗拒执行。" },
  { id:"dt6", topic:"ds-leak-scene", cat:"datasec", level:"初级", q:"分析「有权限但从不使用」的授权，价值在于？", options:["识别并回收冗余权限，缩小潜在影响面","提升查询速度","减少日志量","节省存储"], answer:0, explain:"权限只增不减会形成隐性高权限，是数据泄露的放大器。" },
  { id:"dt7", topic:"ds-audit", cat:"datasec", level:"中级", q:"审计日志自身也需要保护，原因是？", options:["攻击者常优先篡改或删除日志以掩盖痕迹","日志体积太大","日志影响性能","日志格式不统一"], answer:0, explain:"集中收集、只追加与完整性校验是审计可信的前提。" },
  { id:"dt8", topic:"ds-pii", cat:"datasec", level:"入门", q:"合规上「单独同意」通常适用于哪些场景？", options:["处理敏感个人信息或向第三方提供个人信息","记录访问日志","系统性能监控","内部代码评审"], answer:0, explain:"这些场景危害更高，需要单独告知并取得明确同意。" },
  { id:"dt9", topic:"ds-lifecycle", cat:"datasec", level:"初级", q:"判断某个字段是否属于「最小必要」采集，关键看？", options:["该字段是否为实现已声明功能所必需","字段名是否常见","字段长度","存储成本"], answer:0, explain:"无关字段一律不采，是最省成本的隐私保护方式。" },
  { id:"dt10", topic:"ds-cross-border", cat:"datasec", level:"高级", q:"数据出境合规中，「本地化」为什么常被优先考虑？", options:["能直接规避跨境传输的合规与风险，实现成本可能更低","技术上更先进","性能更好","更便宜"], answer:0, explain:"能不出境就不出境，是风险最低的路径；确实要出境再选评估/合同/认证。" },
  { id:"dt11", topic:"ds-leak-response", cat:"datasec", level:"高级", q:"泄露事件对外通报时最需要避免的是？", options:["在通报中再次暴露不必要的个人信息（二次泄露）","说明已采取的措施","提供咨询通道","说明影响范围"], answer:0, explain:"通报内容要控制细节颗粒度，避免二次伤害与可被利用的信息。" },
  { id:"dt12", topic:"ds-insider", cat:"datasec", level:"中级", q:"对高敏数据的访问做「双人复核」的意义是？", options:["用流程约束抑制单人作恶与误操作","提升访问速度","减少日志量","降低存储成本"], answer:0, explain:"技术手段之外，流程制衡是内部威胁防护的重要一环。" },
  { id:"dt13", topic:"ds-backup", cat:"datasec", level:"高级", q:"备份系统自身成为攻击目标的原因是？", options:["拿到备份权限即可删除或加密所有副本，破坏恢复能力","备份数据量最大","备份耗时长","备份设备便宜"], answer:0, explain:"因此备份账号要与生产隔离，并保留不可变或离线副本。" },
  { id:"dt14", topic:"ds-pii", cat:"datasec", level:"入门", q:"「匿名化」与「假名化」的关键区别是？", options:["匿名化不可还原，假名化借助映射仍可还原","匿名化更便宜","假名化不需要加密","两者完全相同"], answer:0, explain:"假名化数据仍属个人信息受保护，匿名化则在满足条件后不再属于个人信息。" },
  ...
  { id:"dt16", topic:"ds-gateway", cat:"datasec", level:"中级", q:"数据网关若成为单点，应该？", options:["做高可用与旁路只读副本，并监控其自身的安全事件","取消网关直接连库","降低脱敏强度","关闭审计"], answer:0, explain:"收敛入口带来管理与风险集中，必须同步解决可用性与自身安全。" },
  { id:"dt17", topic:"ds-tokenize", cat:"datasec", level:"高级", q:"令牌化方案中最需要严格保护的是？", options:["令牌与原值的映射表（泄露即可还原全部数据）","令牌长度","令牌生成速度","令牌数量"], answer:0, explain:"映射表是令牌化体系的信任根，需独立加密、最小访问、全量审计。" },
  { id:"sv1", topic:"sc-concept", cat:"supply", level:"入门", q:"供应链攻击「一次得手、多点生效」的原因是？", options:["被污染的组件或更新通道会被大量下游复用","攻击者数量多","网络带宽大","用户不更新系统"], answer:0, explain:"复用性带来放大效应，这也是供应链攻击效率高的原因。" },
  { id:"sv2", topic:"sc-sbom", cat:"supply", level:"初级", q:"SBOM 与制品绑定的意义是？", options:["确保清单对应的是这个具体产物，而不是笼统的项目","便于压缩体积","减少构建时间","降低扫描成本"], answer:0, explain:"清单与产物脱钩会导致「查到的组件不是实际运行的组件」。" },
  { id:"sv3", topic:"sc-dependency", cat:"supply", level:"初级", q:"依赖漏洞治理中「临时缓解」适用于？", options:["短期内无法升级时的补偿措施（如加 WAF 规则或关闭功能）","替代所有升级","永久方案","不需要评估"], answer:0, explain:"缓解只是过渡，仍需在期限内完成升级或替换。" },
  { id:"sv4", topic:"sc-sca", cat:"supply", level:"中级", q:"SCA 扫描结果里最需要人工判定的是？", options:["漏洞是否可达（是否真的调用了受影响代码）","漏洞编号长短","扫描耗时","依赖包体积"], answer:0, explain:"可达性判定决定修复优先级，纯按数量排序会浪费大量精力。" },
  { id:"sv5", topic:"sc-concept", cat:"supply", level:"中级", q:"禁止安装脚本（如 npm 的 ignore-scripts）能降低什么风险？", options:["依赖安装时执行恶意代码的风险","下载速度慢的风险","版本冲突风险","许可证风险"], answer:0, explain:"安装期执行是投毒常见手段，禁用后需评估对构建的影响。" },
  { id:"sv6", topic:"sc-concept", cat:"supply", level:"高级", q:"构建环境「一次性」的意义是？", options:["每次构建从干净环境开始，避免残留污染被带入产物","节省成本","加快构建","减少日志"], answer:0, explain:"持久化构建机容易被植入后门或缓存投毒，污染会被固化进产物。" },
  { id:"sv7", topic:"sc-signing", cat:"supply", level:"高级", q:"制品签名验证失败的正确处理是？", options:["拒绝部署并排查（可能是篡改或签名流程问题）","忽略警告继续部署","仅记录日志","重新生成签名"], answer:0, explain:"验签失败意味着完整性存疑，必须先查清原因。" },
  { id:"sv8", topic:"sc-sca", cat:"supply", level:"中级", q:"CI/CD 中被外部输入触发的任务，最需要注意？", options:["外部输入不得参与脚本拼接执行，只能作为数据传入","任务运行时间","日志格式","构建产物大小"], answer:0, explain:"表达式注入是流水线常被忽视的高危点，需避免拼接执行。" },
  { id:"sv9", topic:"sc-secrets", cat:"supply", level:"初级", q:"密钥扫描为什么要放在提交前和 CI 两道？", options:["本地拦截减少泄露概率，CI 兜底防止绕过或漏检","为了统计数量","为了提升构建速度","为了减少仓库体积"], answer:0, explain:"两道拦截互补，任一环节都可能被绕过或误判。" },
  ...
  { id:"sv11", topic:"sc-license", cat:"supply", level:"初级", q:"供应商安全事件通报时限写进合同的意义是？", options:["保证我方能在可控时间内启动应急与告知下游","便于考核供应商","减少沟通成本","满足审计形式要求"], answer:0, explain:"通报时限直接决定你能不能及时止损与履行自身合规义务。" },
  { id:"sv12", topic:"sc-attack", cat:"supply", level:"入门", q:"Log4Shell 事件中最缺的能力是什么？", options:["快速判定「哪些系统用了这个组件」的依赖台账能力","更强的防火墙","更多的运维人员","更高的带宽"], answer:0, explain:"没有台账就只能在恐慌中逐个排查，时间成本极高。" },
  ...
  { id:"sv14", topic:"sc-concept", cat:"supply", level:"入门", q:"「软件供应链」的组成不包括？", options:["办公区绿植与照明","开源依赖库","构建工具与流水线","制品仓库与分发通道"], answer:0, explain:"供应链指构成与分发软件的组件与链路，与办公设施无关。" },
  { id:"sv15", topic:"sc-what", cat:"supply", level:"入门", q:"开源组件引入评审中，询问「是否有健康替代品」的目的是？", options:["为将来替换或弃维护做准备，降低长期风险","为了砍价","为了减少代码量","为了统一技术栈"], answer:0, explain:"替代方案是应对弃维护与高危漏洞的退路，应提前评估。" },
  { id:"sv16", topic:"sc-artifact", cat:"supply", level:"中级", q:"镜像准入控制通常部署在哪一层？", options:["集群的准入控制器（部署前校验签名与漏洞策略）","开发者的本地机器","代码仓库","邮件网关"], answer:0, explain:"准入控制在部署环节强制策略，能把不可信制品挡在生产之外。" },
  { id:"sv17", topic:"sc-monitor", cat:"supply", level:"中级", q:"依赖变化告警中「同一包短时间频繁发版」为何值得警惕？", options:["可能是攻击者在试错或恶意版本反复调整","说明维护者勤快","说明社区活跃","说明兼容性好"], answer:0, explain:"异常发版节奏是投毒与账号被盗的常见信号，需结合来源与内容核对。" },
  { id:"aix1", topic:"aisec-data-leak", cat:"aisec", level:"入门", q:"LLM 应用安全评估的三个主要层次是？", options:["输入、数据（上下文/知识库）、动作（工具调用）","前端、后端、数据库","开发、测试、生产","网络、主机、应用"], answer:0, explain:"三层分别对应提示注入、数据泄露与越权动作，覆盖了 LLM 的主要风险面。" },
  { id:"aix2", topic:"aisec-prompt-injection", cat:"aisec", level:"初级", q:"间接提示注入最常出现的载体是？", options:["模型会读取的外部内容（文档、网页、邮件、工单）","用户直接输入的问题","系统提示词本身","模型权重文件"], answer:0, explain:"外部内容对模型可读即可写，因此是注入的主要入口。" },
  { id:"aix3", topic:"aisec-jailbreak", cat:"aisec", level:"中级", q:"越狱防护中，「限制后果」比「阻止绕过」更重要的原因是？", options:["绕过难以彻底阻止，但可以通过权限与确认限制最坏结果","限制后果更省事","阻止绕过不可能","限制后果更便宜"], answer:0, explain:"把风险控制落在不可逆动作上，是更现实的防护思路。" },
  { id:"aix4", topic:"ai-prompt-eng", cat:"aisec", level:"中级", q:"防止多租户数据串用的关键控制点是？", options:["检索与权限过滤由系统强制注入，不受模型或用户输入影响","提示词里写明不允许","只允许内网访问","关闭流式输出"], answer:0, explain:"安全边界必须由代码强制，提示词不能作为隔离手段。" },
  { id:"aix5", topic:"aisec-rag-risks", cat:"aisec", level:"中级", q:"知识库投毒的防御重点在？", options:["入库审核与写入权限控制，以及检索结果的降权处理","提升检索速度","增大向量维度","增加存储"], answer:0, explain:"先管住「谁能写进去」，再把检索内容当作数据而非指令。" },
  { id:"aix6", topic:"aisec-agent-tools", cat:"aisec", level:"高级", q:"Agent 工具分级中，属于「破坏型」的是？", options:["删除数据、支付、修改权限","查询文档","读取日志","生成报告"], answer:0, explain:"破坏型动作不可逆，必须人工确认甚至双人复核。" },
  { id:"aix7", topic:"aisec-output-handling", cat:"aisec", level:"初级", q:"把模型输出写入数据库时最需要防止的是？", options:["注入类风险（拼接执行导致的 SQL/命令注入）","字段长度超限","编码不一致","字符集问题"], answer:0, explain:"模型输出等同不可信输入，必须参数化与校验。" },
  ...
  { id:"aix9", topic:"aisec-privacy", cat:"aisec", level:"初级", q:"把真实业务数据发给外部模型前，最稳妥的做法是？", options:["先脱敏或用占位符替代，并确认供应商不留存不训练","直接发送以求效果","只发一部分即可","只要加密传输就没问题"], answer:0, explain:"数据一旦发出就不再受你控制，源头最小化最有效。" },
  { id:"aix10", topic:"aisec-jailbreak", cat:"aisec", level:"中级", q:"四层护栏中「能力收敛」指的是？", options:["限制模型可调用的工具与其权限范围","限制用户提问长度","限制输出字数","限制并发请求数"], answer:0, explain:"能力收敛直接决定一次被绕过能造成多大影响。" },
  { id:"aix11", topic:"aisec-eval", cat:"aisec", level:"高级", q:"红队评估中，「影响判定」为什么比「漏洞数量」更重要？", options:["决定修复优先级与资源投入，避免把精力花在无实际影响的问题上","便于统计","因为客户要求","因为报告需要"], answer:0, explain:"安全评估服务于风险管理，影响面决定优先级。" },
  { id:"aix12", topic:"aisec-cost-dos", cat:"aisec", level:"中级", q:"LLM 应用的成本型 DoS 最直接的防护是？", options:["入口限长限频并对 Agent 循环设轮次上限","提高模型参数","增加服务器","减少日志"], answer:0, explain:"限制单次与单位时间的资源消耗，是控制成本风险的基本手段。" },
  { id:"aix13", topic:"aisec-governance", cat:"aisec", level:"入门", q:"AI 治理中，为什么提示词变更要走评审？", options:["它等价于生产配置，改动可能改变安全边界与合规行为","为了流程合规好看","为了记录工作量","为了限制开发速度"], answer:0, explain:"提示词影响模型行为与工具调用，必须像代码一样受控。" },
  { id:"aix14", topic:"ai-how", cat:"aisec", level:"入门", q:"了解「token 与上下文窗口」对安全的实际意义是？", options:["理解上下文长度限制与截断风险，以及注入内容如何进入上下文","为了调参优化效果","为了节省费用","为了选择模型"], answer:0, explain:"上下文是攻击者的目标区域，理解其机制才能设计隔离与限长策略。" },
  ...
  { id:"aix16", topic:"ai-prompt-eng", cat:"aisec", level:"中级", q:"把提示词与安全职责分离的判断标准是？", options:["删掉提示词后系统是否仍然安全","提示词是否够长","是否用了英文","是否包含示例"], answer:0, explain:"安全必须由代码层保证，提示词只承担体验与风格。" },
  { id:"aix17", topic:"aisec-agent-tools", cat:"aisec", level:"中级", q:"AI 应用审计中，工具调用记录至少要包含？", options:["工具名、参数、执行结果与是否经人工确认","只有调用时间","只有工具名","只有耗时"], answer:0, explain:"参数与确认记录决定了能否复盘「模型建议什么、最终执行什么」。" },
  { id:"st1", topic:"soc-basics", cat:"soccti", level:"入门", q:"给安全团队用的「运营情报」主要解决？", options:["日常值班重点与专项排查方向","预算审批","设备采购","组织架构调整"], answer:0, explain:"不同层级情报服务不同决策，错配会导致「情报没人用」。" },
  { id:"st2", topic:"soc-ioc-ioa", cat:"soccti", level:"初级", q:"IOC 生命周期的最后一步是？", options:["到期失效与下架（避免长期误伤）","持续封禁","提高优先级","转成 TTP"], answer:0, explain:"共享 IP 与域名会易主，长期保留旧 IOC 会造成误伤。" },
  { id:"st3", topic:"soc-soc-ops", cat:"soccti", level:"初级", q:"SOC 值夜班时最重要的支撑是？", options:["清晰的升级路径与可联系上的人","更多仪表盘","更快的机器","更多告警源"], answer:0, explain:"夜间资源少，升级路径是否顺畅直接决定响应时效。" },
  { id:"st4", topic:"soc-detection", cat:"soccti", level:"中级", q:"检测规则「可维护」的含义包括？", options:["有测试样本、误报说明与负责人，能随环境演进更新","规则语法正确","规则条数多","规则写在文档里"], answer:0, explain:"没有测试与责任人的规则会逐渐腐化，最终无人敢动。" },
  { id:"st5", topic:"soc-alert", cat:"soccti", level:"中级", q:"研判时判断「已知良性行为」的主要依据是？", options:["变更记录、扫描器/备份等工具行为清单","告警等级","来源 IP 归属地","处理人经验"], answer:0, explain:"对齐合法行为白名单是最有效的降噪手段。" },
  { id:"st6", topic:"soc-mitre", cat:"soccti", level:"高级", q:"狩猎时选择切入点应优先考虑？", options:["高价值风险（凭据滥用、横向移动、持久化）与数据可得性","最容易查的技术","最新的技术","随机选"], answer:0, explain:"优先覆盖影响大且能验证的风险，收益最高。" },
  { id:"st7", topic:"soc-ir-flow", cat:"soccti", level:"中级", q:"应急响应中「根除」阶段的验证要点是？", options:["确认入口与所有持久化后门均已清除，而非仅删除恶意文件","重启服务器","更换密码","通知领导"], answer:0, explain:"根除不彻底会导致攻击者利用残留后门重新进入。" },
  { id:"st8", topic:"soc-forensics", cat:"soccti", level:"高级", q:"取证时先做内存镜像的原因是？", options:["内存数据断电即失，包含密钥、进程与网络连接等线索","磁盘数据不重要","内存更容易分析","为了节省磁盘"], answer:0, explain:"内存是「活证据」，一旦重启就永久丢失。" },
  { id:"st9", topic:"soc-intel-priority", cat:"soccti", level:"初级", q:"ATT&CK 编号在事件复盘中的价值是？", options:["统一描述攻击者用了哪些技术，便于对齐检测与情报","便于写报告页码","便于统计字数","便于选择工具"], answer:0, explain:"共同语言让复盘结论能直接转化为检测改进项。" },
  { id:"st10", topic:"soc-ir-flow", cat:"soccti", level:"中级", q:"情报源「机读」属性为什么重要？", options:["可自动导入检测与处置流程，减少人工搬运","体积更小","更新更快","更准确"], answer:0, explain:"不可机读的情报只能当阅读材料，落地效率低。" },
  { id:"st11", topic:"soc-metrics", cat:"soccti", level:"中级", q:"若某指标鼓励「多关告警」，可能导致的后果是？", options:["为刷指标而草率关闭，真实威胁被漏掉","提升效率","降低误报","增加检测覆盖"], answer:0, explain:"指标设计不当会诱导错误行为，应关注准确率与漏检复盘。" },
  { id:"st12", topic:"soc-attribution", cat:"soccti", level:"高级", q:"归因结论标注「置信度」的作用是？", options:["表明证据强度，避免把低置信推断当作事实使用","便于排版","便于统计","便于对外沟通"], answer:0, explain:"归因存在假旗与误判风险，置信度让结论被恰当地使用。" },
  { id:"st13", topic:"soc-baseline", cat:"soccti", level:"入门", q:"日志集中化时，「只追加」能带来的安全价值是？", options:["攻击者无法通过修改历史掩盖痕迹，只能新增记录","节省存储","加快查询","减少带宽"], answer:0, explain:"只追加与权限隔离让日志具备抗篡改能力。" },
  { id:"st14", topic:"soc-team", cat:"soccti", level:"入门", q:"安全团队分工时，明确「告警归属」的目的是？", options:["避免问题在交接中流失（都以为别人在看）","便于统计工作量","便于排班","便于考核"], answer:0, explain:"归属清晰是闭环的前提，否则会出现无人处理的告警。" },
  { id:"st15", topic:"soc-alert", cat:"soccti", level:"入门", q:"收到告警后，「有成功迹象」通常指？", options:["登录成功、文件落地、数据外发、命令执行成功等","告警数量变多","来源 IP 变化","时间在深夜"], answer:0, explain:"成功迹象意味着攻防已经从尝试进入实现阶段，优先级最高。" },
  { id:"st16", topic:"soc-tabletop", cat:"soccti", level:"高级", q:"桌面推演故意设置「关键联系人休假」这类卡点，目的是？", options:["检验预案在真实不利条件下是否仍然可用","增加难度系数","为了让演练更精彩","为了考核个人"], answer:0, explain:"真实事件常发生在最不方便的时刻，预案必须对此有安排。" },
  { id:"st17", topic:"soc-detection", cat:"soccti", level:"中级", q:"把情报转成检测假设后，验证方式是？", options:["用模拟动作或历史数据回归，确认规则真能检出","等真实攻击来验证","只做文档记录","通知情报源"], answer:0, explain:"未验证的规则等于假设未闭环，可能长期无效而不自知。" },
  { id:"bz5", topic:"mitigations", cat:"binary", level:"中级", q:"在开启 ASLR 的环境中，利用通常还需要？", options:["先泄露一个地址（信息泄露）来推算基址","关闭防火墙","使用更长的 payload","改用 Windows"], answer:0, explain:"ASLR 让地址随机化，信息泄露是绕过它的常见前置条件。" },

  // ---- v1.5.3 批次 1（套餐 D）：24 知识点 + 72 题 ----
  { id:"nd1", topic:"net-dns-sec", cat:"network", level:"中级", q:"部署伪造 DHCP 服务器最直接的危害是？", options:["把网关与 DNS 指向攻击者，实现中间人劫持","导致交换机重启","提升带宽占用","使域名无法注册"], answer:0, explain:"客户端不校验 DHCP 来源，谁先应答就听谁，从而接管流量路径与解析。" },
  { id:"nd2", topic:"net-dhcp", cat:"network", level:"中级", q:"抑制私接 DHCP 服务器最有效的手段是？", options:["交换机开启 DHCP Snooping，只信任上联口","提高口令复杂度","关闭 ICMP","启用 HTTPS"], answer:0, explain:"二层准入控制从源头拦住伪造应答；终端侧无法自行防御。" },
  { id:"nd3", topic:"net-dns-sec", cat:"network", level:"中级", q:"排查「客户端拿到异常网关」时，最有价值的证据是？", options:["DHCP 租约与交换机端口的 DHCP 日志","浏览器缓存","DNS 查询次数","磁盘使用率"], answer:0, explain:"租约与端口日志能定位到是哪个接口在分发异常配置。" },
  { id:"nb9", topic:"net-bastion", cat:"network", level:"初级", q:"堡垒机的核心价值是？", options:["把运维访问收敛到统一通道做认证、授权与审计","提升服务器性能","加密全部内网流量","替代防火墙"], answer:0, explain:"收敛入口才能实现「一人一号、全程留痕、按需授权」。" },
  { id:"nb10", topic:"net-bastion", cat:"network", level:"初级", q:"要让堡垒机真正生效，必须同时做到？", options:["关掉终端直连生产的路径（安全组只放行堡垒机）","只安装堡垒机软件","只配置录屏","只做双因素"], answer:0, explain:"存在旁路直连时，所有管控都可被绕过。" },
  { id:"nb11", topic:"net-proxy", cat:"network", level:"中级", q:"服务器出网统一走代理的主要目的是？", options:["可审计、可拦截、可限制，降低数据外传与 C2 风险","提升访问速度","节省带宽费用","简化 DNS 配置"], answer:0, explain:"统一出口让出网行为可见可控，是外传与 C2 检测的基础。" },
  { id:"nb12", topic:"net-proxy", cat:"network", level:"中级", q:"仅按域名白名单做出网管控，可能被什么绕过？", options:["直连 IP 或经 DNS 外带数据","增加并发连接","使用 HTTPS","降低发包频率"], answer:0, explain:"因此还需封禁直连公网并管住 DNS，形成多层约束。" },
  { id:"nv1", topic:"net-ipv6", cat:"network", level:"高级", q:"双栈环境中 IPv6 常成为风险盲区，原因是？", options:["安全策略与日志往往只覆盖 IPv4","IPv6 无法被扫描","IPv6 不支持加密","IPv6 地址不固定"], answer:0, explain:"策略未覆盖等于开了一扇无人管理的门，需与 IPv4 一致管控。" },
  { id:"nv2", topic:"net-tunnel", cat:"network", level:"高级", q:"下列哪项可能绕过基于 IPv4 的策略控制？", options:["6to4/Teredo 等隧道机制","启用 HTTPS","使用私有地址","增大 MTU"], answer:0, explain:"隧道会把 IPv6 流量封装进 IPv4 传输，绕过原有策略边界。" },
  { id:"nv3", topic:"net-ipv6", cat:"network", level:"高级", q:"IPv6 下 NAT 遮蔽作用消失，意味着？", options:["内网地址可被直接访问，暴露面管理必须显式化","网络性能下降","必须禁用 IPv6","不再需要防火墙"], answer:0, explain:"失去隐式遮蔽后，只能靠显式的策略最小放通来管理暴露面。" },
  { id:"nz14", topic:"net-segment", cat:"network", level:"高级", q:"零信任与传统 VPN 的核心区别是？", options:["按应用粒度持续校验身份与设备，而不是进网即信任","完全不使用加密","只使用口令认证","取消了网络分区"], answer:0, explain:"零信任把「位置信任」替换为「每次访问都校验」，显著缩小可达范围。" },
  { id:"nz10", topic:"net-ztna", cat:"network", level:"高级", q:"ZTNA 落地最容易退化的地方是？", options:["策略膨胀最终变回「全放通」","加密强度不足","客户端体积过大","日志格式不统一"], answer:0, explain:"因此需要定期复核策略与变更留痕，避免「名义零信任、实际全放通」。" },
  { id:"nz11", topic:"net-bastion", cat:"network", level:"中级", q:"安全组只放行堡垒机来源后，仍需防范的是？", options:["堡垒机自身被攻陷后成为横向跳板","终端无法登录服务器","运维效率下降","日志量增加"], answer:0, explain:"堡垒机是高价值单点，需重点加固并限制其可达范围。" },
  { id:"nz12", topic:"net-monitor", cat:"network", level:"中级", q:"出网日志对检测的价值体现在？", options:["能发现异常目的地、非工作时段大流量上传与少见协议","提升解析速度","减少带宽消耗","加密流量内容"], answer:0, explain:"出网行为基线是发现 C2 与数据外传的有效手段。" },
  ...
  { id:"cc1", topic:"cloud-secret", cat:"cloud", level:"中级", q:"CSPM 的核心作用是？", options:["持续检查云上配置是否偏离安全基线并给出治理优先级","替代漏洞扫描","替代防火墙","管理计费"], answer:0, explain:"云上配置即安全边界，持续比对基线与实际是关键动作。" },
  { id:"cc2", topic:"cloud-network", cat:"cloud", level:"中级", q:"下列哪项属于 CSPM 的典型检查项？", options:["对象存储是否对外公开、安全组是否放通 0.0.0.0/0 管理端口","实例的 CPU 使用率","磁盘 IOPS","镜像构建时长"], answer:0, explain:"CSPM 关注配置类风险（暴露面、权限、日志开关），而非性能指标。" },
  { id:"cc3", topic:"cloud-vuln", cat:"cloud", level:"初级", q:"云上漏洞优先修复顺序应优先考虑？", options:["公网可达 + 核心数据 + 有公开利用的漏洞","漏洞编号大小","扫描耗时","修复难度"], answer:0, explain:"结合暴露面与资产重要性排序，才能把有限人力用在真实风险上。" },
  { id:"cc4", topic:"cloud-vuln", cat:"cloud", level:"初级", q:"漏洞清单要与什么数据合并才有意义？", options:["资产台账与暴露面清单","计费账单","镜像体积","日志条数"], answer:0, explain:"不知道漏洞落在哪个业务与是否对外，就无法判断优先级。" },
  { id:"cc5", topic:"cloud-iac", cat:"cloud", level:"中级", q:"IaC 安全的最大价值是？", options:["配置变更可评审、可回溯，高危配置能在合并前被拦截","提升部署速度","减少云成本","简化监控"], answer:0, explain:"把配置纳入代码流程后，安全基线才能前移到变更入口。" },
  { id:"cc6", topic:"cloud-iac", cat:"cloud", level:"中级", q:"IaC 实践中必须特别注意保护的是？", options:["state 文件（常含敏感信息）与其中的密钥","README 文档","变量命名","目录结构"], answer:0, explain:"state 记录资源与属性，泄露可能导致凭据与架构暴露。" },
  { id:"cc7", topic:"cloud-iac", cat:"cloud", level:"中级", q:"「配置漂移」的治理方式是？", options:["定期比对代码声明与实际配置并收敛差异","关闭审计日志","增加巡检人员","提高扫描频率"], answer:0, explain:"漂移多来自手工临时修改，需自动比对与回收。" },
  { id:"clt1", topic:"cloud-tenant", cat:"cloud", level:"高级", q:"多租户系统中最容易发生跨租户泄露的层是？", options:["数据层（数据库、检索、缓存、队列）缺少强制租户过滤","前端界面","日志系统","DNS 解析"], answer:0, explain:"只要有一处查询漏掉租户条件，就可能读出他人数据。" },
  ...
  { id:"clt3", topic:"cloud-tenant", cat:"cloud", level:"高级", q:"多租户隔离通常需要覆盖哪几层？", options:["网络、身份、数据与运行环境","界面、报表、导出与打印","开发、测试、预发与生产","日志、指标、追踪与告警"], answer:0, explain:"四层缺一即可被绕过，其中数据层强制过滤最关键。" },
  { id:"cd1", topic:"cloud-dr", cat:"cloud", level:"高级", q:"灾备指标 RTO 指的是？", options:["业务恢复所需的时间","能容忍的数据丢失量","备份成功率","演练次数"], answer:0, explain:"RTO 管恢复速度、RPO 管数据丢失容忍度，二者都要书面明确并演练验证。" },
  { id:"cd2", topic:"cloud-tenant", cat:"cloud", level:"高级", q:"为什么备份需要「不可变或离线」副本？", options:["勒索攻击会优先破坏在线备份，不可变副本才能保证可恢复","节省存储","提升备份速度","减少备份数量"], answer:0, explain:"没有隔离副本时，一次入侵就可能同时毁掉数据与备份。" },
  { id:"cd3", topic:"cloud-dr", cat:"cloud", level:"高级", q:"云上灾备最容易忽视的单点是？", options:["依赖项（DNS、密钥服务、外部接口）","实例规格","镜像大小","日志条数"], answer:0, explain:"核心系统恢复后若依赖不可用，业务仍然起不来。" },
  { id:"cd4", topic:"cloud-image-govern", cat:"cloud", level:"中级", q:"CSPM 要与哪些数据联动才有实际价值？", options:["资产、身份、漏洞与 IaC 扫描结果","计费与容量","镜像仓库大小","开发排期"], answer:0, explain:"只有能回答「谁受影响、谁能改、改没改」，配置告警才可运营。" },
  ...
  { id:"bt1", topic:"ids", cat:"blue", level:"中级", q:"告警降噪的第一步应该是？", options:["统计哪几条规则贡献了大部分告警量","直接关闭所有低危规则","增加值班人数","购买新设备"], answer:0, explain:"先量化再治理，避免凭感觉调整导致漏检。" },
  { id:"bt2", topic:"ids", cat:"blue", level:"中级", q:"对「规则本身没问题但缺上下文」导致的告警，正确处理是？", options:["补上下文（资产、账号、历史行为）而不是关规则","直接关闭该规则","提高阈值","删除样本"], answer:0, explain:"补上下文能让原本噪音的规则变成有效告警，关掉会损失检测能力。" },
  { id:"bt3", topic:"blue-alert-tuning", cat:"blue", level:"中级", q:"「承认（acknowledge）」类抑制与直接关闭规则的区别在于？", options:["承认有到期时间与责任人，属于临时处置","承认更彻底","两者完全相同","承认会删除规则"], answer:0, explain:"带到期的抑制能避免「临时规则永久化」造成检测盲区。" },
  { id:"bo1", topic:"blue-oncall", cat:"blue", level:"初级", q:"值班交接清单中必不可少的项是？", options:["未闭环事项与临时抑制规则的到期时间","个人休假安排","设备采购计划","预算余额"], answer:0, explain:"交接遗漏会导致告警无人处理或长期误抑制。" },
  { id:"bo2", topic:"blue-role", cat:"blue", level:"初级", q:"告警分级（P1-P4）的意义是？", options:["让响应时限与升级路径有明确依据","减少告警数量","提升检测精度","降低设备成本"], answer:0, explain:"分级是把有限人力投向最紧急事件的前提。" },
  { id:"bo3", topic:"blue-oncall", cat:"blue", level:"高级", q:"值班人员疲劳与漏判之间的关系是？", options:["疲劳是漏判的主要来源之一，需控制班次负荷与交接质量","疲劳不影响技术判断","只有白班需要关注","与流程设计无关"], answer:0, explain:"人的状态直接决定研判质量，排班与交接规范属于实质性的安全控制。" },
  { id:"be1", topic:"blue-evidence", cat:"blue", level:"中级", q:"处置工单中「判断依据」为什么必须记录？", options:["便于事后复核与复盘，避免结论无法追溯","便于统计字数","便于考核","便于归档"], answer:0, explain:"没有依据的结论无法复核，同期事件也难以沉淀经验。" },
  { id:"be2", topic:"blue-evidence", cat:"blue", level:"中级", q:"涉及主机处置时，先固定证据再动手的原因是？", options:["操作会破坏内存与运行状态证据，导致无法溯源","为了节省时间","因为流程要求","便于生成报告"], answer:0, explain:"内存与运行态证据一旦重启或清理就不可恢复。" },
  { id:"be3", topic:"blue-evidence", cat:"blue", level:"中级", q:"证据文件记录哈希的目的是？", options:["证明文件在保存与移交过程中未被篡改","压缩体积","加快读取","便于检索"], answer:0, explain:"哈希是证据完整性可验证的基础。" },
  { id:"bc1", topic:"blue-role", cat:"blue", level:"中级", q:"云上检测应优先接入的数据是？", options:["云操作审计、身份与权限变更、密钥使用","实例温度监控","磁盘碎片率","屏幕分辨率"], answer:0, explain:"云上攻击多发生在控制面，审计与身份数据最关键。" },
  { id:"bc2", topic:"blue-log", cat:"blue", level:"中级", q:"「关闭审计日志配置」这类操作需要重点告警的原因是？", options:["它是攻击者隐藏痕迹的前置动作","它会增加成本","它会影响业务","它需要重启服务"], answer:0, explain:"日志开关的变更往往是入侵后的第一步，必须高优先告警。" },
  { id:"bc3", topic:"blue-log", cat:"blue", level:"中级", q:"多账号云环境防止业务侧删除日志的做法是？", options:["把审计日志集中到独立账号并设为只追加","在每个账号本地保存","加密日志文件","只保留 7 天"], answer:0, explain:"集中且不可删，才能保证事后取证不受业务账号影响。" },
  { id:"bm1", topic:"blue-report", cat:"blue", level:"初级", q:"事件对外沟通中最重要的纪律是？", options:["只讲已确认的事实，明确区分待确认项","尽量少说话","只发一次通告","由技术负责人直接对媒体说明"], answer:0, explain:"猜测与过度承诺会在后续被推翻，反而放大信任损失。" },
  { id:"bm2", topic:"blue-comm", cat:"blue", level:"初级", q:"提前准备客服 FAQ 的价值是？", options:["一线能统一口径快速回应，避免信息混乱","减少客服人数","降低话务量","便于审计"], answer:0, explain:"事件中客服是用户感知的第一触点，口径混乱会放大恐慌。" },
  ...
  { id:"mc1", topic:"mob-crypto", cat:"mobile", level:"中级", q:"客户端密钥若可被导出，说明？", options:["加密保护基本失效，攻击者可解密全部本地数据","加密强度不足但仍安全","需要更长密钥","需要更换算法"], answer:0, explain:"可导出的密钥等于把钥匙一起交出去，应使用系统密钥库并禁止导出。" },
  { id:"mc2", topic:"mob-crypto", cat:"mobile", level:"中级", q:"客户端加密的防御上界是？", options:["设备被完全控制时无法保证，关键校验必须在服务端","等同于服务端防护","可完全替代服务端校验","取决于加密算法强度"], answer:0, explain:"客户端只是提高成本，安全判断不能依赖它。" },
  { id:"mc3", topic:"root-detect", cat:"mobile", level:"中级", q:"检查本地数据保护时，最容易被忽略的一处是？", options:["备份导出中是否包含明文敏感数据","应用图标分辨率","启动耗时","界面语言"], answer:0, explain:"备份通道常绕过数据保护策略，需单独核查。" },
  { id:"mo1", topic:"mob-obfuscate", cat:"mobile", level:"中级", q:"代码混淆的真实作用是？", options:["提高逆向成本，但不能阻止有经验的分析者","彻底防止逆向","替代服务端校验","加密网络流量"], answer:0, explain:"混淆是成本博弈，安全逻辑仍应放在服务端。" },
  { id:"mo2", topic:"mobile-api", cat:"mobile", level:"中级", q:"评估混淆效果最实际的判据是？", options:["关键逻辑（签名、风控）是否仍能被定位","类名长度","包体积","构建耗时"], answer:0, explain:"混淆的价值体现在关键逻辑的可分析难度上。" },
  { id:"mo3", topic:"app-hardening", cat:"mobile", level:"高级", q:"面对强混淆与加固，安全投入更合理的分配是？", options:["把重点放在服务端校验与风控，客户端只做成本提升","无限加大混淆强度","放弃客户端保护","依赖应用商店审核"], answer:0, explain:"客户端防护收益递减，服务端才是可控的信任边界。" },
  { id:"ms1", topic:"mob-permission", cat:"mobile", level:"初级", q:"应用上架被退回的常见原因是？", options:["申请的权限与功能不匹配、隐私政策与实际采集不一致","应用体积过大","图标不够美观","启动速度慢"], answer:0, explain:"权限、功能、政策三者必须一致，第三方 SDK 采集也要如实声明。" },
  { id:"ms2", topic:"mob-store", cat:"mobile", level:"初级", q:"上架前的「三方核对」指？", options:["权限声明、实际功能与隐私政策三者一致","开发、测试与运维三方确认","三家应用商店同时提交","三种机型兼容测试"], answer:0, explain:"三者不一致是合规风险的主要来源，也是审核重点。" },
  { id:"ms3", topic:"mob-store", cat:"mobile", level:"高级", q:"把上架合规当成持续过程而非一次性动作的原因是？", options:["政策、SDK 与地区要求会变化，需纳入发版流程定期复核","审核流程太长","商店规则不公开","用户会投诉"], answer:0, explain:"合规要求持续演进，临时补材料必然滞后。" },
  { id:"dm1", topic:"ds-datamap", cat:"datasec", level:"中级", q:"数据资产地图的最小可用内容应包括？", options:["数据位置、字段含义、等级、责任人与访问者","服务器型号与机柜位置","开发人员名单","备份软件版本"], answer:0, explain:"地图要能回答「数据在哪、谁负责、谁能访问」，才能驱动后续治理。" },
  { id:"dm2", topic:"ds-datamap", cat:"datasec", level:"中级", q:"数据血缘最直接的价值是？", options:["出事时能快速圈定受影响的下游范围","提升查询性能","减少存储占用","方便做报表美化"], answer:0, explain:"血缘把「影响面判定」从人工排查变成可查询的路径。" },
  ...
  { id:"dh1", topic:"ds-encrypt", cat:"datasec", level:"中级", q:"向合作方提供数据时，「能聚合就不给明细」体现了？", options:["最小必要原则：只提供实现目的所需的最小粒度","数据脱敏技术","加密传输要求","存储优化策略"], answer:0, explain:"粒度最小化能显著降低泄露与再识别风险。" },
  { id:"dh2", topic:"ds-dlp", cat:"datasec", level:"中级", q:"对外数据接口的必备控制是？", options:["鉴权、限流与字段白名单","CAPTCHA","静态资源缓存","界面水印"], answer:0, explain:"这三项分别对应「谁能调、能调多少、能取哪些字段」。" },
  { id:"dh3", topic:"ds-tokenize", cat:"datasec", level:"高级", q:"数据共享中「组合风险」指的是？", options:["对方将本批数据与他方数据结合后可能产生新的识别能力","数据量过大","格式不统一","传输延迟"], answer:0, explain:"因此要在给之前评估组合后的可识别性，并做相应脱敏。" },
  { id:"da1", topic:"ds-tokenize", cat:"datasec", level:"高级", q:"使用真实数据训练模型前，最应先确认的是？", options:["原始授权是否覆盖「训练模型」这一用途","数据量是否足够","字段是否整齐","算力是否充足"], answer:0, explain:"用途超出原授权范围会直接构成违规，需重新取得同意或改用脱敏数据。" },
  { id:"da2", topic:"ds-ai-data", cat:"datasec", level:"高级", q:"「从模型中删除某条训练数据」在技术上难以实现的含义是？", options:["应在源头把关数据准入，而不是指望事后删除","模型不会被泄露","删除功能没必要","只需重训一次即可"], answer:0, explain:"模型可能记住训练数据，事后删除极难，源头控制更现实。" },
  { id:"da3", topic:"ds-pii", cat:"datasec", level:"中级", q:"训练数据优先使用脱敏或合成数据的原因是？", options:["在满足训练效果的同时降低个人信息暴露与合规风险","合成数据更便宜","脱敏能提升准确率","真实数据不可用"], answer:0, explain:"降低敏感度是合规与安全的最优解，但需评估对效果的影响。" },
  { id:"sl1", topic:"sc-license", cat:"supply", level:"初级", q:"开源「宽松许可证」通常的主要义务是？", options:["保留版权与许可声明","必须开源自己的代码","禁止商业使用","必须支付费用"], answer:0, explain:"MIT/Apache/BSD 类以保留声明为主，传染性许可才涉及开源衍生作品。" },
  { id:"sl2", topic:"sc-license", cat:"supply", level:"初级", q:"引入 GPL 类组件时最需要评估的是？", options:["其传染性是否与产品分发方式兼容","代码行数","Star 数量","文档语言"], answer:0, explain:"许可证不兼容可能导致被迫开源或法律风险。" },
  { id:"sl3", topic:"sc-license", cat:"supply", level:"高级", q:"AGPL 相比 GPL 的额外要求主要针对？", options:["以网络服务形式提供软件的场景","移动应用分发","嵌入式设备","内部使用"], answer:0, explain:"AGPL 把「通过网络提供服务」也纳入开源义务触发条件。" },
  { id:"su1", topic:"sc-sca", cat:"supply", level:"中级", q:"依赖升级「越拖越难升」的根本原因是？", options:["版本跨度变大后接口变更与兼容风险累积","依赖包体积增长","仓库带宽限制","许可证过期"], answer:0, explain:"小步持续升级能把风险摊薄，积攒大版本则一次跨越过多变更。" },
  { id:"su2", topic:"sc-upgrade", cat:"supply", level:"中级", q:"无法立即升级的高危依赖，应如何处理？", options:["记录原因与补偿控制并设定复核期限","从清单中删除","标记为已修复","忽略不管"], answer:0, explain:"显式登记与补偿控制比隐瞒更安全，也便于后续跟踪。" },
  { id:"su3", topic:"sc-artifact", cat:"supply", level:"中级", q:"把「依赖新鲜度」当作健康指标的意义是？", options:["可持续发现长期不升级的组件，避免漏洞债累积","提升构建速度","减少镜像体积","降低测试成本"], answer:0, explain:"指标化能让升级这件事从「想起来才做」变成常态化管理。" },
  { id:"sa1", topic:"sc-concept", cat:"supply", level:"中级", q:"引入第三方组件前的评审清单中，属于高风险信号的是？", options:["包含安装/构建脚本且来源不可信","文档使用英文","有单元测试","有 CI 配置"], answer:0, explain:"安装脚本在安装期执行代码，是投毒的常见载体。" },
  { id:"sa2", topic:"sc-typosquat", cat:"supply", level:"中级", q:"对加密、网络、系统操作类组件应重点审查什么？", options:["其权限与网络行为是否与声称功能相符","代码注释数量","变量命名风格","目录结构"], answer:0, explain:"能力越强的组件一旦被投毒，危害越大，需要更严格审查。" },
  ...
  { id:"aix18", topic:"aisec-rag-risks", cat:"aisec", level:"中级", q:"RAG 工程中，入库环节最该控制的是？", options:["谁能写入与内容是否经过审核","检索速度","向量维度","分块大小"], answer:0, explain:"知识库对模型是可信感很强的信息源，写入权限与审核是投毒防御的第一道门。" },
  { id:"aix19", topic:"aisec-output-handling", cat:"aisec", level:"中级", q:"检索结果交给模型时，正确的处理是？", options:["明确标注为「数据」并与指令分隔，降低被当作指令的概率","直接拼在系统提示后面","加密后再拼接","转成图片给模型"], answer:0, explain:"检索内容属不可信输入，结构化标注能显著降低间接注入成功率。" },
  { id:"aix20", topic:"aisec-rag-risks", cat:"aisec", level:"高级", q:"判断 RAG 回答是否可信，最实用的工程手段是？", options:["要求引用可溯源并能点回原文核对","提高温度参数","增大上下文窗口","换成更大的模型"], answer:0, explain:"引用溯源让用户能验证依据，也是发现投毒与幻觉的主要途径。" },
  { id:"aig1", topic:"ai-agent-guard", cat:"aisec", level:"高级", q:"Agent 护栏中「是否需要人工确认」应由谁决定？", options:["由服务端代码按动作等级决定，不能交给模型判断","由模型自行判断","由用户随口决定","由前端界面决定"], answer:0, explain:"模型判断可被诱导绕过，确认逻辑必须在不可被模型影响的一侧。" },
  { id:"aig2", topic:"aisec-agent-tools", cat:"aisec", level:"高级", q:"工具调用的「幂等」控制解决什么问题？", options:["防止重复调用造成重复副作用（如重复下单、重复发送）","提升调用速度","减少 token 消耗","降低模型幻觉"], answer:0, explain:"幂等与去重是限制副作用放大的关键工程手段。" },
  { id:"aig3", topic:"aisec-eval", cat:"aisec", level:"中级", q:"衡量护栏有效性应关注？", options:["拦截率与漏拦率，并用红队样本持续回归","拦截次数总量","代码行数","部署组件数量"], answer:0, explain:"只有可度量的护栏才能持续改进，也才能发现被绕过的情况。" },
  { id:"ail1", topic:"ai-label", cat:"aisec", level:"初级", q:"对外发布 AI 生成内容时，基本要求是？", options:["标注为 AI 生成/辅助并保留生成记录","完全不需要说明","只保留最终版本","由发布者口头说明"], answer:0, explain:"标识与留痕是合规与信任的基础，也便于事后追溯。" },
  { id:"ail2", topic:"ai-label", cat:"aisec", level:"初级", q:"把「AI 说的」当作免责理由为何不可接受？", options:["对外内容的责任仍在发布方，需人工审核与责任人确认","因为 AI 不会出错","因为法律没有规定","因为用户不在意"], answer:0, explain:"审核与责任必须落到人，AI 只是工具。" },
  ...
  { id:"st18", topic:"soc-cti-sharing", cat:"soccti", level:"中级", q:"对外共享威胁情报前，必须先做的是？", options:["脱敏，去掉内部资产、IP 与人员等敏感信息","先发给所有订阅方","先公开到社交媒体","先删除时间信息"], answer:0, explain:"共享即外传，未脱敏可能泄露自身资产与调查进展。" },
  { id:"st19", topic:"soc-triage", cat:"soccti", level:"中级", q:"共享情报时标注可信度与来源的价值是？", options:["让接收方知道该情报的证据强度与适用性","便于统计条数","便于美化报告","便于收费"], answer:0, explain:"缺可信度标注会让接收方误用低置信推断，造成误封与误判。" },
  { id:"st20", topic:"soc-detection", cat:"soccti", level:"高级", q:"收到外部情报后，让它产生价值的动作是？", options:["转化为检测规则或狩猎假设并做回归验证","直接归档","转发到群里","打印留存"], answer:0, explain:"情报价值在落地与验证，未验证的情报无法确定是否真的有效。" },
  { id:"sn1", topic:"soc-hunting", cat:"soccti", level:"高级", q:"当主机日志被清理时，网络侧数据能提供什么？", options:["通信对象、时间窗与数据量等元数据证据","完整的文件内容（加密时也总能解密）","操作系统日志","用户口令"], answer:0, explain:"网络元数据在主机证据缺失时是关键的替代与交叉验证来源。" },
  { id:"sn2", topic:"soc-net-forensics", cat:"soccti", level:"高级", q:"流量回溯的正确起点是？", options:["先定位时间窗与相关主机，再提取对应会话","直接导出全部流量","先看 DNS 日志","先重启设备"], answer:0, explain:"按时间与五元组收窄范围，才能高效定位并重建会话。" },
  { id:"sn3", topic:"soc-baseline", cat:"soccti", level:"中级", q:"全流量留存与元数据留存应如何取舍？", options:["元数据长期留存用于基线与回溯，全流量按需开启并短留存","全部全流量长期留存","只保留 DNS 日志","只保留告警"], answer:0, explain:"在成本与调查能力之间取平衡，是流量留存设计的核心取舍。" },
  ...
  { id:"sh2", topic:"soc-hunt-data", cat:"soccti", level:"中级", q:"把常用狩猎查询沉淀成「查询库」的价值是？", options:["复用与迭代，避免每次从零写查询","便于统计查询数量","便于考核","减少存储"], answer:0, explain:"查询库让狩猎经验可累积，也便于定期复跑发现变化。" },
  { id:"sh3", topic:"soc-baseline", cat:"soccti", level:"高级", q:"如何让数据采集策略跟上狩猎需求？", options:["把高频狩猎需求反馈给采集侧，按需补齐字段与留存","只增加存储容量","只买新工具","加密所有日志"], answer:0, explain:"采集与狩猎之间要有反馈闭环，才能持续提升可见性。" },

  // ---- v1.5.3 批次 2b：binary/crypto/pentest 补齐到 20 ----
  { id:"bs1", topic:"bin-static", cat:"binary", level:"中级", q:"静态分析寻找内存漏洞时，最优先关注的函数类型是？", options:["不受长度限制的拷贝与格式化函数","数学运算函数","界面渲染函数","日志打印函数"], answer:0, explain:"这类函数对外部输入长度不做约束，是溢出类漏洞的高发点。" },
  { id:"bs2", topic:"bin-static", cat:"binary", level:"中级", q:"判断「用户输入能否走到危险函数」的过程称为？", options:["数据流追踪（污点分析思路）","模糊测试","符号执行","逻辑回归"], answer:0, explain:"从入口到危险点的可达性是漏洞成立的前提，也是静态分析的核心工作。" },
  ...
  { id:"bk1", topic:"bin-kernel", cat:"binary", level:"高级", q:"内核驱动漏洞最常见的成因是？", options:["对用户传入的参数（长度、指针、类型）校验不足","代码注释太少","函数命名不规范","缺少单元测试"], answer:0, explain:"用户态可控参数未校验会导致越界读写、任意地址访问等严重后果。" },
  { id:"bk2", topic:"rop", cat:"binary", level:"高级", q:"内核漏洞利用通常还需要绕过哪些保护？", options:["SMEP/SMAP、KASLR、CFI 等缓解机制","HTTPS 证书校验","防火墙策略","反病毒特征"], answer:0, explain:"内核侧缓解更强，利用门槛高但一旦成功即取得最高权限。" },
  { id:"bk3", topic:"bin-kernel", cat:"binary", level:"高级", q:"从防御角度降低内核漏洞影响的方向是？", options:["最小化驱动加载、要求签名、及时更新并开启缓解机制","关闭所有日志","降低内核版本","禁用内存保护"], answer:0, explain:"减少可加载的驱动面并保持更新，能显著缩小内核攻击面。" },
  { id:"cr1", topic:"rand", cat:"crypto", level:"中级", q:"使用时间戳作为随机数种子会导致？", options:["生成的令牌或密钥可被预测","随机数变慢","需要更多内存","加密强度提升"], answer:0, explain:"可预测的随机等于没有随机，令牌与密钥都可能被推算。" },
  { id:"cr2", topic:"rand", cat:"crypto", level:"中级", q:"签名中的 nonce（随机数）被重复使用会有什么后果？", options:["可能直接泄露签名私钥","签名变慢","签名验证失败","密钥自动轮换"], answer:0, explain:"这是密码学中著名的致命错误，nonce 必须每次唯一且不可预测。" },
  { id:"cr3", topic:"rand", cat:"crypto", level:"高级", q:"容器或虚拟机启动初期熵不足的风险是？", options:["随机数质量下降，可能生成可预测的密钥或令牌","启动变慢","内存占用升高","网络不通"], answer:0, explain:"熵不足是云上容易被忽视的随机数风险，敏感生成应等待熵就绪。" },
  { id:"cs1", topic:"crypto-side-channel", cat:"crypto", level:"高级", q:"比较口令或令牌时禁止使用普通字符串比较，原因是？", options:["普通比较会提前返回，时间差可被用来逐字节推断","普通比较不支持中文","速度太慢","占用内存过多"], answer:0, explain:"必须使用常量时间比较函数，避免时间侧信道泄露。" },
  ...
  { id:"cs3", topic:"blockmode", cat:"crypto", level:"高级", q:"padding oracle 类攻击的成立条件是？", options:["解密失败与填充错误返回了可区分的响应","网络延迟过高","密钥长度不足","使用了哈希"], answer:0, explain:"统一解密失败响应、并优先使用 AEAD 模式可消除该类攻击面。" },
  { id:"pc1", topic:"priv-esc", cat:"pentest", level:"高级", q:"云环境渗透最常见的突破口是？", options:["暴露在公网的服务与存储，以及泄露的访问密钥","操作系统内核漏洞","物理机房进入","无线信号干扰"], answer:0, explain:"云上问题以「配置与身份」为主，而非传统主机漏洞。" },
  { id:"pc2", topic:"pt-cloud", cat:"pentest", level:"高级", q:"拿到云上临时凭证后的第一步应是？", options:["枚举「我是谁、能做什么」，搞清权限边界","立即删除资源","对外公开凭证","先扫描全内网"], answer:0, explain:"权限枚举决定后续可行路径，也能避免越界操作。" },
  { id:"pc3", topic:"cloud-basics", cat:"pentest", level:"中级", q:"SSRF 打云元数据之所以高价值，是因为？", options:["可获取实例临时凭证，进而访问云资源","能让实例重启","可修改 DNS","可提升带宽"], answer:0, explain:"云上 SSRF 常直接升级为凭证窃取与资源控制，应强制 IMDSv2 缓解。" },
    { id:"bin-asm-q1", topic:"bin-asm", cat: "binary", level: "入门", q: "在 Linux x86-64（System V 约定）下，函数调用的第一个整数参数放在哪个寄存器？",
    ...
    { id:"bin-gdb-q1", topic:"race", cat: "binary", level: "入门", q: "程序崩溃后想知道「崩在哪个函数、被谁调用」，最直接的命令是？",
      options: ["bt（打印调用栈）", "run", "quit", "make"], answer: 0,
      explain: "bt（backtrace）打印调用栈，能立刻看到崩溃点与调用链，是崩溃定位的第一步。" },
    { id:"bin-gdb-q2", topic:"bin-gdb", cat: "binary", level: "初级", q: "要抓「某块内存被谁越界写坏」，比翻日志更有效的做法是？",
    ...
    { id:"bin-format-q1", topic:"bin-format", cat: "binary", level: "入门", q: "同一个程序，为什么「改可执行文件里的表格」也可能改变它的行为？",
    ...
    { id:"bin-format-q2", topic:"bin-format", cat: "binary", level: "初级", q: "判断一个 ELF 是否启用了 PIE（地址随机化），看哪一项最快？",
    ...
    { id:"bin-format-q3", topic:"rop", cat: "binary", level: "中级", q: "程序里某个段的权限是「可写且可执行」（W+X），安全上意味着什么？",
    ...
    { id:"bin-prim-q1", topic:"fmt", cat: "binary", level: "中级", q: "堆利用中常说的「先泄后写」指的是什么顺序？",
    ...
    { id:"bin-prim-q2", topic:"bin-heap", cat: "binary", level: "高级", q: "为什么拿到任意写之后，常改「保护机制/回调类数据结构」而不是直接写代码指针？",
    ...
    { id:"bin-prim-q3", topic:"heap", cat: "binary", level: "高级", q: "针对伪造堆链表（如 tcache poisoning）最直接的缓解手段是？",
    ...
    { id: "blue-sample-q1", cat: "blue", level: "入门", q: "判定一个可疑文件是否恶意，最稳妥的第一步是？",
      options: ["先丢进隔离环境跑一遍看行为", "先重命名看有没有用", "先用杀软扫一下再直接双击打开", "先看文件大小"], answer: 0,
      explain: "先隔离再分析是铁律；直接在办公机上双击是最危险的做法。静态看与沙箱动态跑都要在隔离环境里做。" },
    { id: "blue-sample-q2", cat: "blue", level: "初级", q: "静态分析一个样本时，用 `file` 命令先看真实类型，意义是？",
      options: ["文件真实格式不受扩展名欺骗，能判断该用哪套分析工具", "能直接看出是不是病毒", "能算出哈希", "能解密样本"], answer: 0,
      explain: "扩展名可以随便改；文件头决定的真实格式才可靠，也决定后续用 PE 还是 ELF 的分析方法。" },
    { id:"blue-sample-q3", topic:"blue-sample", cat: "blue", level: "中级", q: "样本在沙箱里立刻「休眠」不做事，最可能的原因是？",
      options: ["检测到虚拟/分析环境后故意潜伏", "样本已损坏", "沙箱网络不通", "样本只针对特定语言"], answer: 0,
      explain: "反沙箱是常见对抗：检测到 CPU 核数、内存、驱动名或缺少用户交互就潜伏。对策是延长观察、模拟交互、换环境复跑。" },
    { id:"blue-forensics-q1", topic:"blue-forensics", cat: "blue", level: "初级", q: "主机取证时，为什么强调「先取内存再关机动磁盘」？",
      options: ["内存里的进程与连接关机即失，磁盘镜像可以稍后做", "内存更重要所以先算哈希", "关机会损坏磁盘", "磁盘镜像不需要写保护"], answer: 0,
      explain: "易失性顺序：内存/进程 → 网络连接 → 会话与历史 → 磁盘与日志。关机让大量线索永久消失。" },
    { id:"blue-forensics-q2", topic:"blue-forensics", cat: "blue", level: "中级", q: "把文件时间戳、登录时间与进程启动时间对齐到一张表，这种方法的名称与价值是？",
      options: ["时间线分析——能让异常活动自己浮现", "哈希校验——能证明文件未改", "内存取证——能找注入痕迹", "基线核查——能发现配置漂移"], answer: 0,
      explain: "时间线（timeline）是主机取证的核心方法：多源时间统一后，异常时间点的行为关系一眼可见。" },
    { id: "blue-forensics-q3", cat: "blue", level: "高级", q: "攻击者删掉了系统日志以掩盖痕迹，还能从哪里找线索？",
      options: ["文件系统残留（USN 日志、预取文件、注册表）与内存镜像", "已经完全无法追溯", "只能问员工", "查看防火墙厂商的默认规则"], answer: 0,
      explain: "日志被删不等于痕迹消失：USN/预取/注册表与内存镜像常能反推出执行历史，这也是取证先取内存的原因之一。" },
    { id:"ds-compliance-q1", topic:"ds-compliance", cat: "datasec", level: "入门", q: "关于等保（等级保护），下面哪种理解更准确？",
      options: ["按系统重要程度定级，再按等级落实要求，并留下可查证的记录", "买齐安全设备就能通过", "只对金融行业适用", "一次测评通过后永久有效"], answer: 0,
      explain: "等保的核心是「定级 → 落实 → 可查证」，不是设备清单；系统或环境变化后需要重新评估。" },
    { id:"ds-compliance-q2", topic:"ds-audit", cat: "datasec", level: "初级", q: "等保检查中，审计日志方面最常见的失分点是？",
      options: ["只在应用层记录，系统层与关键操作没有留痕，且留存时间不足", "日志文件太大", "日志格式不是 JSON", "日志存放在本地磁盘"], answer: 0,
      explain: "要求是「关键操作可追溯」：系统层、数据库层、管理操作都要留痕，且留存周期要满足硬指标。" },
    { id:"ds-compliance-q3", topic:"ds-audit", cat: "datasec", level: "中级", q: "面对一堆合规条款，整改时优先做哪类工作？",
      options: ["优先做「可查证」的部分：日志、备份、权限清单", "先写制度文档，技术措施以后再说", "先买设备", "先做渗透测试"], answer: 0,
      explain: "可查证的证据（日志留存、可恢复的备份、最小权限清单）既是硬指标，又能被直接核验，性价比最高。" },
    { id:"ds-thirdparty-q1", topic:"ds-thirdparty", cat: "datasec", level: "初级", q: "把数据交给第三方处理时，下列哪种做法责任边界最清楚？",
      options: ["委托处理：由你确定处理目的与方式，对方按约定执行", "共同处理：双方都能决定用途", "直接把数据库账号给对方", "口头约定即可"], answer: 0,
      explain: "委托处理的职责边界更明确；共同处理意味着双方都对处理目的负责，风险更高。" },
    { id:"ds-thirdparty-q2", topic:"ds-gateway", cat: "datasec", level: "中级", q: "与第三方共享数据时，技术层面最该做的三件事是？",
      options: ["字段最小化与脱敏、约定删除与返还、出口可观测并留痕", "加密后直接全量给对方", "只签保密协议", "定期索要对方的安全报告"], answer: 0,
      explain: "管理手段之外必须有技术手段：少给字段、能删可证、出口能审计——三者缺一都会留下盲区。" },
    { id: "ds-thirdparty-q3", cat: "datasec", level: "高级", q: "担心数据被合作方二次外泄，哪种技术手段能帮助溯源？",
      options: ["对共享数据加水印/指纹，外泄时可比对定位来源", "要求对方承诺不外泄", "把数据加密封装后给对方密码", "只在合同里写明罚则"], answer: 0,
      explain: "水印/指纹（含微小的字段或记录级扰动）能在事后比对出泄漏来源，是比「承诺」更硬的约束。" }
