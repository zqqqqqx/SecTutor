...
> 版本：v1 · 动效强度档位：**明显**（250–350ms，带明显位移与缩放）
> 适用：`cybersec-agent/`（前端单页 + Electron 壳 + 单文件 `sec-tutor.html`）
> 状态：**待评审**，评审通过后按 P0 → P3 顺序实施
...
深色主题下 `--muted: #64748b` 在 `--panel: #111824` 上的对比度约 4.3:1，接近 AA 下限。建议将深色主题的 `--muted` 提亮至 `#8b98ad`（约 6:1）。
...
| 批次 | 内容 | 新增断言 |
| --- | --- | --- |
| 第 1 批 | P0（Toast + loading + 骨架屏） | +7 |
| 第 2 批 | P1（快捷键 + 焦点管理） | +7 |
| 第 3 批 | P2（滚动记忆 + 撤销 + 草稿） | +5 |
| 第 4 批 | P3（微交互 + 无障碍 + 对比度） | +6 |
